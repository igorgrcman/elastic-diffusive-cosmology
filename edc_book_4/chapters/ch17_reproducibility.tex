...
\textbf{Purpose:} Specify exact procedures for reproducing all numerical claims
in Book IV; provide verification tiers and contamination scan protocols.
\textbf{Inputs:} All results from Ch.~\ref{ch:anchor}--\ref{ch:high_coord};
calibration data from App.~Q.
\textbf{Outputs:} (i)~Verification tier definitions (Tier 1/2/3)~[Def],
(ii)~reproduction recipes for each key result, (iii)~contamination scan protocol.
\textbf{Dependencies:} Ch.~\ref{ch:unified} (synthesis).
\textbf{Forward links:} None (final chapter).
\textbf{Epistemic note:} This chapter is \emph{meta-procedural}---no new physics.
...
\begin{tabular}{llc}
    ...
    \midrule
    \LaTeX\ sources & \texttt{./} (book root) & 1 \\
...
\begin{tabular}{llll}
    ...
    $L_0/\delta = \pi^2$ & Deterministic & \tagDer & Ch.~\ref{ch:L0delta} \\
...
\begin{itemize}
    ...
    \item $L_0/\delta = \pi^2$ (Ch.~\ref{ch:L0delta})
...
\textbf{Procedure:}
\begin{enumerate}
    \item Apply Z$_6$ crystallization formula (Ch.~\ref{ch:junction})
...
\textbf{Ingredients:}
\begin{itemize}
    \item M6 coordination geometry (Ch.~\ref{ch:junction})
    \item Junction energetics from anchor state (Ch.~\ref{ch:anchor})
    \item Degeneracy accounting (junction-mode multiplicities)
\end{itemize}
...
These terms require routing to Appendix~\ref{app:analogies} if needed:
