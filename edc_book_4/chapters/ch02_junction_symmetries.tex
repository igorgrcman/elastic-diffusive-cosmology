...
\textbf{Purpose:} Derive the junction symmetry group $G_{\text{jun}} \cong \Zsix$
from 5D crystallization and unpack its subgroup structure; introduce the
$\Msix$ coordination lattice.
\textbf{Inputs:} (i)~Anchor junction from Ch.~\ref{ch:anchor}~[Dc],
(ii)~Postulate P2 (repulsion + confinement)~[P], (iii)~Kepler--Hales packing~[M].
\textbf{Outputs:} (i)~$G_{\text{jun}} \cong \Zsix$~[Dc], (ii)~subgroup chain
$\Zsix \supset \Zthree \supset \Ztwo$~[Der], (iii)~$\Msix$ lattice with $n=6$~[Der],
(iv)~allowed set $S = \{2^a \times 3^b\}$~[Der].
\textbf{Dependencies:} Ch.~\ref{ch:anchor} ($\Zsix$ ground state).
\textbf{Forward links:} $\Zthree$ branch to Ch.~\ref{ch:metastable};
$\Msix$ coordination to Ch.~\ref{ch:M6}; allowed set $S$ to Ch.~\ref{ch:frustrated}.
...
\begin{derivationbox}[title={Derivation: \texorpdfstring{$n = 6$}{n = 6} from Graph Duality}]
    \textbf{Claim:} Each vertex in the dual graph $G^*$ has degree 6.

    \textbf{Proof:}
    \begin{enumerate}
        \item The primal graph $G$ is 3-valent (each Y-junction has 3 legs).
        \item At each vertex of $G$, the three edges meet at $120^\circ$ angles.
        \item The exterior angle at each vertex is $180^\circ - 120^\circ = 60^\circ$.
        \item For a closed face, total exterior angle $= 360^\circ$.
        \item Minimum vertices per face $= 360^\circ/60^\circ = 6$.
...
What the observer records: the $\Zsix$ and $\Zthree$ junction states appear
as distinct stable/metastable species with measured masses. The symmetry
group structure manifests as discrete quantum numbers at the projection level.
\end{observerbox}
...
% ----------------------------------------------------------------------------
\paragraph{Bridge to Chapter~\ref{ch:metastable}.}
The subgroup chain $\Zsix \supset \Zthree$ suggests a natural question: what
happens when the junction configuration breaks from full $\Zsix$ symmetry to
the $\Zthree$ subgroup? Chapter~\ref{ch:metastable} shows that the $\Zthree$
branch corresponds to a \emph{metastable} junction---an excited configuration
sitting in a double-well potential whose barrier height determines the
tunneling rate. This is the first step toward the instanton calculation that
ultimately predicts the metastable lifetime $\tau_n$.
