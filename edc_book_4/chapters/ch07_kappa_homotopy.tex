...
\textbf{Purpose:} Derive the instanton prefactor $\kappa = 2\pi$ from winding
number quantization in the compact fifth dimension.
\textbf{Inputs:} (i)~$\SE$ structure from Ch.~\ref{ch:instanton}~[Dc],
(ii)~compact $\Sone$ topology of fifth dimension~[P], (iii)~fundamental group theorem $\pione(\Sone) = \mathbb{Z}$~[M].
\textbf{Outputs:} (i)~Winding number $w \in \mathbb{Z}$~[Der],
(ii)~$\kappa = 2\pi$ from winding quantization~[Dc].
\textbf{Dependencies:} Ch.~\ref{ch:instanton} ($\SE$ formula structure).
\textbf{Forward links:} $\kappa = 2\pi$ to Ch.~\ref{ch:tau_n} ($\taun$ assembly).
...
% ----------------------------------------------------------------------------
\paragraph{Bridge to Chapter~\ref{ch:L0delta}.}
With $\kappa = 2\pi$ established, the remaining unknown in the Euclidean
action is the geometric ratio $L_0/\delta$. Chapter~\ref{ch:L0delta}
develops the hypothesis $L_0/\delta = \pi^2$ from 5D geometric arguments.
Unlike $\kappa$, this ratio is currently [I]---identified from consistency
rather than rigorously derived. The two factors combine in Ch.~\ref{ch:tau_n}
to predict $\taun$.
