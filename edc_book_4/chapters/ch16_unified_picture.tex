...
\textbf{Purpose:} Synthesize all derivation chains from Book IV; present the
complete epistemic ledger and derivation tree.
\textbf{Inputs:} All results from Ch.~\ref{ch:anchor}--\ref{ch:high_coord}.
\textbf{Outputs:} (i)~Derivation tree diagram~[summary], (ii)~epistemic ledger
with tags for each claim, (iii)~OPEN problem inventory.
\textbf{Dependencies:} All previous chapters.
\textbf{Forward links:} Reproducibility procedures in Ch.~\ref{ch:repro}.
\textbf{Epistemic note:} This chapter is \emph{synthesis only}---no new physics.
...
\begin{tabular}{lp{2.5cm}p{3.5cm}ccc}
    ...
    J$'$ & Closure definition & \ClosedFour{} minimality theorem &
        \ref{ch:helium4} & [Der] & N \\
    K & Nodes I,J,J$'$ & Composition principle, emission bias &
...
% ----------------------------------------------------------------------------
\paragraph{Bridge to Chapter~\ref{ch:repro}.}
The physics content of Book IV is now complete. But reproducibility is
paramount: Chapter~\ref{ch:repro} specifies the exact procedures for
verifying all numerical claims, from Tier~1 (compile-only) through
Tier~3 (code-assisted reproduction). The chapter also provides the
contamination scan protocol that ensures Layer~A purity is maintained.
