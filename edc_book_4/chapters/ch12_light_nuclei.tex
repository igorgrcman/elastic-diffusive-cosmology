...
\textbf{Purpose:} Extend the pinning framework to $A = 2$--10 clusters; establish
the composition principle based on closed-4 building blocks.
\textbf{Inputs:} (i)~Closed-4 unit from Ch.~\ref{ch:helium4}~[Der],
(ii)~$\Kpin$ from Ch.~\ref{ch:sigma_to_K}~[Dc], (iii)~minimality theorem from Ch.~\ref{ch:helium4}~[Der].
\textbf{Outputs:} (i)~Closure-based taxonomy (open/closed/composite)~[Dc],
(ii)~A=8 instability prediction~[Dc], (iii)~composition principle for $A > 4$~[P].
\textbf{Dependencies:} Ch.~\ref{ch:helium4} (closed-4 unit).
\textbf{Forward links:} Composition principle to Ch.~\ref{ch:barrier_release} (baseline lane).
...
% ----------------------------------------------------------------------------
\paragraph{Bridge to Chapter~\ref{ch:barrier_release}.}
The composition principle explains \emph{static} binding patterns. But
what about \emph{dynamical} processes---specifically, the release of closed-4
units from large clusters? Chapter~\ref{ch:barrier_release} establishes
the baseline law for barrier-limited release, parametrized by barrier and
release-energy variables. This baseline is empirical [BL]; EDC enters via
the frustration correction in Chapter~\ref{ch:frustrated}.
