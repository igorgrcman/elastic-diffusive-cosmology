...
\begin{resultbox}
    ...
    conditions in the minimal thin-junction model do not generate it
    (Appendix~\ref{app:P2_WP2_Israel}); the thick-junction/internal-core
    route yields bounded insufficiency---the energy scale is right but
    monotone core profiles cannot oppose the geometric restoring force
    (Appendix~\ref{app:P2_N7_core}); the non-monotone core-profile
    loophole is physically empty---the sign argument extends to the
    general (non-separable) case, and no admissible mechanism within
    $S_{\mathrm{EH}} + S_{\mathrm{NG}}$ breaks the $q = 0$ core
    minimum (Appendix~\ref{app:P2_NMCP}); bulk gravitational
    backreaction (N2) is structurally $\kappa_5^2$-suppressed---cross-term
    and LO self-energy reduce to tension renormalization, and NLO shape
    corrections require $\Mfive \lesssim 100\MeV$ for relevance
    (Appendix~\ref{app:P2_N2_bulk}).
