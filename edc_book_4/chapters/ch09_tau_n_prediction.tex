...
\textbf{Outputs:} (i)~$\SE/\hbar = 2\pi^3 \approx 62$~[Dc$\times$P],
(ii)~$\taun \approx 880\second$~[Dc+P+Cal].
...
and quarantined in Appendix~\ref{app:quarantine}. It is \emph{not} adjusted to fit
the observed $\taun$. The exponent $\SE/\hbar = 2\pi^3$ is fixed by the derivation
chain (Chapters~\ref{ch:kappa}--\ref{ch:L0delta}). Sensitivity analysis in
\S\ref{sec:taun:sensitivity} demonstrates robustness to prefactor variations.
