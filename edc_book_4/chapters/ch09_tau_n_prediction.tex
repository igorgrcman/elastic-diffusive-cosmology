...
\begin{resultbox}
    ...
    \begin{itemize}
        ...
        \item Achieved $< 1\%$ agreement with measured value
              (contingent on [P] hypothesis $\Lzero/\bdelta = \pi^2$
              and [Cal] prefactor $A \approx 0.9$)
        \item Identified dominant uncertainty: $\Lzero/\bdelta = \pi^2$ [P];
              model-dependent analysis shows this is one candidate in a
              continuous family (Appendix~\ref{app:L0delta_bvp})
    \end{itemize}

    \textbf{Key insight:} The factor $\exp(62) \approx 10^{27}$ converts the
    junction timescale ($10^{-23}\second$) to the macroscopic lifetime
    ($10^{3}\second$). The exponent is dominated by topology ($\kappa = 2\pi$~[Dc])
    and a geometric candidate ($\Lzero/\bdelta = \pi^2$~[P]); the prefactor
    $A$~[Cal] contributes $< 0.1$~dex. Full closure requires deriving
    $\Lzero/\bdelta$ and $A$ from first principles (both [OPEN]).
\end{resultbox}
...
\bigskip
\noindent
\textbf{Derivation chain (Phase~1 status --- partially closed):}
\begin{equation*}
...
The metastable sector is partially closed: $\taun \approx 880\second$ is assembled
from instanton tunneling through the double-well barrier, with two open steps
($\Lzero/\bdelta$ derivation and prefactor $A$) remaining. We now turn to
