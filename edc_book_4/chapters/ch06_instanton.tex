...
\textbf{Preview:} With $\kappa = 2\pi$~[Dc] and the candidate
$\Lzero/\bdelta \approx \pi^2$~[P] (one value in a continuous family;
see Ch.~\ref{ch:L0delta} and Appendix~\ref{app:L0delta_bvp}):
\begin{equation}
...
\textbf{Chapter~\ref{ch:tau_n}} assembles all parameters and evaluates
$\taun \approx 880\second$ (using the candidate $\Lzero/\bdelta = \pi^2$~[P]
and calibrated $A \approx 0.9$~[Cal]).
