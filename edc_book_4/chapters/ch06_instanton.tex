...
\begin{itemize}
    ...
    \item Metastable: $q = q_n$ (local minimum, $V = \Delta m_{np}$)
    \item Barrier: $q = q_B$ (saddle point, $V = 3\Delta m_{np}$)
\end{itemize}
...
\begin{derivationbox}[title=Effective Mass]
    ...
    (constant), with $M$ of order the junction mass scale.
    \tagDc
\end{derivationbox}
...
As established in Chapter~\ref{ch:metastable}, $V(q)$ has double-well structure
with minima at $q = 0$ (anchor) and $q = q_n$ (metastable), and a maximum at
$q = q_B$ (barrier).
...
$M(q)$ & Effective mass & [P] (constant approx.) & Eq.~\eqref{eq:Seff} \\
$V(q)$ & Effective potential & [P] (double-well) & Eq.~\eqref{eq:Seff} \\
