...
% v2 — PRC Submission Ready — 2026-03-16
%
\documentclass[aps,prc,twocolumn,showpacs,preprintnumbers,amsmath,amssymb]{revtex4-2}
...
$\langle|\Delta|\rangle = 0.48$\,dex---a $16\times$ improvement over
...
\pacs{21.10.-k, 23.60.+e, 21.60.Cs}
\keywords{superheavy elements, alpha decay, Geiger--Nuttall law,
  topological pinning, coordination frustration}
...
The standard Geiger--Nuttall (GN) law~\cite{Geiger1911,Gamow1928},
\begin{equation}
...
(EDC)~\cite{EDCPartI,EDCPaper2,EDCBook4}.  The correction introduces a
...
Elastic diffusive cosmology (EDC)~\cite{EDCPartI,EDCPaper2}
models baryons as topological defects---Y-shaped
junctions---of a three-dimensional membrane (brane) embedded in a
five-dimensional bulk spacetime with topology
$\mathcal{M}^4 \times S^1$.  The fifth dimension $\xi$ is
compactified on a circle of radius $R_\xi$.

Within this framework, nuclear binding arises from contact
interactions between adjacent junctions on the brane.  The
junction arrangement inherits a discrete $\mathbb{Z}_6$
rotational symmetry from the sixfold symmetry of the Y-junction
geometry~\cite{EDCBook4}.  This symmetry constrains which
coordination numbers are compatible with the lattice, defining
an \emph{allowed set}
\begin{equation}
  ...
  \label{eq:S}
\end{equation}
The group factorization
$\mathbb{Z}_6 \cong \mathbb{Z}_3 \times \mathbb{Z}_2$ generates
this set through independent doublings and triplings of the
unit coordination.

The key physical insight for nuclear physics is that nuclei
whose effective coordination number falls \emph{between} elements
of~$S$ experience \emph{coordination frustration}: the junction
lattice cannot accommodate the nucleus's coordination geometry
without strain, enhancing the tunneling probability for
$\alpha$~decay.

This paper uses only the frustration correction $g \cdot d(n)$
as a phenomenological input; all EDC-specific parameters
($R_\xi$, membrane tension~$\sigma$, junction thickness
$\delta_J$) enter only through the motivation for the allowed
set~$S$.  The model is therefore testable independently of the
broader EDC framework.

\subsection{Frustration-corrected Geiger--Nuttall law}
\label{sec:model}

The effective coordination number of a nucleus with mass
number~$A$ is
\begin{equation}
  n(A) = p \cdot A^{1/3},
  \label{eq:nA}
\end{equation}
with prefactor $p = 6.1$ calibrated to $n(208) \approx 36 =
2^2 \times 3^2$ for ${}^{208}$Pb---the heaviest doubly-magic
nucleus and the most stable heavy $\alpha$-emitter.
...
nuclei with large $\dn$ are \emph{frustrated}, enhancing
...
recent experimental updates~\cite{Gates2024}.
The dataset spans 18 elements, half-lives from $\sim 10^{-7}$\,s
to $\sim 10^{17}$\,s ($\sim 24$ orders of magnitude),
$\Qalpha$ from $\sim 4$ to $\sim 10$\,MeV, and includes
82 allowed (H0), 8 first-forbidden (H1), and
...
The model achieves $R^2 = 0.980$ on the training set and
$R^2_{\mathrm{CV}} = 0.971$ under 10-fold cross-validation,
with RMSE $= 0.810$\,dex and AIC $= 266.2$.
...
Figure~\ref{fig:validation} presents the validation results
graphically: panel~(a) shows predicted vs.\ experimental
half-lives for the six OOS isotopes alongside the GN baseline,
and panel~(b) shows predictions for $Z = 119$ and $120$.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/superheavy_validation}
\caption{(a) Out-of-sample validation: predicted vs.\
  experimental $\log_{10}(\thalf/\text{s})$ for six measured
  superheavy isotopes ($Z=114$--$118$).  Filled blue circles:
  frustration-corrected model (this work); open red squares:
  standard Geiger--Nuttall baseline.  Gray band: $\pm 1.5$\,dex.
  (b) Predictions for undiscovered isotopes ${}^{298}119$,
  ${}^{302}120$, and ${}^{304}120$ ($N=184$).  Error bars
  represent $\pm 1.3$\,dex total uncertainty.  Green band:
  range from Sobiczewski \emph{et al.}~\cite{Sobiczewski2007}.}
...
The total uncertainty on $\log_{10}\thalf$ combines three sources:
(i) $\Qalpha$ propagation,
  $\sigma_Q = |a \cdot \Zd / (2 \Qalpha^{3/2})| \times
  \delta\Qalpha \approx 0.8$\,dex for $\delta\Qalpha = 0.5$\,MeV;
(ii) model coefficient uncertainty,
  $\sigma_{\mathrm{model}} \approx 0.9$\,dex (from SE of $a$, $g$,
  $b$ propagated through the linear model); and
(iii) out-of-sample scatter,
...
\paragraph{JINR Dubna.}
The Superheavy Element Factory with its
...
\paragraph{GSI Darmstadt.}
SHIP and the TASCA separator target $Z = 119$ via
...
For ${}^{298}119$ ($\thalf \sim 0.6$\,s): easily
...
(1)~\emph{Power-law coordination:} $n(A) = p \cdot A^{1/3}$
  with constant $p$---the simplest ansatz consistent
  with nuclear volume scaling.
(2)~\emph{Hexagonal allowed set:} $S = \{2^a \times 3^b\}$,
  derived from $\mathbb{Z}_6$ symmetry of the
  Y-junction lattice.
(3)~\emph{Linear frustration coupling:} The correction is
  linear in $\dn$; nonlinear terms are not included.
(4)~\emph{$\alpha$-decay dominance:} We predict
...
experimental mass measurements of neighboring nuclei
(e.g., via Penning traps at ISOLDE or TITAN) or
improved global mass models with better SHE
extrapolation.
...
The model does not address production cross-sections
(which require reaction theory), branching ratios between
$\alpha$~decay and spontaneous fission, excited-state
properties, or nuclei below $A \sim 200$ (where frustration
effects are small).
...
The author thanks the developers of the NUBASE2020 and
AME2020 evaluations for publicly available nuclear data.
Statistical analysis and manuscript preparation were
performed with assistance from Claude (Anthropic).
The EDC framework is developed as an independent
research program; prior publications are available at
Zenodo~\cite{EDCPartI,EDCPaper2}.
\end{acknowledgments}
...
\bibitem{EDCPartI}
  I.~Gr\v{c}man,
  \emph{Elastic Diffusive Cosmology, Part~I},
  Zenodo (2026),
  \href{https://doi.org/10.5281/zenodo.18176174}{doi:10.5281/zenodo.18176174}.

\bibitem{EDCPaper2}
  I.~Gr\v{c}man,
  \emph{Elastic Diffusive Cosmology, Paper~2: Membrane Tension
...
\bibitem{ViolaSeaborg}
  V.~E.~Viola and G.~T.~Seaborg,
  J.\ Inorg.\ Nucl.\ Chem.\ \textbf{28}, 741 (1966).

\bibitem{Royer2000}
  G.~Royer,
  J.\ Phys.\ G \textbf{26}, 1149 (2000).

\bibitem{Dong2010}
  T.~Dong and Z.~Ren,
  Eur.\ Phys.\ J.\ A \textbf{44}, 247 (2010).

\bibitem{Qi2009}
  C.~Qi, F.~R.~Xu, R.~J.~Liotta, and R.~Wyss,
  Phys.\ Rev.\ Lett.\ \textbf{103}, 072501 (2009).
