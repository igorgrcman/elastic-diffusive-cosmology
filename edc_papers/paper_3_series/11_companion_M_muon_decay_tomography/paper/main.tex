...
\begin{tcolorbox}[edcGuardrail, title=\textbf{Epistemic Guardrail}]
\textbf{No ``bulk escape'' channel:} In muon decay, we postulate \tagP{} that
energy cannot escape back into the bulk-core. This is consistent with the
muon being brane-dominant: it has no bulk degrees of freedom to retreat to.
\end{tcolorbox}
...
\[
    \mu^- \to e^- + \nu_\mu + \bar{\nu}_e
\]
with branching ratio $\approx 100\%$ \tagBL{}.
...
\subsection{Forbidden Channels}

\begin{table}[htbp]
\centering
\caption{Forbidden muon decay channels and their suppression mechanism}
\label{tab:forbidden}
\begin{tabular}{lll}
\toprule
\textbf{Channel} & \textbf{Suppression} & \textbf{EDC interpretation} \\
\midrule
$\mu^- \to e^- + \gamma$ & $< 4.2 \times 10^{-13}$ \tagBL{} & Lepton flavor violation \\
$\mu^- \to e^- + e^+ + e^-$ & $< 1.0 \times 10^{-12}$ \tagBL{} & Mode incompatibility \\
$\mu^- \to e^- + \nu_e + \bar{\nu}_\mu$ & Forbidden & Wrong lepton numbers \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tcolorbox}[edcPathway, title=\textbf{Chiral Filter Mechanism}]
...
mode spectrum, acts as a \emph{chiral filter}: modes of one handedness
couple more strongly to the 3D projection than the other.

\medskip
\textbf{Consequence:} The weak interaction's ``maximal parity violation''
emerges from the \emph{geometric asymmetry} of the brane boundary, not
from an intrinsic property of the interaction vertex.
\end{tcolorbox}
