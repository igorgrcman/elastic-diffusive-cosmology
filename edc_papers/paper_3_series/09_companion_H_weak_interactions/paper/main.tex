...
% -----------------------------------------------------------------------------
\subsection{Formal Coupling Term: Bulk-Core to Brane-Layer}
\label{subsec:formal_coupling}
% -----------------------------------------------------------------------------

Having established the qualitative picture, we now introduce the minimal formal coupling between bulk-core dynamics and brane-layer modes.

\begin{definition}[Brane-Layer Field \Def]
\label{def:brane_field}
Let $\phi(y, t)$ denote the brane-layer field, where:
\begin{itemize}[nosep]
    \item $y \in [-\delta/2, +\delta/2]$ is the coordinate across the brane thickness
    \item $y = -\delta/2$: bulk-facing boundary
    \item $y = +\delta/2$: observer-facing boundary
    \item $t$ is the proper time on the brane
\end{itemize}
The field $\phi$ represents collective excitations (phonon-like modes) of the membrane.
\end{definition}

\begin{definition}[Minimal Bulk-Brane Coupling \Pp/\Open]
\label{def:minimal_coupling}
The interaction between the bulk-core coordinate $q(t)$ and the brane-layer field $\phi(y,t)$ at the bulk-facing boundary is:
\begin{equation}
\boxed{\mathcal{L}_{\mathrm{int}} = g \, q(t) \, \phi\bigl(y = -\tfrac{\delta}{2}, t\bigr)}
\label{eq:coupling_lagrangian}
\end{equation}
where $g$ is the effective coupling constant with dimensions $[g] = E \cdot L^{-1}$.

\textbf{Alternative (derivative coupling):}
\begin{equation}
\mathcal{L}_{\mathrm{int}}^{(\dot{q})} = g' \, \dot{q}(t) \, \phi\bigl(y = -\tfrac{\delta}{2}, t\bigr)
\label{eq:coupling_derivative}
\end{equation}
This form emphasizes ``pumping'' through junction motion rather than position.
\end{definition}

\begin{remark}[Epistemic Status of Coupling]
The coupling Lagrangian Eq.~\eqref{eq:coupling_lagrangian} is \Pp/\Open:
\begin{itemize}[nosep]
    \item \textbf{Postulated \Pp:} Not derived from the 5D action in this work
    \item \textbf{Open \Open:} The value of $g$ (or $g'$) is not determined
    \item \textbf{Structural:} The form is dictated by locality (coupling at bulk-facing boundary) and leading-order expansion
\end{itemize}
No numerical claims are made. This provides the ``socket'' for future derivations.
\end{remark}

% -----------------------------------------------------------------------------
\subsection{Energy Partition Ledger}
\label{subsec:energy_partition}
% -----------------------------------------------------------------------------

When a bulk-core transition occurs (e.g., junction relaxation $\ket{1} \to \ket{0}$), energy flows through the coupling into the brane-layer. We formalize this as an energy partition statement.

\begin{tcolorbox}[colback=green!5,colframe=green!50!black,title=\textbf{Energy Partition Rule \Dc/\Pp}]
\label{box:energy_partition}
For a bulk-core transition releasing energy $\Delta E_{\mathrm{bulk}}$:
\begin{equation}
...
where:
\begin{itemize}[nosep]
    \item $\Delta E_{\mathrm{bulk}}$: Energy released by bulk-core transition (e.g., $\Delta m_{np} c^2 = 1.293$ MeV for $n \to p$)
    \item $\Delta E_{\mathrm{brane\text{-}layer}}$: Energy deposited in brane-layer modes (becomes observed particle kinetic energy)
    \item $\Delta E_{\mathrm{residual}}$: Energy remaining in bulk-core recoil, internal modes, or unobserved channels
\end{itemize}

\medskip
\textbf{Status:}
\begin{itemize}[nosep]
    \item Conservation $\Delta E_{\mathrm{bulk}} = \Delta E_{\mathrm{brane\text{-}layer}} + \Delta E_{\mathrm{residual}}$: \Dc{} (follows from 5D closure)
    \item Partition mechanism (how energy divides): \Pp/\Open
\end{itemize}
\end{tcolorbox}

\begin{remark}[Observed Particles Come from Brane-Layer]
The 3D particles detected in experiments (electrons, neutrinos) receive their energy from $\Delta E_{\mathrm{brane\text{-}layer}}$ after projection through the observer-facing boundary. The partition fraction determines the spectrum shape.
\end{remark}

% -----------------------------------------------------------------------------
\subsection{Frozen Projection Operator}
\label{subsec:projection_operator}
% -----------------------------------------------------------------------------

The frozen projection boundary at $y = +\delta/2$ implements a mapping from brane-layer modes to observed 3D particle states.

\begin{definition}[Frozen Projection Operator \Def/\Dc]
\label{def:projection_operator}
The \textbf{frozen projection operator} $\mathcal{P}_{\mathrm{frozen}}$ acts on brane-layer field configurations at the observer-facing boundary:
\begin{equation}
\boxed{\mathcal{P}_{\mathrm{frozen}}: \quad \phi\bigl(y = +\tfrac{\delta}{2}, t\bigr) \;\longmapsto\; \{e^-, e^+, \nu, \bar{\nu}, \gamma, \ldots\}_{\mathrm{3D}}}
\label{eq:projection_operator}
\end{equation}
This is not a dynamical process but an \textbf{identification}: the observable content of the brane-layer excitation is what the 3D observer detects.
\end{definition}

\begin{definition}[Frozen Criterion \Def/\Dc]
\label{def:frozen_criterion}
A brane-layer mode with characteristic frequency $\omega$ is \textbf{frozen} (projected to a definite 3D state) when:
\begin{equation}
\boxed{\hbar\omega \gg E_{\mathrm{env}}}
\label{eq:frozen_criterion}
\end{equation}
where $E_{\mathrm{env}}$ is the relevant environmental energy scale (thermal $kT$, interaction energy, or probe resolution).

\medskip
\textbf{Consequence:} Modes satisfying Eq.~\eqref{eq:frozen_criterion} are adiabatically eliminated from the effective 3D dynamics---they appear as fixed particle states, not fluctuating fields.
\end{definition}

\begin{remark}[Projection is NOT Dynamics]
The operator $\mathcal{P}_{\mathrm{frozen}}$ does not describe a scattering process or a time evolution. It is a \textbf{dictionary} between descriptions:
\begin{itemize}[nosep]
    \item \textbf{5D description:} Brane-layer field $\phi(y,t)$ with mode decomposition
    \item \textbf{3D description:} Particle states $\{e^-, \nu, \ldots\}$ with definite quantum numbers
\end{itemize}
The ``freezing'' reflects that the 3D observer cannot resolve the internal brane-layer dynamics at scales $\ll \delta$.
\end{remark}

% -----------------------------------------------------------------------------
\subsection{Complete Pipeline: Diagram and Summary Table}
...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners, minimum width=2.5cm, minimum height=1.2cm, align=center},
    arrow/.style={->, thick, >=stealth}
]
    % Bulk-core box
    \node[box, fill=blue!15] (bulk) at (0,0) {\textbf{Bulk-Core}\\$q(t)$\\junction dynamics};

    % Coupling
    \draw[arrow, red!70!black, very thick] (bulk.east) -- ++(1.5,0)
        node[midway, above, font=\footnotesize] {$\mathcal{L}_{\mathrm{int}}$}
        node[midway, below, font=\footnotesize] {$y = -\delta/2$};

    % Brane-layer box
    \node[box, fill=yellow!25] (brane) at (5,0) {\textbf{Brane-Layer}\\$\phi(y,t)$\\modes in $\delta$};

    % Projection
    \draw[arrow, orange!80!black, very thick] (brane.east) -- ++(1.5,0)
        node[midway, above, font=\footnotesize] {$\mathcal{P}_{\mathrm{frozen}}$}
        node[midway, below, font=\footnotesize] {$y = +\delta/2$};

    % 3D output box
    \node[box, fill=green!15] (output) at (10,0) {\textbf{3D Outputs}\\$e^-, \bar{\nu}_e$\\observed particles};

    % Energy flow annotation
    \draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick]
        (0,-1.2) -- (10,-1.2)
        node[midway, below=8pt, font=\small] {$\Delta E_{\mathrm{bulk}} = \Delta E_{\mathrm{brane\text{-}layer}} + \Delta E_{\mathrm{residual}}$};

    % Epistemic tags
    \node[font=\footnotesize, gray] at (0,1.2) {\Pp};
    \node[font=\footnotesize, gray] at (5,1.2) {\Def};
    \node[font=\footnotesize, gray] at (10,1.2) {\BL};
\end{tikzpicture}
\caption{Complete pipeline for weak processes. Bulk-core dynamics (junction coordinate $q$) couple to brane-layer modes ($\phi$) at the bulk-facing boundary via $\mathcal{L}_{\mathrm{int}}$. The frozen projection operator $\mathcal{P}_{\mathrm{frozen}}$ at the observer-facing boundary maps brane-layer excitations to observed 3D particle states.}
...
\begin{tabular}{llll}
\toprule
\textbf{Object} & \textbf{Where It Lives} & \textbf{What Carries Energy} & \textbf{Tag} \\
\midrule
Junction state $\ket{0}, \ket{1}$ & Bulk-core (5D bulk) & Config.\ potential $V(q)$ & \Pp \\
Brane-layer mode $\phi(y,t)$ & Brane thickness $\delta$ & Mode excitations & \Def \\
Coupling $\mathcal{L}_{\mathrm{int}}$ & Bulk-facing ($y = -\delta/2$) & Energy transfer $g \cdot q \cdot \phi$ & \Pp/\Open \\
Projection $\mathcal{P}_{\mathrm{frozen}}$ & Observer-facing ($y = +\delta/2$) & Identification map & \Def/\Dc \\
Observed particle $e^-, \nu$ & 3D spacetime & Kinetic + rest mass & \BL \\
\bottomrule
\end{tabular}
\end{center}
