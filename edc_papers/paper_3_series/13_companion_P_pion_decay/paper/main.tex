...
This companion addresses \emph{leptonic} pion decays only. Hadronic modes
...
\begin{enumerate}
    \item \textbf{Ontology:} If the pion's 5D structure is shown to be
          bulk-dominant (not brane-dominant), the ontology fails.
    ...
    \item \textbf{Selection rules:} If allowed channels violate the
          ...
          becomes dominant), the selection rules fail.
    ...
    \item \textbf{Universality:} If the chiral filter must be different
          for pions than for leptons, the universal hypothesis fails.
\end{enumerate}
...
\textbf{P} & \textbf{Pion} & \textbf{Brane-dominant (composite)} & \textbf{v0.1} \\
\bottomrule
\end{tabular}
\end{table}
