...
% ============================================================
% Canonical styles for Paper 3 series (epistemic tags, tcolorbox, symbols)
\input{../../../_shared/style/edc_style}
% Reusable TikZ figure styles
...
\begin{tikzpicture}[scale=0.85, transform shape]
    % Bulk region (5D) - using style
    \fill[bulk region] (-6,-3) rectangle (6,3);

    % Brane layer (thick) - using style
    \fill[brane region] (-6,-0.8) rectangle (6,0.8);
    \draw[brane edge] (-6,0.8) -- (6,0.8);
    \draw[brane edge] (-6,-0.8) -- (6,-0.8);

    % Bulk-facing side - using style
    \draw[bulk boundary] (-6,0.8) -- (6,0.8);
    \node[red!70!black, above, yshift=2pt] at (0,0.85) {\footnotesize\textbf{Bulk-Facing ($y = -\delta/2$)}};

    % Observer-facing side (frozen projection) - using style
    \draw[observer boundary] (-6,-0.8) -- (6,-0.8);
    \node[green!50!black, below, yshift=-2pt] at (0,-0.85) {\footnotesize\textbf{Observer-Facing / Frozen Projection ($y = +\delta/2$)}};

    % Labels for regions (repositioned for clarity)
    \node[section label] at (-4.5,2.3) {\textbf{5D Bulk}};
    \node at (4.5,2.3) {\small $\nabla_A T^{AB}_{(5)} = 0$};
    \node at (0,0) {\textbf{Brane-Layer} (thickness $\delta$)};
    \node[section label] at (-4.5,-2.3) {\textbf{3D Observer}};

    % Y-junction in bulk (neutron excited state) - using styles
    \begin{scope}[shift={(-3,1.8)}]
        \draw[flux arm] (0,0) -- (0.5,0.6);
        \draw[flux arm] (0,0) -- (-0.6,0.4);
        \draw[flux arm] (0,0) -- (0.05,-0.65);
        \node[junction point] at (0,0) {};
        \node[blue!60!black, right, xshift=2pt] at (0.5,0.4) {\footnotesize $q_n > 0$};
        \node[red!60!black, below left, font=\footnotesize, xshift=-2pt] at (-0.2,-0.1) {n};
    \end{scope}

    % Energy flow arrow (inflow) - repositioned
    \draw[edc flow, red!60!black] (-1.2,1.6) -- (-1.2,0.95);
    \node[red!60!black, left, align=right, font=\footnotesize] at (-1.3,1.3) {$J^\nu_{\mathrm{bulk}\to\mathrm{brane}} > 0$\\[-2pt](inflow)};

    % Brane-layer mode - using style
    \draw[wave field, purple!70!black] (1.5,-0.3) -- (3.5,-0.3);
    \node[purple!70!black, above, yshift=2pt] at (2.5,0) {\footnotesize $\phi(y,t)$};

    % 3D outputs (electron + antineutrino) - using styles
    \node[particle] (elec) at (1.5,-2) {};
    \node[below, yshift=-2pt] at (elec) {\footnotesize $e^-$};
    \node[neutrino] (nu) at (2.8,-2) {};
    \node[below, yshift=-2pt] at (nu) {\footnotesize $\bar{\nu}_e$};
    \draw[edc arrow, green!50!black] (2.1,-0.9) -- (2.1,-1.7);

    % Dimension labels
    \draw[edc bidir] (5.5,-3) -- (5.5,3);
    \node[right, font=\footnotesize] at (5.6,0) {$\xi$ (5th dim)};

    % Criterion box - using style
    \node[label box, inner sep=4pt] at (3.8,2.4)
...
\begin{tikzpicture}[scale=0.95]
    % Bulk side - using style
    \fill[bulk region] (-3,-1.5) rectangle (-0.5,1.5);
    \node[section label] at (-1.75,1.9) {\textbf{5D Bulk (Plenum)}};
    \node at (-1.75,0.2) {$P > 0$};
    \node at (-1.75,-0.4) {\small Energy fluid};

    % Brane - using style
    \fill[brane region] (-0.5,-1.5) rectangle (0.5,1.5);
    \draw[very thick, gray!60] (-0.5,-1.5) -- (-0.5,1.5);
    \draw[very thick, gray!60] (0.5,-1.5) -- (0.5,1.5);
    \node[section label] at (0,1.9) {\textbf{Brane ($\delta$)}};
    \node[font=\footnotesize] at (0,0) {thick layer};

    % Observer side - using style
    \fill[observer region] (0.5,-1.5) rectangle (3,1.5);
    \node[section label] at (1.75,1.9) {\textbf{3D Observable}};
    \node at (1.75,0) {\small Our physics};

    % Labels (repositioned below for clarity)
    \node[font=\scriptsize, gray!70!black] at (-0.5,-1.75) {bulk-facing};
    \node[font=\scriptsize, gray!70!black] at (0.5,-1.75) {observer-facing};

    % Arrows - using styles
    \draw[edc flow] (-2.5,0.8) -- (-0.7,0.8) node[midway,above,font=\footnotesize] {cause};
    \draw[edc flow] (0.7,0.8) -- (2.5,0.8) node[midway,above,font=\footnotesize] {effect};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.6]
    ...
    % Energy difference arrow (repositioned for clarity)
    \draw[edc bidir] (1.7,0.3) -- (1.7,1.5);
    \node[right, xshift=2pt, font=\small] at (1.7,0.9) {$\Delta E$};

    % Barrier label (repositioned)
...
\begin{tikzpicture}[scale=1.1]
    ...
    % Junction center - using style
    \node[junction point, minimum size=5pt] at (0,0) {};

    % Three springs at 120 degrees (equilibrium) - using style
    \foreach \angle/\col in {90/blue!70!black, 210/red!70!black, 330/green!60!black} {
        \draw[spring, \col] (0,0) -- (\angle:0.75);
        \fill[\col] (\angle:1) circle (2.5pt);
    }

    % Angles (repositioned for clarity)
    \node[above, yshift=3pt, font=\footnotesize] at (0,1.05) {$\theta_1$};
    \node[below left, xshift=-3pt, yshift=-3pt, font=\footnotesize] at (-0.9,-0.6) {$\theta_2$};
    \node[below right, xshift=3pt, yshift=-3pt, font=\footnotesize] at (0.9,-0.6) {$\theta_3$};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=0.85, >=stealth, edc compact]
    % Bulk core - using style
    \node[bulk box, minimum width=3.2cm, minimum height=1.4cm] (bulk) at (0,0) {};
    \node[above, yshift=1pt] at (bulk.north) {\small\textbf{Bulk-Core}};
    \node at (0,0.1) {\small Junction: $|0\rangle$, $|1\rangle$};
    \node[below, font=\scriptsize, yshift=-1pt] at (bulk.south) {$|0\rangle$ = proton, $|1\rangle$ = neutron};

    % Arrow down - using style
    \draw[edc flow, purple!70!black] (0,-0.95) -- (0,-1.85);
    \node[right, xshift=3pt, font=\footnotesize] at (0,-1.4) {pumping};

    % Brane layer - using style
    \node[brane box, minimum width=3.2cm, minimum height=1.4cm] (brane) at (0,-3.2) {};
    \node[above, yshift=1pt] at (brane.north) {\small\textbf{Brane-Layer}};
    \node at (0,-3.1) {\small $\phi(y,t)$ modes};
    \node[below, font=\scriptsize, yshift=-1pt] at (brane.south) {thickness $\delta$};

    % Arrow down - using style
    \draw[edc flow, orange!70!black] (0,-4.55) -- (0,-5.45);
    \node[right, xshift=3pt, font=\footnotesize] at (0,-5.0) {frozen projection};

    % 3D outputs - using style
    \node[output box, minimum width=3.2cm, minimum height=1.1cm] (out) at (0,-6.4) {};
    \node at (0,-6.4) {\small $e^- + \bar{\nu}_e$};
    \node[below, font=\scriptsize, yshift=-1pt] at (out.south) {3D particles};
\end{tikzpicture}
\end{center}
...
% Bulk-core (left) - using styles
\node[bulk box, minimum width=2.0cm] (bulk) {Bulk junction\\$|n\rangle: q > 0$};

% Pumping arrow and label - using styles
\node[brane box, right=of bulk, minimum width=2.0cm] (charge) {Brane store\\$\mathcal{E}_{\mathrm{brane}}(t)$};
\draw[edc arrow, red!70!black] (bulk) -- node[above, font=\tiny] {$\Pi_{\mathrm{pump}}$} (charge);

% Brane-layer modes - using styles
\node[brane box, right=of charge, minimum width=1.8cm] (modes) {Layer modes\\$\{\phi_k\}$};
\draw[edc arrow, green!60!black] (charge) -- node[above, font=\tiny] {$\Pi_{\mathrm{release}}$} (modes);

% Frozen projection - using styles
\node[output box, right=of modes, minimum width=1.5cm] (frozen) {$\mathcal{P}_{\mathrm{frozen}}$};
\draw[edc arrow, blue!60!black] (modes) -- (frozen);

% 3D outputs - using styles
\node[output box, right=of frozen, minimum width=1.8cm] (output) {3D outputs\\$e^-, \bar{\nu}_e$};
\draw[edc arrow, blue!60!black] (frozen) -- (output);

% Phase labels below (using style)
\node[phase label, below=0.2cm of bulk, red!60!black] {5D Cause};
\node[phase label, below=0.2cm of charge, green!60!black] {Absorption};
\node[phase label, below=0.2cm of modes, green!60!black] {Dissipation};
\node[phase label, below=0.2cm of frozen, blue!60!black] {Projection};
\node[phase label, below=0.2cm of output, blue!60!black] {Observation};

% Regime indicator (repositioned for clarity)
\node[above=0.4cm of charge, font=\tiny, align=center] {$\Xi = \Pi_{\mathrm{pump}}/\Pi_{\mathrm{release}}$};
\node[above=0.4cm of modes, font=\tiny, fill=white, inner sep=1pt] {Trigger: $\Xi(t_*) \ll 1$};

% Ledger closure annotation (repositioned)
\draw[dashed, gray!60] ([yshift=-0.9cm]bulk.south west) -- ([yshift=-0.9cm]output.south east);
\node[below=1.0cm of modes, font=\tiny, align=center, fill=white, inner sep=1pt]
    {Ledger: $|\Delta E_{\mathrm{bulk}}| = \Delta E_{\mathrm{brane}} + E_{\mathrm{other}}$};
