...
\begin{itemize}[nosep]
  \item \textbf{Bulk-core:} The degrees of freedom of the Y-junction and its three
        attached flux-tube arms, located in the 5D bulk away from the brane. The
        collective coordinate $q(t)$ parametrizes the junction configuration:
...
Barrier height $V_B$ & $\sim 2.6$ MeV & \tagCal{} & Fitted to $\tau_n$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[nosep]
  \item \textbf{Derive $V_B$ from 5D action} (open): Current status is $V_B
  ...
  \item \textbf{Precise value of $q_n$} \tagI{}: Currently $q_n \approx
