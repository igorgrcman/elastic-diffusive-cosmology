...
% ==============================================================================
\subsection{V(z) Candidates Catalogue (Toy Shapes, Not Derived)}
\label{subsec:bvp_vz_catalogue}

Before deriving $V(z)$ from the 5D action, it is useful to survey candidate
...
\begin{itemize}[nosep]
    ...
    \item Prepare numerical infrastructure for eventual derived $V(z)$
\end{itemize}
...
\end{equation}
where $\zeta = z/\ell$ is dimensionless, $V_0 > 0$ is the well depth, and $a > 0$
sets the core width.
...
\paragraph{5D origin.}
Could arise from $V_{\text{warp}} \sim (A')^2 - A''$ where $A(z)$ is the warp
factor in $ds^2 = e^{2A(z)}\eta_{\mu\nu}dx^\mu dx^\nu + dz^2$.
...
$\phi(z) = \phi_0 \tanh(z/a)$ via Yukawa coupling $m(z) = y\phi(z)$.
...
\paragraph{5D origin.}
Arises when the warp factor $A(z)$ decays exponentially (e.g., linear dilaton
...
\begin{itemize}[nosep]
    \item \textbf{Closure requires:} Deriving $V(z)$ and BCs from the 5D action
...
\begin{itemize}[nosep]
    \item V(z) derivation pipeline: Sec.~\ref{subsubsec:vz_pipeline}
