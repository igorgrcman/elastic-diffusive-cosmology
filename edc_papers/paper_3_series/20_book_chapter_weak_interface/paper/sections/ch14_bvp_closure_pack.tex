...
\subsection{Numerical Pipeline Demonstration}
\label{subsec:bvp_numerical_demo}

This subsection provides a \textbf{minimal numerical demonstration} of the BVP
pipeline. The purpose is to show that the computational infrastructure works,
\emph{not} to claim physical closure.
...
The results in Table~\ref{tab:bvp_toy_demo} can be regenerated by running:
\begin{verbatim}
python3 code/bvp_halfline_toy_demo.py
\end{verbatim}
from the paper root directory. The script uses \texttt{scipy} for sparse
eigenvalue computation and requires no external calibration data.
