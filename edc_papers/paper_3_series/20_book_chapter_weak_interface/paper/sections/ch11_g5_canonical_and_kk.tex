...
\subsection{Canonical $g_5$ Normalization and KK Spectrum}
...
\begin{equation}
    S_{\text{5D}} = -\frac{1}{4g_5^2} \int d^4x \int_0^\ell dz \; F_{MN} F^{MN}
...
\begin{equation}
    A_\mu(x,z) = \sum_{n=0}^\infty A_\mu^{(n)}(x) \, \chi_n(z)
    \label{eq:ch11_kk_decomp}
\end{equation}
where $\{\chi_n(z)\}$ are orthonormal mode functions satisfying:
\begin{equation}
    \int_0^\ell dz \; \chi_m(z) \chi_n(z) = \delta_{mn}
...
\medskip
\textbf{Alternative:} If modes are normalized as $\int dz \, \chi^2 = \ell$, then
...
The KK mode functions $\chi_n(z)$ satisfy the eigenvalue equation:
\begin{equation}
    -\partial_z^2 \chi_n(z) = m_n^2 \chi_n(z)
    \label{eq:ch11_kk_eigenvalue}
\end{equation}
subject to boundary conditions at $z=0$ and $z=\ell$.
