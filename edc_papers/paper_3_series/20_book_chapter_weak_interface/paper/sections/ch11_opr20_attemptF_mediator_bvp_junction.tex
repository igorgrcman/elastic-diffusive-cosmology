...
Consider a scalar or gauge mediator $\phi(x^\mu, z)$ propagating in the thick-brane
background. Separating variables $\phi(x,z) = \varphi(x) f(z)$, the extra-dimensional
profile $f(z)$ satisfies a Schr\"odinger-type equation \tagP{}:
\begin{equation}
    \boxed{
...
\end{equation}
where $m^2$ is the 4D mass-squared eigenvalue and $V(z)$ encodes the brane geometry.
The domain is $z \in [0, \ell]$ with boundary conditions to be specified.

\paragraph{Dimensionless formulation \tagDc{}.}
Define dimensionless coordinate $\tilde{z} = z/\ell \in [0,1]$ and rescaled quantities:
\begin{align}
    \tilde{V}(\tilde{z}) &= \ell^2 V(\ell\tilde{z}), \label{eq:attemptF_Vtilde} \\
    \lambda &= \ell^2 m^2, \quad x = \sqrt{\lambda}. \label{eq:attemptF_lambda}
\end{align}
The eigenvalue equation becomes:
\begin{equation}
    \left[ -\frac{d^2}{d\tilde{z}^2} + \tilde{V}(\tilde{z}) \right] \tilde{f}(\tilde{z}) = \lambda \tilde{f}(\tilde{z}),
...
\begin{equation}
    V(\tilde{z}) = \begin{cases}
        0 & \text{if } |\tilde{z} - 1/2| < w/2 \\
        V_0 & \text{otherwise}
    \end{cases}
...
\begin{equation}
    V(\tilde{z}) = V_0 \left[ 1 - \operatorname{sech}^2\left(\frac{\tilde{z} - 1/2}{w}\right) \right]
    \label{eq:attemptF_V2}
\end{equation}
Standard kink/domain-wall potential. The mode is localized in the well at $\tilde{z} = 1/2$.

\medskip

\textbf{V3: Gaussian core}
\begin{equation}
    V(\tilde{z}) = V_0 \left[ 1 - \exp\left(-\frac{(\tilde{z} - 1/2)^2}{2w^2}\right) \right]
...
At a thin brane located at $z = z_*$, the Israel junction condition relates the
...
\begin{equation}
    \left. \partial_z \phi \right|_{z_*^+} - \left. \partial_z \phi \right|_{z_*^-}
    = \lambda \, \Box_4 \phi \big|_{z_*},
...
\begin{equation}
    2 f'(z_*) = -\lambda p^2 f(z_*).
\end{equation}
At the boundary $z = 0$ (or $z = \ell$), this becomes a Robin condition \tagDc{}:
\begin{equation}
    \boxed{
