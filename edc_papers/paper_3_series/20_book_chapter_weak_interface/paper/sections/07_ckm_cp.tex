...
If generations are well-separated (distance $\Delta z \gg$ profile width $\kappa$),
off-diagonal overlaps are \textbf{exponentially suppressed}: $|V_{ij}| \sim e^{-|z_i - z_j|/2\kappa}$.
...
\begin{description}[style=nextline, leftmargin=1em, font=\normalfont\bfseries]
    ...
    \item[Attempt 2.1: Non-uniform spacing \tagDc{}/\tagCal{}:]
        Two parameters: $\Delta z_{12}/(2\kappa) = 1.49$, $\Delta z_{23}/(2\kappa) = 3.17$.
...
Three Gaussian-like profiles for $(u, c, t)$ at positions $z_1, z_2, z_3$,\\
with corresponding $(d, s, b)$ profiles slightly offset.\\
Shaded overlap regions show: large diagonal overlap, small off-diagonal.\\
Arrows indicate $\Delta z_{12}$ and $\Delta z_{23}$ separations.
}}
...
($\Delta z \gg \kappa$) produce exponential suppression of off-diagonal
...
The uniform-spacing model (Attempt~2) uses a single parameter $\Delta z/(2\kappa)$
...
\begin{align}
    |V_{us}| &= \exp\bigl(-\Delta z_{12}/(2\kappa)\bigr)
    \quad\Rightarrow\quad \Delta z_{12}/(2\kappa) = -\ln|V_{us}| \approx 1.49
    \label{eq:ch7_d12_cal} \\
    |V_{cb}| &= \exp\bigl(-\Delta z_{23}/(2\kappa)\bigr)
    \quad\Rightarrow\quad \Delta z_{23}/(2\kappa) = -\ln|V_{cb}| \approx 3.17
...
\begin{equation}
    |V_{ub}|_{\text{pred}} = \exp\Bigl(-\frac{\Delta z_{12} + \Delta z_{23}}{2\kappa}\Bigr)
...
\begin{align}
    ...
    \Delta z/(2\kappa_q) \approx 1.5 \\
    \sin\theta_{12}^{\text{PMNS}} &\approx 0.55 \quad\Rightarrow\quad
    \Delta z/(2\kappa_\ell) \approx 0.6
\end{align}

If the inter-generation spacing $\Delta z$ is \emph{universal} (same for quarks
and leptons), then:
\begin{equation}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Attempt 2.1: Non-uniform spacing} \tagDc{}/\tagCal{}:
          Two parameters ($\Delta z_{12}$, $\Delta z_{23}$) calibrated from $|V_{us}|$, $|V_{cb}|$.
...
Wolfenstein hierarchy from $\Delta z/\kappa$ & \textcolor{OliveGreen}{GREEN} & \tagDc{} \\
