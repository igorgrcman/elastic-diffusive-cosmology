...
\begin{itemize}[nosep]
    ...
    \item CP violation: Jarlskog $J$ predicted within 6\% from $\mathbb{Z}_3$ phases \tagDc{}
\end{itemize}
\textbf{What is NOT claimed:} Full $(\bar\rho, \bar\eta)$ derivation; CP phase $\delta$
has factor-2 discrepancy (120° predicted vs.\ 65° observed).
\end{tcolorbox}
...
\begin{description}[style=nextline, leftmargin=1em, font=\normalfont\bfseries]
    ...
    \item[Attempt 3: CP phase from $\mathbb{Z}_3$ \tagDc{}:]
        Generations carry $\mathbb{Z}_3$ charges $\to$ CKM phases $\omega^{q_u - q_d}$.
        Jarlskog $J = 2.9 \times 10^{-5}$ predicted (PDG: $3.1 \times 10^{-5}$), 6\% agreement.
        Phase $\delta = 120°$ predicted vs.\ PDG $65°$ (factor 2 discrepancy).
    ...
    \item[Open:]
        Derive $(\bar\rho, \bar\eta)$ from 5D geometry; resolve $\delta$ factor-2 discrepancy.
\end{description}
\end{tcolorbox}
...
\textbf{Attempt 3: Jarlskog $J$ prediction} & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagDc{} \\
\textbf{Attempt 3: CP mechanism ($\mathbb{Z}_3$ phases)} & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagDc{} \\
CP phase $\delta$ (120° vs 65°) & YELLOW & \tagI{} \\
