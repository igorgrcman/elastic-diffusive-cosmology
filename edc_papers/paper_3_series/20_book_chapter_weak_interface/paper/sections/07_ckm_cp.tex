...
at different positions along the extra dimension $z$ (or equivalently, at different
angular positions in the $\mathbb{Z}_3$ lattice). Each generation has a wavefunction
profile $f_i(z)$ that is peaked at its localization center \tagP{}.
...
$V_{ij} \propto \int f_i^{(u)}(z) f_j^{(d)}(z) dz$ \tagDc{}.
...
\begin{enumerate}
    ...
    \item \textbf{Non-exponential profiles:}
          Gaussian profiles $f(z) \propto \exp(-z^2/2\sigma^2)$ have different
...
\begin{itemize}[nosep]
    ...
    \item Localized profile ansatz: exponential $f(z) \propto e^{-|z-z_0|/\kappa}$ \tagP{}
\end{itemize}
