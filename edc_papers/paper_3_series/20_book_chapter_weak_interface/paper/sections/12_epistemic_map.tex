...
OPR-05 & PMNS mixing angles & \textcolor{YellowOrange}{\textbf{YELLOW}} [Dc/I] & $\theta_{23}$ derived; $\theta_{12},\theta_{13}$ structure identified \\
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{YellowOrange}{\textbf{Partial:}} Rank-2 + $\varepsilon$ construction (Attempt~4) achieves all three angles within 3\% [GREEN numerically]; $\theta_{12}^0 = 33.7°$ and $\varepsilon \approx 0.15$ rad identified but not derived \tagI{}
    \item \textcolor{BrickRed}{\textbf{Open:}} Geometric origins of $\theta_{12}^0$, $\varepsilon$; CP phase; Dirac vs Majorana
\end{itemize}
...
\textbf{P1-B: PMNS Mixing (OPR-05, OPR-08) --- STRUCTURE IDENTIFIED}
\begin{itemize}[nosep]
    \item \emph{Status:} Attempts~2--4 completed; $\theta_{23}$ derived from $\mathbb{Z}_6$ geometry \tagDc{}; rank-2 + $\varepsilon$ structure identified \tagI{}
    ...
    \item \emph{Result (Attempt~4):} $U = R_{23}(\theta_{23}^0) \cdot R_{13}(\varepsilon) \cdot R_{12}(\theta_{12}^0)$ with $\theta_{12}^0 = 33.7°$, $\varepsilon = 0.15$ rad achieves \textbf{GREEN} for all three angles ($<$3\% error)
    \item \emph{Key insight:} Asymmetric PMNS pattern requires rank-2 baseline with reactor perturbation; $\theta_{12}^0$ and $\varepsilon$ values identified but not derived
    \item \emph{Next:} Geometric origin of $\theta_{12}^0 \approx 33.7°$ and $\varepsilon \approx 0.15$ rad
    \item \emph{Payoff:} $\theta_{23}$ derived; PMNS structure identified; two parameters need derivation
\end{itemize}
