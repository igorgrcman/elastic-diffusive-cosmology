...
\textit{Latest:} OPR-20 factor-8 forensic sweep (Attempt A3): 19 candidates surveyed; best [Dc] is $2\pi$ (19\% off); attack surface narrowed to BC/junction/$R_\xi$.
}}
\end{center}
...
\begin{itemize}[nosep]
    ...
    \item \emph{Open:} $g_5$ value (OPR-19: $4\pi$ coefficient); $\ell$ from membrane (OPR-20: $f_{\rm geom}$); $I_4$ from BVP profiles (OPR-21)
    \item \emph{Suppression attempt (OPR-20):} Candidate A ($f = R_\xi/r_e \sim 10^{-3}$) is SM-free [P], but predicts $m_\phi \approx 620$ GeV (8$\times$ overshoot).
    \item \emph{Factor-8 forensic (Attempt A3):} 19 mechanisms surveyed; best [Dc] is $2\pi$ (19\% off); C=8 best fit but [P]. Attack surface: BC eigenvalue, junction normalization, or $R_\xi$ value.
