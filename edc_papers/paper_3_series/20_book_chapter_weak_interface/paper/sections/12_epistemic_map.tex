...
Neutrino mixing ($\theta_{12}$, $\theta_{13}$) & Non-overlap physics; $\theta_{23}$ already derived \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{YellowOrange}{\textbf{Partial:}} Mass suppression mechanism (edge-mode overlap) \tagP{}; $\theta_{13}$ improved but not reproduced
    \item \textcolor{BrickRed}{\textbf{Open:}} $\theta_{12}$ (requires non-overlap physics); CP phase; Dirac vs Majorana
\end{itemize}
...
\begin{itemize}[nosep]
    \item \emph{Status:} Attempt~2 completed; $\mathbb{Z}_6$ submixing derives $\theta_{23} = 0.564$ vs 0.546 observed \tagDc{}
    \item \emph{Result:} $\theta_{23}$ GREEN (3\%); $\theta_{12}$, $\theta_{13}$ RED
    \item \emph{Key insight:} Overlap model produces democratic or hierarchical mixing, not the asymmetric PMNS pattern
    \item \emph{Next:} Non-abelian flavor symmetry extension or charged lepton corrections
    \item \emph{Payoff:} One of three PMNS angles derived; framework for remaining
\end{itemize}
