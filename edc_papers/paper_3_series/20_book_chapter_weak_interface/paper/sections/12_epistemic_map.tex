...
OPR-17 & SU(2)$_L$ gauge embedding & RED (open) & 5D gauge theory \\
OPR-22 & $G_F$ first-principles & RED-C (open) & Closure of BVP chain \\
\bottomrule
\end{tabular}
\end{table}
...
\end{itemize}
\noindent\emph{PMNS closure advanced:} $\theta_{23}$ is now closed by $\mathbb{Z}_6$ geometry (OPR-05a, GREEN), while $\theta_{13}$ is partially closed via a no-fit reactor scale $\varepsilon = \lambda/\sqrt{2}$ (OPR-05b, YELLOW, 15\% from PDG) and $\theta_{12}$ remains identified but not derived (OPR-05c, YELLOW).
...
\textbf{P2: $G_F$ Chain (OPR-19, OPR-20, OPR-22)}
\begin{itemize}[nosep]
    \item \emph{Status:} Structural pathway established; numerical closure via EW relations
    \item \emph{Next:} KK reduction for $g_5$, $m_\phi$; complete BVP chain
    \item \emph{Payoff:} First-principles weak coupling derivation
\end{itemize}

\textbf{P3: Gauge Structure (OPR-17)}
\begin{itemize}[nosep]
    \item \emph{Status:} V$-$A chirality derived; full SU(2)$_L$ open
    \item \emph{Next:} 5D gauge theory with mass generation mechanism
    \item \emph{Payoff:} Complete electroweak embedding
\end{itemize}
