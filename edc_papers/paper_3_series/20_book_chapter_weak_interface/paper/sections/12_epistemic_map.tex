...
\textit{Latest:} OPR-19 coefficient provenance (Attempt 2): $4\pi$ best [Dc], $g^2 = 0.373$ (6\% from SM); OPR-20 factor-8 forensic sweep (Attempt 3): 19 mechanisms, best [Dc] is $2\pi$ (19\% off).
}}
\end{center}
...
\begin{itemize}[nosep]
    ...
    \item \emph{Coefficient provenance (OPR-19):} 15 candidates surveyed; best [Dc] is $4\pi$ (solid angle) giving $g^2 = 0.373$ (6\% below SM). No unique derivation selects $4\pi$---status RED-C [OPEN].
    \item \emph{Suppression attempt (OPR-20):} Candidate A ($f = R_\xi/r_e \sim 10^{-3}$) is SM-free [P], but predicts $m_\phi \approx 620$ GeV (8$\times$ overshoot). Factor-8 forensic sweep (Attempt 3): 19 mechanisms surveyed; best [Dc] is $2\pi$ (19\% off); $C=8$ numeric match (3.7\%) but [P]. Status RED-C [OPEN].
