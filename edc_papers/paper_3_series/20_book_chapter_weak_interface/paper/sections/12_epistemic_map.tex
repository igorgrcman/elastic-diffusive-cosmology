...
\textit{Latest:} OPR-19 4$\pi$ derivation: Dual-route via Gauss+isotropy $\to$ $C=4\pi$ \textbf{YELLOW [Dc]+[P]}; $g^2 = 0.373$ (6\% from SM). OPR-20 factor-8 forensic: BC route \textbf{CLOSED [Dc]} (standard BCs fail; Robin needs tuning); geometric ($2\pi$, $8$) routes remain [OPEN]. Status: RED-C [Dc]+[OPEN].
}}
\end{center}
...
\begin{itemize}[nosep]
    ...
    \item \emph{Suppression attempt (OPR-20):} Candidate A ($f = R_\xi/r_e \sim 10^{-3}$) is SM-free [P], but predicts $m_\phi \approx 620$ GeV (8$\times$ overshoot). \textbf{Factor-8 forensic (BC sweep):} Standard BCs fail (max factor-4); Robin BCs ($a\ell \sim 0.1$) work but need derivation; geometric prefactors ($2\pi$: 24\% off, $8$: 3\% match) remain [P]. \textbf{BC route CLOSED [Dc]}. Status RED-C [Dc]+[OPEN].
