...
\textit{$G_F$:} sanity skeleton complete + \textbf{chain tightened} ($g_5$ canonical [Dc], $m_\phi = x_1/\ell$ [Dc]); first-principles closure remains \textbf{RED-C} pending BVP numerics.
\textbf{Hard blockers (P1):} OPR-02/21 thick-brane BVP; OPR-22 combine without SM circularity. \textit{(OPR-19/20 now have derivation spines.)}
}}
\end{center}
...
OPR-11 & CKM $(\bar\rho, \bar\eta)$ & RED (open) & Structure correct; $\mathbb{Z}_2$ parity needed \\
...
OPR-22 & $G_F$ first-principles & RED-C (open) & Chain tightened ($g_5$, $m_\phi$ [Dc]); BVP numerics open \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{BrickRed}{\textbf{Open:}} $\mathbb{Z}_2$ parity assignment; $(\bar\rho, \bar\eta)$ derivation
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{BrickRed}{\textbf{Open:}} $g_5$ value; $\ell$ from membrane; exact BVP solutions
\end{itemize}
...
\textbf{P2: $G_F$ Chain (OPR-19--22) --- SANITY SKELETON + CHAIN TIGHTENED}
\begin{itemize}[nosep]
    \item \emph{Status:} Chain map + dimensional checks + attack-surface analysis done (\S11.8); \textbf{chain tightened} (\S11.9)
    \item \emph{Closed:} Dimensional consistency; structural form $G \sim g^2/m^2$; circularity explicitly noted
    \item \emph{Closed (new):} $g_4 = g_5$ canonical normalization [Dc]; $m_\phi = x_1/\ell$ KK form [Dc]
    \item \emph{Open:} $g_5$ value; $\ell$ from membrane physics; BVP profiles (OPR-21)
    \item \emph{Payoff:} First-principles $G_F$ without SM circularity (OPR-22)
\end{itemize}
\noindent\emph{G$_F$ chain:} OPR-19/20 now have derivation spines [Dc]; only BVP numerics + underlying $g_5$ remain. True prediction is $\sin^2\theta_W = 1/4$; numerical $G_F$ uses SM relations (consistency, not derivation). Attack surface mapped; no hidden tuning.
