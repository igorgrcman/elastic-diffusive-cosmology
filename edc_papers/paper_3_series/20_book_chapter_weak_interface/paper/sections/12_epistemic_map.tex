...
\midrule
OPR-02 & KK tower truncation (N$_{\text{gen}}$) & RED [P] & Solve thick-brane BVP \\
...
OPR-05c & PMNS $\theta_{12}$ & \textcolor{YellowOrange}{\textbf{YELLOW}} [I] & $35°$ discrete or $33.7°$ identified \\
OPR-11 & CKM $(\bar\rho, \bar\eta)$ & RED (open) & Z$_6$ refinement \\
OPR-12 & CP phase $\delta$ (120° vs 65°) & YELLOW [I] & Attempt 4: Z$_6$ selection \\
OPR-17 & SU(2)$_L$ gauge embedding & \textcolor{YellowOrange}{\textbf{YELLOW}} [P] & Where/how fixed; origin+masses OPEN \\
OPR-22 & $G_F$ first-principles & RED-C (open) & Sanity skeleton done; first-principles open \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{YellowOrange}{\textbf{Partial:}} Jarlskog $J = 2.9 \times 10^{-5}$ without calibration (6\% of PDG) \tagDc{}
    \item \textcolor{BrickRed}{\textbf{Open:}} Phase $\delta$ (120° vs 65°); $(\bar\rho, \bar\eta)$ derivation
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{YellowOrange}{\textbf{Partial:}} Rank-2 + $\varepsilon$ construction (Attempt~4) achieves all three angles; $\varepsilon = \lambda/\sqrt{2}$ (Attempt~4.1) predicts reactor scale from Wolfenstein $\lambda$ [BL$\to$Dc] with 15\% accuracy---no PDG-smuggling
    \item \textcolor{BrickRed}{\textbf{Open:}} Geometric origin of $\theta_{12}^0$ ($35°$ discrete or $33.7°$ identified); CP phase; Dirac vs Majorana
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{BrickRed}{\textbf{Open:}} $g_5$ normalization; $m_\phi$ KK spectrum; exact BVP solutions
\end{itemize}
...
\begin{itemize}[nosep]
    \item \emph{Status:} Attempt~4 achieved $\delta = 60°$ (5° from PDG) via $\mathbb{Z}_2$ sign selection
    \item \emph{Remaining:} Derive specific $\mathbb{Z}_2$ parity assignment; close $(\bar\rho, \bar\eta)$
    \item \emph{Payoff:} Completes CKM sector; provides template for PMNS CP phase
\end{itemize}

\textbf{P1-B: PMNS Mixing (OPR-05a/b/c, OPR-08) --- $\varepsilon$ MECHANISM CLOSED}
\begin{itemize}[nosep]
    \item \emph{Status:} Attempts~2--4.1 completed; $\theta_{23}$ derived from $\mathbb{Z}_6$ geometry \tagDc{}; $\varepsilon = \lambda/\sqrt{2}$ predicted from Wolfenstein scale [BL$\to$Dc]
    \item \emph{Result (Attempt~2):} $\theta_{23}$ GREEN (3\%); $\theta_{12}$, $\theta_{13}$ RED
    \item \emph{Result (Attempt~3):} Discrete $\mathbb{Z}_6$ phases insufficient
    \item \emph{Result (Attempt~4):} Rank-2 structure $U = R_{23} \cdot R_{13}(\varepsilon) \cdot R_{12}$ achieves GREEN
    \item \emph{Result (Attempt~4.1):} $\varepsilon = \lambda/\sqrt{2} = 0.159$ rad predicts $\sin^2\theta_{13} = 0.025$ (15\% from PDG) \emph{without fitting}; with discrete $\theta_{12}^0 = 35°$, all angles GREEN
    \item \emph{Key insight:} Reactor perturbation $\varepsilon$ is tied to CKM Wolfenstein scale---no new [I] dependency
    \item \emph{Next:} Geometric origin of $\theta_{12}^0$ (best discrete candidate: $35°$)
    \item \emph{Payoff:} $\theta_{23}$ derived; $\varepsilon$ mechanism closed; only $\theta_{12}^0$ remains
\end{itemize}
\noindent\fbox{\parbox{0.96\textwidth}{\small
\textbf{PMNS closure:} OPR-05a GREEN ($\theta_{23}$ from $\mathbb{Z}_6$), OPR-05b YELLOW ($\varepsilon = \lambda/\sqrt{2} \to \theta_{13}$, 15\% off, no fit), OPR-05c YELLOW ($\theta_{12}$ identified, not derived); C2 $\varepsilon = \lambda\kappa$ is \textbf{CLOSED/RED}.}}

\textbf{P1-C: Thick-Brane BVP (OPR-02, OPR-09, OPR-14, OPR-21)}
\begin{itemize}[nosep]
    \item \emph{Status:} Profile ansatz postulated; no explicit solution
    \item \emph{Next:} Solve fermion BVP with explicit boundary conditions
    \item \emph{Payoff:} Unlocks generation counting, mode profiles, overlap integrals
\end{itemize}

\textbf{P2: $G_F$ Chain (OPR-19--22) --- SANITY SKELETON COMPLETE}
\begin{itemize}[nosep]
    \item \emph{Status:} Chain map + dimensional checks + attack-surface analysis done (\S11.8)
    \item \emph{Closed:} Dimensional consistency; structural form $G \sim g^2/m^2$; circularity explicitly noted
    \item \emph{Open:} $g_5$ from action (OPR-19); $m_\phi$ from KK (OPR-20); BVP profiles (OPR-21)
    \item \emph{Payoff:} First-principles $G_F$ without SM circularity (OPR-22)
\end{itemize}
\noindent\emph{G$_F$ skeleton:} True prediction is $\sin^2\theta_W = 1/4$; numerical $G_F$ uses SM relations (consistency, not derivation). Attack surface mapped; no hidden tuning.
