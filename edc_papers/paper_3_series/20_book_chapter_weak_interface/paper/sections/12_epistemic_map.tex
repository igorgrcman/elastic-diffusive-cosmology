...
Neutrino mixing ($\theta_{12}$, $\theta_{13}$) & Discrete $\mathbb{Z}_6$ phases insufficient (Attempt~3); need non-abelian symmetry \\
\bottomrule
\end{tabular}
\end{center}
...
OPR-05 & PMNS mixing angles & \textcolor{YellowOrange}{\textbf{YELLOW}} [Dc] & $\theta_{23}$ derived; $\theta_{12},\theta_{13}$ open \\
...
\begin{itemize}[nosep]
    ...
    \item \textcolor{YellowOrange}{\textbf{Partial:}} Mass suppression mechanism (edge-mode overlap) \tagP{}; discrete $\mathbb{Z}_6$ phases tested and found insufficient (Attempt~3)
    \item \textcolor{BrickRed}{\textbf{Open:}} $\theta_{12}$, $\theta_{13}$ (require physics beyond exponential localization + discrete phases); CP phase; Dirac vs Majorana
\end{itemize}

\paragraph{$G_F$ and Weak Coupling (Chapter~\ref{sec:gf_summary}).}
\begin{itemize}[nosep]
...
\textbf{P1-B: PMNS Mixing (OPR-05, OPR-08) --- PARTIAL SUCCESS}
\begin{itemize}[nosep]
    \item \emph{Status:} Attempts~2+3 completed; $\mathbb{Z}_6$ submixing derives $\theta_{23} = 0.564$ vs 0.546 observed \tagDc{}
    \item \emph{Result (Attempt~2):} $\theta_{23}$ GREEN (3\%); $\theta_{12}$, $\theta_{13}$ RED
    \item \emph{Result (Attempt~3):} Discrete $\mathbb{Z}_6$ phases are either gauge artifacts (removable by rephasing) or make fit worse; cannot fix $\theta_{12}$, $\theta_{13}$
    \item \emph{Key insight:} Overlap model produces democratic or hierarchical mixing, not the asymmetric PMNS pattern; discrete phases insufficient
    \item \emph{Next:} Non-abelian flavor symmetry (A$_4$, S$_4$), Higgs anisotropy, or charged lepton corrections
    \item \emph{Payoff:} One of three PMNS angles derived; clear identification of required physics
\end{itemize}
