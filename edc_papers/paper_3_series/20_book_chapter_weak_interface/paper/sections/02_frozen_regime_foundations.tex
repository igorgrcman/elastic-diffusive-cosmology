...
\end{equation}
where $\Gamma$ is the transition rate. In the semiclassical large-tension regime,
transitions are exponentially suppressed as
\begin{equation}
...
sectors labeled by an invariant $n$ (winding data, homotopy class, etc.). If $n$ is
...
The key mathematical result is the \emph{isoperimetric theorem}, which explains why
stable particles must be spherical.
