...
\begin{equation}
    \frac{df}{d\xi} + \alpha_{\text{phys}} \cdot f = 0,
...
\begin{equation}
    ...
    \Big|_{\xi = 0},
\end{equation}
where $\partial_n = n^M \partial_M$ is the normal derivative ($n^\xi = 1$ for
$\xi$-pointing normal).

\paragraph{Boundary equation.}
The boundary term vanishes for arbitrary $\delta\phi$ only if:
\begin{equation}
    \partial_\xi \phi + \tilde{\lambda} p^2 \phi = 0 \quad \text{at } \xi = 0,
\end{equation}
where we used $\Box_4 \phi = -p^2 \phi$ for a 4D mode.

For the $Z_2$ orbifold (identifying $z \to -z$), the one-sided derivative
becomes $2 \partial_\xi \phi$ after matching, giving:
\begin{equation}
    \partial_\xi f + \frac{\tilde{\lambda} p^2}{2} f = 0
