...
decomposes under $z \to -z$:
\begin{align}
    A_\mu(x, -z) &= +A_\mu(x, z) \quad \text{(even parity)} \\
    A_5(x, -z) &= -A_5(x, z) \quad \text{(odd parity)}
\end{align}
This parity assignment preserves 4D Lorentz invariance and gauge symmetry
at the fixed points.

\paragraph{Resulting KK profiles.}
On the fundamental domain $z \in [0, \ell]$:
\begin{align}
    A_\mu^{(n)}(z) &\propto \cos\left(\frac{n\pi z}{\ell}\right)
        \quad \text{(Neumann: } f'|_{\text{bdy}} = 0\text{)} \\
    A_5^{(n)}(z) &\propto \sin\left(\frac{n\pi z}{\ell}\right)
...
localized at the observer boundary ($z = 0$) with profile $f_L(z)$ peaked
at $z \approx 0$. The effective 4D coupling of a mediator field $\phi(z)$ to
these fermions is proportional to the \emph{overlap integral}:
\begin{equation}
    g_{\text{eff}} \propto \int_0^\ell |f_L(z)|^2 \, |\phi(z)|^2 \, dz
    \label{eq:attemptH1_overlap}
\end{equation}

\textbf{Key observation:} The mediator profile $\phi(z)$ at $z \approx 0$
...
\begin{itemize}[nosep]
    \item Mediator peaked at boundary ($z=0$): $\mathcal{O}(1)$ overlap with LH fermions
...
\paragraph{(i) KK zero-mode of $A_\mu$ (even, Neumann, $n=0$).}
\begin{itemize}[nosep]
    ...
    \item Profile: $A_\mu^{(0)}(z) = \text{const}$ — flat, nonzero everywhere
...
\paragraph{(ii) First KK mode of $A_\mu$ (even, Neumann, $n=1$).}
\begin{itemize}[nosep]
    \item Parity: even (+) \tagBL{}
    \item BC: Neumann ($f' = 0$ at fixed points)
    \item Eigenvalue: $x_1 = \pi$ (first excited)
    \item Mass: $m_\phi = \pi/\ell \approx 70$ GeV
    \item Profile: $A_\mu^{(1)}(z) \propto \cos(\pi z/\ell)$ — \textbf{peaked at boundaries}
    \item Coupling: \textbf{$\mathcal{O}(1)$} overlap (cosine maximum at $z=0$)
\end{itemize}
...
\paragraph{(iii) $A_5$ scalar component (odd, Dirichlet).}
\begin{itemize}[nosep]
    \item Parity: odd ($-$) \tagBL{}
    \item BC: Dirichlet ($f = 0$ at fixed points)
    \item Eigenvalue: $x_1 = \pi$ (ground state for odd field)
    \item Mass: $m_\phi = \pi/\ell \approx 70$ GeV
    \item Profile: $A_5^{(1)}(z) \propto \sin(\pi z/\ell)$ — \textbf{ZERO at boundaries}
...
The $A_5$ mode has profile $\sin(\pi z/\ell)$ which satisfies:
\[
    A_5(0) = A_5(\ell) = 0 \quad \text{(Dirichlet BC)}
\]
Since left-handed fermions are localized at $z \approx 0$ (Ch.~9), the overlap
integral~\eqref{eq:attemptH1_overlap} is suppressed:
\[
    g_{\text{eff}}^{(A_5)} \propto \int |f_L(z)|^2 \sin^2(\pi z/\ell)\, dz
...
\paragraph{Consistency with OPR-17 (SU(2)$_L$ embedding).}
The brane-localized SU(2)$_L$ embedding (\S\ref{sec:ch9_su2l_embedding}) assumes
