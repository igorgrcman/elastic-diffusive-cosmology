...
\begin{itemize}[nosep]
    ...
    \item Geometric domain: half-line $z \in [0, \infty)$ \tagP{}
...
the mass profile $m(z)$ as long as $m(z) > 0$ for $z > 0$.
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item \textbf{5D cause:} Plenum inflow creates directional mass gradient $m(z) > 0$ for $z > 0$.
...
The 5D universe has a depth coordinate $z$; observers live on a brane at $z = 0$.
A sign-changing mass term---induced by Plenum inflow---acts as a \textbf{chirality
filter}: the left-handed zero-mode wavefunction $f_L(z)$ clings to the boundary
(exponentially localized at $z = 0$), while the right-handed wavefunction $f_R(z)$
...
\textbf{Throughout this chapter we work on the half-line $z \in [0, \infty)$} with
...
deferred to Chapter~12 (BVP Work Package).
...
domain $z \in [0, \infty)$ captures the physics close to the observer boundary
...
BVP work package, Chapter~12). The zero-mode analysis captures the leading
...
pieces are distributed in $z$. For a massless 4D mode with momentum $p_\mu$, write:
\begin{equation}
...
$\int |f(z)|^2 \, dz < \infty$. Whether a profile is normalizable depends on
both the mass function $m(z)$ and the domain.

\paragraph{Half-line domain ($z \in [0, \infty)$).}
On the half-line (our working assumption), normalizability requires:
\begin{itemize}[nosep]
    \item $f_L$: Normalizable if $\int_0^\infty m(z')\,dz' \to +\infty$ as $z \to \infty$
          (the integral diverges, so $f_L \to 0$ fast enough).
    \item $f_R$: \textbf{Not normalizable} because the exponential grows without bound.
\end{itemize}
With $m(z) > 0$ for all $z > 0$, the left-handed mode is the \emph{only} normalizable
zero mode.

\paragraph{Finite interval ($z \in [0, \ell]$).}
On a finite interval, both profiles are formally normalizable (integrals are finite).
However, the boundary conditions at $z = \ell$ determine which modes survive:
\begin{itemize}[nosep]
...
\end{itemize}
The finite-interval case is addressed in the BVP work package (Chapter~12).
...
\begin{equation}
    R_{\text{LR}} = \exp\left(-2\int_0^{z_*} m(z')\,dz'\right)
    \label{eq:va:RLR_exact}
\end{equation}
where $z_*$ is a characteristic depth (e.g., where the RH mode peaks, or the
domain cutoff).
...
\begin{equation}
    \mu \equiv \int_0^{z_*} \frac{m(z')}{\Lambda}\,dz'
    \label{eq:va:mu_def}
\end{equation}
...
\paragraph{The suppression inequality.}
Requiring $R_{\text{LR}} < 10^{-3}$ gives:
\begin{equation}
    \boxed{
    e^{-2\mu} < 10^{-3}
    \quad\Longrightarrow\quad
    \mu > \frac{1}{2}\ln(10^3) \approx 3.45
    }
    \label{eq:va:mu_bound}
\end{equation}

This is the \textbf{closure target}: the barrier parameter must satisfy $\mu \gtrsim 3.5$.

\paragraph{Interpretation.}
\begin{itemize}[nosep]
    \item $\mu$ measures the ``cumulative mass barrier'' the RH mode must cross
          to reach the brane.
    \item $\mu \sim 3.5$ is a modest requirement---roughly 3--4 $e$-foldings in
          the wavefunction before it becomes negligible.
    \item For a linear mass profile $m(z) = m_0 z/\lambda$, this requires
          $m_0 z_*/\lambda \gtrsim 3.5$, i.e., the characteristic depth must be
          several times the localization scale.
\end{itemize}
...
\begin{enumerate}[nosep]
    \item The mass profile $m(z)$, which must be derived from Plenum-fermion
...
\begin{itemize}[nosep]
    \item Express $\mu$ in terms of $(\sigma, r_e)$ from Part~I
    \item Show $\mu \gtrsim 3.5$ follows from membrane physics, not from tuning
...
\begin{itemize}[nosep]
    ...
    \item V(z) candidates catalogue: Ch.~14, Sec.~\ref{subsec:bvp_vz_catalogue}
\end{itemize}
...
\begin{itemize}[nosep]
    \item Plenum inflow creates mass gradient $m(z) > 0$
    \item Left-handed modes localize at $z=0$; right-handed displaced to bulk
\end{itemize}
...
is \textbf{localized at the observer boundary} $z = 0$.
...
proportional to how much of the fermion's wavefunction is present at $z = 0$.

\textbf{Alternative:} If the $W$ propagated through the bulk, the overlap integral
would sample all $z$ values, and the chirality filter would be weakened. The
...
Half-line domain $z \in [0, \infty)$ & Geometric choice & \tagP{} \\
...
$m(z) > 0$ for $z > 0$ & From inflow direction (5D cause) & \tagDc{} \\
