...
\subsection{Attempt H: Thick-Brane Microphysics and the δ = R$_\xi$ Gate}
...
% ------------------------------------------------------------------------------
\subsubsection{H.1: Definition of the Boundary Layer Thickness δ}
\label{sec:attemptH_delta_def}

The Robin boundary condition $f'(z_0) + \alpha f(z_0) = 0$ emerges from the
variational principle when the brane has \emph{finite thickness}. Consider a
5D bulk scalar field $\phi(x^\mu, z)$ with action:
\begin{equation}
    S = \int d^4x \int_0^\ell dz \left[
        \frac{1}{2}(\partial_z \phi)^2 + \frac{1}{2}m_5^2 \phi^2
    \right]
    + S_{\text{boundary}}
...
\begin{equation}
    S_{\text{bdy}} = \int d^4x \left[
        -\frac{\tau}{2} \phi^2(0) + \frac{\lambda}{2} \phi(0) \partial_z \phi(0)
    \right]
...
% ------------------------------------------------------------------------------
\subsubsection{H.3: Physical Argument for δ = R$_\xi$}
...
\begin{itemize}
    \item \textbf{δ (boundary layer):} Thickness over which the Robin BC
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!60!black,
    title=\textbf{Identification: δ = R$_\xi$ [Def]}]
...
% ------------------------------------------------------------------------------
\subsubsection{H.5: Candidate Alternatives to δ = R$_\xi$}
\label{sec:attemptH_alternatives}

\begin{table}[ht]
\centering
\caption{Alternative δ candidates and their implications}
