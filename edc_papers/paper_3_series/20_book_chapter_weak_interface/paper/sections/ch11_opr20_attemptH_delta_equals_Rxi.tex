...
Let $\phi(z)$ satisfy the thick-brane field equation with diffusive dynamics
...
\begin{enumerate}[nosep]
    \item \textbf{Inner expansion:} Rescale $z = \delta \zeta$ and solve in the
          boundary layer where $\zeta = O(1)$.
    \item \textbf{Outer expansion:} Solve in the bulk where $z = O(\ell)$.
    \item \textbf{Matching condition:} Require the inner and outer solutions to
          agree in the overlap region $\delta \ll z \ll \ell$. This fixes $\delta$
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Generation counting:} $N_{\text{bound}}$ is determined by $V(z)$
