...
integral between up-type profile $f^u_i(z)$ and down-type profile $f^d_j(z)$:
\begin{equation}
    V_{ij} \propto \int_0^\ell f^u_i(z) \, f^d_j(z) \, w(z) \, dz
    \label{eq:ch7_ckm_overlap_z2}
\end{equation}

\paragraph{Brane-reflection operator.}
Define the reflection $\mathcal{R}: z \mapsto \ell - z$ that exchanges the
observer boundary ($z = 0$) with the bulk boundary ($z = \ell$). Under this
...
\paragraph{Candidate 1: $V_{cb}$ odd parity.}
...
\paragraph{Candidate 2: $V_{ub}$ odd parity.}
