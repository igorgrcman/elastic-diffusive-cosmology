...
\textbf{Step A2:} Apply boundary-layer analysis near the brane ($z \to 0$).
...
$\tau$ are defined. However, these are 4D quantities---the transverse ($z$)
...
\begin{itemize}[nosep]
    \item Inner expansion near $z = 0$ (brane region)
    \item Outer expansion for $z \gg \delta$ (bulk region)
    \item Matching condition at $z \sim \delta$
\end{itemize}
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!70!black,
    title=\textbf{Route A Verdict: BLOCKED}]
...
\begin{itemize}[nosep]
    \item Transverse ($z$-direction) diffusion equation \tagOPEN{}
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!60!black,
    title=\textbf{Route B Verdict: PARTIAL}]
...
\begin{tcolorbox}[colback=orange!5!white, colframe=orange!60!black,
    title=\textbf{Failure Modes: How $\delta = R_\xi$ Could Be Wrong}]
...
\begin{tcolorbox}[colback=yellow!10!white, colframe=yellow!50!black,
    title=\textbf{Figure H2-1: What $\delta$ Is (Placeholder)}]

\textit{[FIGURE PLACEHOLDER]} \tagP{}

\textbf{Content:} Schematic showing the boundary-layer structure:
\begin{itemize}[nosep]
    \item $z$-axis (transverse dimension) with brane at $z = 0$
    \item Bulk region ($z > \delta$): field in bulk-dominated regime
    \item Boundary layer ($0 < z < \delta$): field transitions to brane mode
    \item Brane surface ($z = 0$): Robin BC applies
    \item Annotation showing $\delta = R_\xi$ identification
\end{itemize}

\textbf{Purpose:} Clarify the geometric meaning of $\delta$ for the reader.
\end{tcolorbox}

\begin{tcolorbox}[colback=yellow!10!white, colframe=yellow!50!black,
    title=\textbf{Figure H2-2: Two-Route Convergence Map (Placeholder)}]

\textit{[FIGURE PLACEHOLDER]} \tagP{}

\textbf{Content:} Flowchart showing:
\begin{itemize}[nosep]
    \item \textbf{Route A:} Diffusion PDE $\to$ [BLOCKED] $\to$ BL theorem $\to$ $\delta_{\text{BL}}$
    \item \textbf{Route B:} Junction $\to$ Robin BC $\to$ $\alpha \sim \ell/\delta$
          $\to$ [P] $\delta = R_\xi$
    \item \textbf{Convergence:} Both routes $\to$ [CANNOT TEST]
    \item Color-coding: Green = [Dc], Yellow = [P], Red = [OPEN]
\end{itemize}

\textbf{Purpose:} Visual summary of the audit findings for reviewers.
\end{tcolorbox}
