...
$\mathbb{Z}_3$ \tagI{}, and two computed PMNS baselines: the DFT baseline (Attempt~1)
is falsified by $\theta_{13}$, while the $\mathbb{Z}_6$ overlap model (Attempt~2)
\emph{derives} the atmospheric mixing angle $\theta_{23} \approx 45°$ from pure
geometry \tagDc{}. Solar ($\theta_{12}$) and reactor ($\theta_{13}$) angles
require additional physics beyond the overlap model. Dirac/Majorana nature
remains open.
