...
$\theta_{12}, \theta_{13}$ from discrete phases & RED & \tagDc{} (OPR-05) \\
...
$\mathbb{Z}_3$ \tagI{}, and three computed PMNS baselines: the DFT baseline (Attempt~1)
is falsified by $\theta_{13}$; the $\mathbb{Z}_6$ overlap model (Attempt~2)
\emph{derives} the atmospheric mixing angle $\theta_{23} \approx 45°$ from pure
geometry \tagDc{}; and Attempt~3 shows that discrete Z$_6$ phases alone cannot
correct the remaining angles. Solar ($\theta_{12}$) and reactor ($\theta_{13}$)
require physics beyond exponential localization with discrete phases (non-abelian
flavor symmetry, Higgs profile anisotropy, or charged lepton corrections).
Dirac/Majorana nature remains open.
