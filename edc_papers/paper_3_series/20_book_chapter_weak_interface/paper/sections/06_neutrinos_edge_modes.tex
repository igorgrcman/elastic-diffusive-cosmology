...
This membrane has finite thickness $\delta$ along the fifth dimension $z$. At $z = 0$,
...
its wavefunction $\psi_\nu(z)$ peaks sharply at the interface ($z \approx 0$) and
decays exponentially into the brane interior \tagP{}.

\textbf{Step 3: The Higgs lives in the interior, not at the edge.}
The mass-generating mechanism (Higgs field profile $h(z)$) is localized in the brane
interior ($z > 0$), not at the interface. Electrons and quarks, which live in the
interior, have large overlap with $h(z)$ and acquire ``normal'' masses. Neutrinos,
...
The effective 4D mass comes from the overlap integral $m \sim \int |\psi(z)|^2 h(z) dz$.
...
\paragraph{The setup.}
Imagine a 1D potential $V(z)$ with the following structure:
\begin{itemize}[nosep]
    \item For $z < 0$ (bulk): $V(z) = V_{\text{bulk}} > 0$ (large positive value)
    \item At $z = 0$ (interface): sharp boundary where potential changes
    \item For $z > 0$ (brane interior): $V(z) = 0$ (attractive region)
\end{itemize}

A particle in this potential has two types of states:
\begin{enumerate}[nosep]
    \item \textbf{Interior modes:} Wavefunction peaked at $z > 0$, freely propagating
          in the brane. These are electrons, quarks---``normal'' particles.
    \item \textbf{Edge mode:} Wavefunction trapped at $z \approx 0$, exponentially
...
Add a ``Higgs bump'' $h(z)$ that is nonzero only for $z > z_H$ (deep in the interior).
An interior mode overlaps fully with $h(z)$ and gets mass $m \sim m_0$.
The edge mode, trapped at $z = 0$, overlaps only via its exponential tail:
$m_{\text{edge}} \sim m_0 \cdot e^{-z_H / \lambda}$ where $\lambda$ is the decay length.
...
\begin{itemize}[nosep]
    ...
    \item[\ding{51}] Why left-handed only: boundary conditions at $z = 0$ select chirality
          (see Ch.~\ref{ch:va_structure})
\end{itemize}

\paragraph{What this toy model ignores:}
\begin{itemize}[nosep]
    \item[$\times$] The actual form of $V(z)$ from EDC action (not derived: OPR-04)
...
\begin{itemize}[nosep]
    \item Side view of thick brane showing $z$-axis (fifth dimension)
    \item Three regions labeled: 5D Bulk (left), Interface (center), Brane Interior (right)
    \item Neutrino wavefunction $|\psi_\nu(z)|^2$ peaked at interface, decaying both ways
    \item Electron wavefunction $|\psi_e(z)|^2$ peaked in interior
    \item Higgs profile $h(z)$ localized in interior (shaded region)
