...
\begin{itemize}[nosep]
    ...
    \item Potential shape $V(z)$ comes from membrane geometry (ansatz, not derived)
...
\textbf{OPEN:}
\begin{itemize}[nosep]
    \item Derive $V(z)$ from $(\sigma, r_e)$ membrane parameters
...
\begin{itemize}[nosep]
    \item \textbf{5D cause:} Thick-brane profile $f(z)$ governed by effective potential $V(z)$.
...
A 4D particle (electron, quark) corresponds to a profile $f(z)$ in the 5th dimension.
...
$[-d^2/dz^2 + V(z)]f = m^2 f$. This is a 1D quantum mechanics problem with $V(z)$
...
If $f(z)$ is normalized ($\int |f|^2 dz = 1$), then the 4D effective coupling $g_4$
inherits the 5D coupling $g_5$ without extra factors. The \emph{shape} of $f(z)$
determines how strongly the particle couples at any given $z$ \tagDc{}.

\textbf{Step 6: The overlap integral $I_4$ controls weak interactions.}
For four-fermion processes (like $G_F$), what matters is $I_4 = \int |f_L|^4 dz$.
...
Different BCs at $z = 0$ and $z = \ell$ can project out left- or right-handed
...
what could go wrong. Solving the BVP with physical $V(z)$ closes OPR-02/21.
...
For $V(z) = 0$ inside $[0, \ell]$ and $V = \infty$ outside (Dirichlet BCs):
\[
    f_n(z) = \sqrt{\frac{2}{\ell}} \sin\left(\frac{n\pi z}{\ell}\right), \quad
    m_n^2 = \frac{n^2 \pi^2}{\ell^2}
\]
The overlap integral is:
\[
    I_4^{(n)} = \int_0^\ell |f_n|^4 \, dz = \frac{3}{2\ell}
\]
This is $O(1/\ell)$, independent of $n$. In dimensionless units: $\tilde{I}_4 = 3/2$.

\paragraph{Soft walls (realistic case).}
If $V(z)$ is a smooth potential (e.g., sech$^2$ well), the profiles are not pure
...
\begin{itemize}[nosep]
    \item Left panel: Effective potential $V(z)$ vs $z$ (sech$^2$ well)
    \item Right panel: First three bound state profiles $f_0, f_1, f_2$ (ground + excited states)
    \item Annotations: ``LH fermion localized at $z \approx 0$,'' ``Mediator profile peaked at center''
...
\begin{tikzpicture}[node distance=1.8cm, >=stealth, font=\small]
    ...
    \node (pot) [draw, rounded corners, right of=brane, xshift=1.5cm] {Potential $V(z)$};
    \node (bvp) [draw, rounded corners, right of=pot, xshift=1.2cm] {Solve BVP};
    \node (profile) [draw, rounded corners, right of=bvp, xshift=1.2cm] {Profile $f(z)$};
...
\begin{itemize}[nosep]
    \item Physical potential $V(z)$ from membrane parameters
...
The actual physics requires inputting the correct $V(z)$ and BCs.

\textbf{ALLOWED:} Reporting skeleton outputs as ``test case: $I_4 = 1.5$ for sech$^2$ well.''\\
\textbf{FORBIDDEN:} Claiming ``EDC predicts $I_4 = 1.5$'' without deriving $V(z)$.
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item \textbf{RED $\to$ YELLOW:} Derive $V(z)$ from $(\sigma, r_e)$; justify BCs from physics
