...
A 4D particle corresponds to a 5D mode $f(z)$ localized within the brane. The mode
must satisfy boundary conditions at $z = 0$ (bulk-brane interface) and $z = \ell$
...
Given the mode profile $f(z)$, the four-fermion overlap $I_4 = \int |f_L|^4 dz$
determines how strongly fermions couple at a point. Localized profiles (small $I_4$)
give weak coupling---this is the geometric origin of ``weakness'' \tagDc{}.
...
$\to$ mode profile $f(z)$ (5D) $\to$ overlap $I_4$ (5D) $\to$ $G_F$ (3D shadow).
...
For Robin BC at $z = 0$ with Dirichlet at $z = L$, the eigenvalue equation is:
\[
    k \cot(kL) = -\alpha
\]
...
\begin{itemize}[nosep]
    \item[$\times$] Non-trivial potential $V(z)$ (thick-brane shape)
...
\begin{itemize}[nosep]
    \item \textbf{Panel A (DD):} Dirichlet at both ends. Ground state $\sin(\pi z/L)$.
          Eigenvalue $x_1 = \pi$.
    \item \textbf{Panel B (NN):} Neumann at both ends. Ground state constant.
          Eigenvalue $x_1 = 0$.
    \item \textbf{Panel C (Robin):} Robin at $z = 0$, Dirichlet at $z = L$.
...
\begin{tikzpicture}[node distance=1.5cm, >=stealth, font=\small]
    ...
    \node (profile) [draw, rounded corners, left of=mphi, xshift=-1.0cm] {Profile $f(z)$};
