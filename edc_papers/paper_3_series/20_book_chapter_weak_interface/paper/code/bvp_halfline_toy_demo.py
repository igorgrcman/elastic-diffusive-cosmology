...
def generate_latex_table(results: List[dict], output_path: str = None) -> str:
    ...
    lines = [
        ...
        r"% Does NOT close OPR-21; remains OPEN until V(\xi), BCs derived from 5D action",
        r"",
        r"\begin{table}[htbp]",
        r"\centering",
        ...
        r"\toprule",
        r"$\xi_{\max}$ & $\kappa$ & $N_{\text{bound}}$ & $E_0$ & $x_1 = |E_0|$ & $I_4^{(0)}$ \\",
        r"\midrule",
    ]
...
def generate_phase_table(results: List[dict], output_path: str = None) -> str:
    ...
    lines = [
        ...
        r"% Shows stepwise spectral counting; N_bound is OUTPUT of V(\xi)+BC",
        r"% Does NOT close OPR-02/OPR-21; requires physical V(\xi) from 5D action",
        r"",
        r"\begin{table}[htbp]",
        r"\centering",
    ...
    lines.extend([
        ...
        r"an EDC prediction. Physical closure requires deriving $V(\xi)$ from membrane action.}",
        r"\end{table}",
    ])
...
# ==============================================================================
# FIGURE GENERATION: V(ξ) and ψ_0(ξ)
# ==============================================================================
...
def generate_potential_wavefunction_figure(
    ...
    """
    Generate a simple figure showing V(ξ) and ground state ψ_0(ξ).
    ...
    color1 = 'tab:blue'
    ax1.set_xlabel(r'$\xi$ (dimensionless)', fontsize=11)
    ax1.set_ylabel(r'$V(\xi)$', color=color1, fontsize=11)
    ax1.plot(z, V, color=color1, linewidth=1.5, label=r'$V(\xi) = -V_0 \mathrm{sech}^2(\xi/a)$')
    ...
    ax2 = ax1.twinx()
    color2 = 'tab:red'
    ax2.set_ylabel(r'$\psi_0(\xi)$', color=color2, fontsize=11)
    ax2.plot(z, psi0, color=color2, linewidth=1.5, linestyle='-', label=r'$\psi_0(\xi)$')
