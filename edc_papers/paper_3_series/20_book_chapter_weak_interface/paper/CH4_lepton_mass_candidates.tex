...
\begin{enumerate}[nosep]
    \item[(O1)] Full 5D gauge field $A_M$ (Kaluza--Klein reduction)
    \item[(O2)] ``E-only'' static reduction ($A_0 \neq 0$, $\vec{A} = 0$)
    \item[(O3)] Scalar potential model $\Phi$
    \item[(O4)] Polarization/susceptibility model with $\chi \propto e^2$
    \item[(O5)] Kinematic emergence of $\vec{B}$ from motion
    \item[(O6)] Gauge stiffness vs.\ source cancellation
\end{enumerate}
...
\begin{tcolorbox}[colback=yellow!6!white, colframe=yellow!45!black,
    title=\textbf{Critical conclusion for the muon ratio}]
