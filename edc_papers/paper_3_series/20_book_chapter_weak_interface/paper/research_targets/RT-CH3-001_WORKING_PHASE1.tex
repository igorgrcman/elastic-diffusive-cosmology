...
\begin{itemize}
\item If $m(\xi): -m_0 \to +m_0$ as $\xi: -\infty \to +\infty$, there is a
      \textbf{left-handed} zero mode localized at $\xi=0$
\item If $m(\xi): +m_0 \to -m_0$, there is a \textbf{right-handed} zero mode
\end{itemize}

The zero mode profile is:
\begin{equation}
f_L(\xi) \propto \exp\left(-\int_0^\xi m(\xi') d\xi'\right) = \text{sech}^{m_0 L}(\xi/L)
\label{eq:zero_mode}
\end{equation}

\begin{problembox}[The Question]
What determines the \textbf{sign} of the mass profile? In particular, what
fixes whether $m(\xi)$ goes from $-m_0$ to $+m_0$ or vice versa?
...
\begin{equation}
J^\xi_{\text{Plenum}} > 0 \quad \text{(flow in $+\xi$ direction)}
\end{equation}
...
\begin{equation}
\text{sgn}(m(\xi)) = \text{sgn}\left(\frac{\partial P}{\partial \xi}\right)
\end{equation}
...
\begin{enumerate}
\item Plenum flows in $+\xi$ direction (from bulk to brane)
\item This creates higher pressure on the bulk side, lower on the observer side
\item $\partial P/\partial \xi < 0$ (pressure decreases toward observer)
\item The effective fermion mass is induced by this pressure gradient
\end{enumerate}

\begin{keybox}[Key Insight]
If the Plenum pressure gradient determines $m(\xi)$, and the pressure
decreases toward the observer ($\xi=0$), then:
\begin{equation}
m(\xi) = m_0 \tanh\left(\frac{\xi - \xi_0}{L}\right)
\end{equation}
...
\begin{equation}
T^{\xi\xi}_{\text{Plenum}} = \rho_{\text{Plenum}} v_\xi^2 > 0
\end{equation}
...
Since $T^{\xi\xi} > 0$ in the bulk and decreases toward the boundary:
\begin{equation}
m(\xi) > 0 \quad \text{for } \xi > 0 \quad \text{(bulk side)}
\end{equation}
\begin{equation}
m(\xi) \to 0 \quad \text{as } \xi \to 0 \quad \text{(boundary)}
\end{equation}

\subsection{Step 3: Half-Domain Wall}

The EDC geometry is a ``half'' domain wall: $\xi \in [0, L]$ rather than
$\xi \in (-\infty, +\infty)$.

The mass profile is:
\begin{equation}
m(\xi) = m_0 \left(1 - e^{-\xi/\lambda}\right) \approx m_0 \frac{\xi}{\lambda} \quad \text{for small } \xi
\end{equation}
...
With $m(\xi) > 0$ for $\xi > 0$:
\begin{equation}
f_L(\xi) = f_L(0) \exp\left(-\int_0^\xi m(\xi') d\xi'\right)
\end{equation}

Since $m(\xi) > 0$, the integral is positive, so $f_L(\xi)$ \textbf{decreases}
as $\xi$ increases. The mode is localized at the boundary $\xi=0$.
...
This gives:
\begin{equation}
f_R(\xi) = f_R(0) \exp\left(+\int_0^\xi m(\xi') d\xi'\right)
\end{equation}

This \textbf{increases} as $\xi$ increases --- not normalizable! The right-handed
...
\item Plenum inflow in $+\xi$ direction \tagP{}
\item Fermion-Plenum coupling via stress tensor \tagP{}
\end{itemize}

\textbf{We derive:}
\begin{enumerate}
\item The effective mass $m(\xi) > 0$ for $\xi > 0$ (rises into bulk)
...
$m(\xi)$ rises into bulk & From inflow direction & \tagDc{} \\
...
\begin{enumerate}
\item Plenum inflow direction ($+\xi$) --- this is EDC's basic mechanism
