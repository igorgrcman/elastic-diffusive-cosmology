...
eigenvalues of a Schr\"odinger-type equation in the $z$-direction.
...
\begin{enumerate}[label=\textbf{I.\arabic*},nosep]
  \item \textbf{Thick Brane Potential}: $V(z)$ that localizes modes
  \item \textbf{5D Dirac Structure}: Fermion equation in curved 5D
  \item \textbf{Boundary Conditions}: At $z=0$ (frozen) and $z=L$ (bulk)
...
\begin{equation}
\left[ -\frac{d^2}{dz^2} + V_{\text{eff}}(z) \right] f_n(z) = m_n^2 f_n(z)
\end{equation}
...
\begin{itemize}
  \item Volcano potential: $V(z) = V_0 \, \text{sech}^2(z/\Delta)$
  \item Asymmetric well: different behavior at $z=0$ vs $z=L$
