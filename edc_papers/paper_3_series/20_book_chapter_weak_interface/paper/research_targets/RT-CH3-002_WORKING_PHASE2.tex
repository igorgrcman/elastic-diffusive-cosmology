...
\begin{equation}
S_{5D} = \int d^4x \int_0^L d\xi \, g_5 \, \bar\Psi_1 \Gamma^A \Psi_2 \, \Phi_A
\end{equation}

After integrating over $\xi$ and inserting mode profiles:
\begin{equation}
...
\begin{equation}
\mathcal{I}_{\text{overlap}} = \int_0^L |f_1|^2 |f_2|^2 d\xi \sim \frac{1}{\sigma_L}
\end{equation}
...
\begin{equation}
f_L(\xi) = N_L \exp\left( -m_0 \xi + m_0\lambda (1 - e^{-\xi/\lambda}) \right)
\end{equation}

Near the boundary ($\xi \ll \lambda$), this reduces to a Gaussian:
\begin{equation}
f_L(\xi) \approx N_L \exp\left( -\frac{m_0 \xi^2}{2\lambda} \right)
\end{equation}
...
\begin{equation}
\mathcal{I}_4 = \int_0^\infty |f_L(\xi)|^4 d\xi
\end{equation}

With the Gaussian approximation:
\begin{equation}
|f_L|^4 \propto \exp\left( -\frac{2 m_0 \xi^2}{\lambda} \right)
\end{equation}

\begin{equation}
\mathcal{I}_4 = N_L^4 \int_0^\infty \exp\left( -\frac{2 m_0 \xi^2}{\lambda} \right) d\xi
...
\begin{equation}
[\mathcal{I}_4] = \int [|f_L|^4] d\xi = [L^{-2}] \times [L] = [L^{-1}] = [E]
\end{equation}
