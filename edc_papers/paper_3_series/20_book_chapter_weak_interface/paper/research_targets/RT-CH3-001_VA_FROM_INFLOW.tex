...
\begin{quote}
``Impose $P_L \psi |_{\xi=0} = 0$ as boundary condition $\Rightarrow$ V$-$A emerges.''
\end{quote}
...
\begin{enumerate}[label=\textbf{I.\arabic*},nosep]
  ...
  \item \textbf{Plenum Inflow}: Energy flows from bulk INTO brane,
        $J^\xi_{\text{bulk}\to\text{brane}} > 0$ (sign convention from Framework v2.0)
  \item \textbf{5D Dirac Structure}: Fermions are 5D spinors $\Psi(x^\mu, \xi)$
        with standard Clifford algebra $\{\gamma^A, \gamma^B\} = 2\eta^{AB}$
  \item \textbf{Thick Brane Potential}: Mass function $m(\xi)$ or potential $V(\xi)$
        that localizes modes
  \item \textbf{Frozen Boundary}: Observer-facing edge at $\xi=0$ is quasi-static
...
\begin{enumerate}
\item \textbf{Plenum flow defines $+\xi$ direction}

The inflow $J^\xi > 0$ breaks $\xi \leftrightarrow -\xi$ symmetry. This is not
imposed for V$-$A; it's the basic EDC energy-transfer mechanism.

\item \textbf{5D spinor decomposes under flow}

A 5D Dirac spinor $\Psi$ can be decomposed relative to the $\xi$-direction:
\begin{equation}
...
The 5D Dirac equation with $\xi$-dependent mass:
\begin{equation}
\left( i\gamma^\mu \partial_\mu + i\gamma^5 \partial_\xi - m(\xi) \right) \Psi = 0
\end{equation}
The $\gamma^5 \partial_\xi$ term couples chirality to $\xi$-momentum.

\item \textbf{Boundary selects surviving mode}

At the frozen boundary $\xi=0$, require:
\begin{itemize}
...
Separate variables: $\Psi(x,\xi) = \psi(x) f(\xi)$. The $\xi$-dependent part satisfies:
\begin{equation}
\left( \pm \partial_\xi - m(\xi) \right) f_\pm(\xi) = \lambda f_\pm(\xi)
\end{equation}
where $\pm$ corresponds to chirality.

\textbf{Step 2: Inflow boundary condition}

The Plenum inflow means energy flows in $+\xi$ direction. For the mode to be
``carried by'' the inflow (physical) vs ``fighting against'' it (unphysical):
\begin{equation}
\text{Physical:} \quad \partial_\xi f \cdot J^\xi > 0 \quad \text{(mode flows with Plenum)}
\end{equation}
...
\begin{itemize}
  \item Normalizability in the bulk ($\xi \to L$)
  \item Inflow-compatibility at boundary ($\xi = 0$)
\end{itemize}
...
Solve 5D Dirac equation with simple $m(\xi) = m_0 \tanh(\xi/L)$ (domain wall).
...
Compute effective 4D vertex by integrating over $\xi$:
\begin{equation}
\mathcal{L}_{\text{eff}} = \int_0^L d\xi \, \bar\Psi \Gamma \Psi \cdot (\text{mediator})
\end{equation}
