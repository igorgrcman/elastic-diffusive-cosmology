...
\begin{enumerate}[label=\textbf{I.\arabic*},nosep]
  ...
  \item \textbf{Thick Brane Structure}: $V(z)$ with asymmetric boundaries
  \item \textbf{Plenum Inflow}: $J^z > 0$ defines preferred direction
...
\begin{equation}
f_\nu(z) \sim e^{-\kappa z} \quad \text{for } z > 0 \text{ (into bulk)}
\end{equation}
...
\end{equation}
where $h(z)$ is the Higgs/mass profile (brane-localized).

\textbf{Step 3: Suppression mechanism}

If $h(z)$ is localized at $z=0$ but $f_\nu(z)$ extends into the bulk:
\begin{equation}
\frac{m_\nu}{m_e} \sim \frac{\int |f_\nu|^2 h \, dz}{\int |f_e|^2 h \, dz} \sim \exp(-\Delta/\kappa^{-1})
\end{equation}
