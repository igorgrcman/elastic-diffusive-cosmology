...
\paragraph{\texorpdfstring{C1: $\varepsilon = \lambda / \sqrt{2}$.}{C1: epsilon = lambda/sqrt(2).}}
...
\paragraph{\texorpdfstring{C2: $\varepsilon = \lambda \times (\kappa_q / \kappa_\ell)$.}{C2: epsilon = lambda x (kappa-q/kappa-l).}}
...
\paragraph{\texorpdfstring{Full PMNS with discrete $\theta_{12}$ (no smuggling).}{Full PMNS with discrete theta-12 (no smuggling).}}
...
\smallskip
\noindent\emph{PMNS: $\theta_{23}$ closed; $\theta_{13}$ partially closed by $\varepsilon=\lambda/\sqrt{2}$ (no tuning); $\theta_{12}$ still identified---not derived (OPR-05a/b/c).}
\end{tcolorbox}
