% ==============================================================================
% Section 1.4: Unified Pipeline
% ==============================================================================

\section{The Unified Weak-Sector Pipeline}
...
content $E_{\text{brane}}(t)$. Energy conservation at the level of
...
where:
\begin{itemize}[nosep]
  \item $\Pi_{\text{pump}}$ is the bulk$\to$brane pumping power,
  \item $\Pi_{\text{release}}$ is the brane$\to$3D release power,
...
\paragraph{\texorpdfstring{$\mathcal{P}_{\text{energy}}$}{P-energy}: Kinematic gate.}
Enforces kinematic admissibility: only channels with positive Q-value and
available phase space are allowed/\tagBL{}. This is purely kinematic
and does not require EDC-specific assumptions.

\paragraph{\texorpdfstring{$\mathcal{P}_{\text{mode}}$}{P-mode}: Mode-to-output mapping.}
...
\paragraph{\texorpdfstring{$\mathcal{P}_{\text{chir}}$}{P-chir}: Chirality filter.}
...
The 3D output set is defined at the level of the pipeline as:
\begin{equation}
...
For any process, the energy ledger must close:
\begin{equation}
