...
\subsection{Value Closure Attempt: $g_5$ and $\ell$ from Membrane Parameters}
...
$g_4 = g_5$ & With orthonormal KK modes $\int \chi^2 dz = 1$ & Eq.~\eqref{eq:ch11_g4_g5_relation} \\
$g_4 = g_5/\sqrt{\ell}$ & Alternative: if $\int \chi^2 dz = \ell$ & \S\ref{sec:ch11_g5_canonical} \\
\addlinespace
...
\paragraph{Alternative: Direct $R_\xi$ identification.}
...
\begin{equation}
    ...
    \sim 1.7 \text{ MeV}^{1/4} \cdot \text{(GeV)}^{-1/4}
\end{equation}
This is consistent with tight localization on the brane layer.
