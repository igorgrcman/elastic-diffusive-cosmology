...
\subsection{Suppression Mechanism for $\ell$: Attempt A2}
...
\textbf{Falsification:} If BVP solver with physical $V(z)$ gives $m_\phi$ inconsistent
...
\begin{enumerate}[nosep]
    \item \textbf{Input:} Potential $V(z)$ derived from membrane parameters
          $(\sigma, r_e, R_\xi)$
    \item \textbf{Output:} KK spectrum $\{m_n\}$ and mode profiles $\{f_n(z)\}$
    \item \textbf{Test A:} If $m_1 \sim 600$--800 GeV emerges from $V(z)$ with
...
The suppression candidates suggest that the BVP potential $V(z)$ should have:
\begin{equation}
