...
\paragraph{Core distinction (geometry vs physics).}
\textbf{[Def]} In a 5D ambient space, there exists a continuum of distinct 4D
submanifolds (linear hyperplanes), each specified by a normal direction.
Equivalently, the space of 4D hyperplanes through the origin is parameterized
by the normal $n \in S^4$ up to sign, i.e.\ by $RP^4$.
...
coupling structure), not merely by a coordinate rotation.
In EDC, the brane-layer is treated as a \emph{mechanistically realized} 4D
...
\begin{itemize}[nosep]
  ...
  \item We do \emph{not} claim a full SM derivation; this is a mechanism-level
...
side (within the thick-brane zone). Their possible interactions with the
Plenum, and whether they can be indirectly probed, is left for future work.
