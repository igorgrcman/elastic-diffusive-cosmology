...
\input{../../../_shared/style/tikz_style_edc}
...
\begin{tcolorbox}[edcGuardrail, title={Scope Guardrail: Internal Registry}]
This document is a \textbf{living registry/index} for the EDC Weak Program.
It does \textbf{not}:
\begin{itemize}
  \item Introduce new postulates, derivations, or claims
  \item Modify any already-published companion documents
  \item Supersede any published DOI manuscript; it \emph{points to them}
...
The journal-style synthesis (Paper 3J) provides the narrative reading path;
this overview remains an index.
\end{tcolorbox}
...
\midrule
\multicolumn{4}{l}{\textit{OPEN Appendices (toy derivations / structural pathways)}} \\
OPEN-W1 & $G_F$ toy derivation & 10.5281/zenodo.18321396 & v0.1 (internal QA passed) \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Continuum of 4D Submanifolds in 5D and Mechanistic Selection}
\label{sec:4d_in_5d}

\paragraph{Geometry vs.\ physics (core distinction).}
\textbf{[Def]} In a 5D ambient space, there exists a continuum of distinct 4D
linear submanifolds (hyperplanes). A 4D hyperplane through the origin is
uniquely specified by its normal direction $n \in S^4$ up to sign, hence the
space of such hyperplanes is parameterized by
$S^4/\{\pm 1\} \cong \mathbb{RP}^4$.
This is a purely geometric statement: \emph{many} 4D subsets exist.

\paragraph{Mechanistic selection principle.}
\textbf{[P]} A 4D subset becomes \emph{physically distinct} only if 5D dynamics
selects it via a mechanism (boundary conditions, localization potential,
coupling structure, spectral gaps), not merely by a coordinate rotation.
In EDC, the brane-layer is treated as a \emph{mechanistically realized
interface}---a manifestation of physics, not an extra length ``for its own sake''.

\paragraph{EDC viability filter (what can host an observed universe).}
\textbf{[Dc]} We treat the observed brane-layer as a selected interface
constrained by three viability requirements:
\begin{enumerate}[nosep]
  \item[(i)] compatibility with the proton anchor (stable Steiner $120^\circ$
             Y-junction ground state),
  \item[(ii)] ledger closure under bulk--brane exchange,
  \item[(iii)] suppressed leakage at leading order (brane-dominant observed states).
\end{enumerate}
These requirements define a \emph{selection filter} over the continuum of
candidate 4D subsets.

\begin{figure}[ht]
\centering
\input{fig_4d_selection_in_5d}
\caption{Continuum of 4D submanifolds in 5D (geometric) and mechanistic
selection (EDC). A 4D interface becomes physically relevant only when selected
by boundary/coupling structure, constrained by proton-anchor stability and
ledger closure.}
\label{fig:4d_selection}
\end{figure}

\begin{tcolorbox}[edcGuardrail, title={Guardrails (avoid overclaim)}]
\begin{itemize}[nosep]
  \item This subsection does \emph{not} assert the existence of other interfaces
        in nature; it states a geometric continuum and a mechanistic selection
        hypothesis \textbf{[P]/[OPEN]}.
  \item We do \emph{not} claim a full Standard Model derivation; this is a
        mechanism-level organizing principle.
  \item Any ``other 4D interfaces'' and their couplings to the Plenum remain
        \textbf{[OPEN]} until constructed.
  \item Observable consequences, if any, must be stated as falsifiable handles
        and remain \textbf{[OPEN]}.
\end{itemize}
\end{tcolorbox}

\paragraph{Open direction.}
\textbf{[OPEN]} If additional mechanistic 4D interfaces exist, they would
correspond to alternative boundary/coupling realizations on the bulk-facing
side (within the thick-brane zone). Whether they interact with the Plenum and
whether they can be indirectly probed is left for future work.

\subsection{Proton-Anchor Stability Principle}

\begin{postulate}[Proton-Anchor Stability Principle {\normalfont [P]/[Dc]}]
\label{post:anchor}
Within EDC we treat the existence of a stable proton Y-junction ground state
(Steiner 120°, $q=0$) as a \textbf{necessary stability condition} for a
persistent observed sector: without such a minimal-energy anchor, the
bulk--brane ledger lacks a stable baseline and long-lived brane-projected
particle states are not supported.
\end{postulate}

\textbf{Operational meaning (not metaphysics).}
This is not a claim that ``the proton causes spacetime,'' but a
\emph{stabilization requirement}: without a robust, repeatable minimal-energy
junction topology, the bulk--brane ledger would lack a stable baseline and
the model would not support long-lived, reproducible particle-boundary states.
In that sense, the proton ground state is treated as a necessary condition
for a stable, persistent observed sector (our Universe as we measure it).

\begin{tcolorbox}[edcGuardrail, title={Scope Guardrail}]
We do \textbf{not} derive cosmological existence from this principle here;
we use it as a \textbf{selection criterion} for physically realized
mechanistic submanifolds: only those 4D layers/couplings compatible with
a stable proton anchor and ledger closure are considered viable. \tagDc{}
\end{tcolorbox}

\begin{tcolorbox}[edcCornerstone, title={Stability Selection Criterion [P]/[Dc]}]
Any additional 4D mechanistic submanifold coupled to the Plenum must satisfy:
\begin{enumerate}[nosep]
  \item \textbf{Proton-anchor compatibility:} admits a stable Y-junction
        ground state (Steiner 120°)
  \item \textbf{Ledger closure:} preserves the bulk--brane conservation ledger
  \item \textbf{Suppressed leakage:} any cross-layer coupling is subleading
        at leading order
\end{enumerate}
\end{tcolorbox}

\subsection{Generative Closure Principle (Electron + Excitations + Ledger)}

\begin{postulate}[Generative Closure Principle {\normalfont [P]/[Dc]}]
\label{post:generative}
Within EDC we hypothesize that the existence of
\begin{enumerate}[nosep]
  \item[(i)] an \textbf{electron sector} (brane-dominant defect), and
  \item[(ii)] \textbf{excited states} of the proton/electron,
\end{enumerate}
together with \textbf{ledger-closed energy exchange} across the bulk--brane
interface, forms a \emph{necessary generative substrate} for building the rest
of the particle spectrum as admissible brane outputs, higher modes, and
composites. In this view, particles are not created \emph{ex nihilo}; they
arise as allowed outputs/bound states of the same ledger-consistent
absorption $\to$ dissipation $\to$ release pipeline.

This is a \textbf{framework-level closure hypothesis}, not a completed
derivation of the full spectrum.
\end{postulate}

\textbf{One-liner version.}
Without an electron sector + proton/electron excitations that close the ledger,
the model has no generative mechanism for the remaining spectrum.

\begin{tcolorbox}[edcGuardrail, title={Guardrails: What This Principle Does and Does NOT Claim}]
\begin{itemize}[nosep]
  \item \textbf{NOT} a claim that the full SM spectrum is derived here.
  \item \textbf{NOT} a numeric fit: no masses/lifetimes are tuned by this principle.
  \item \textbf{What it asserts:} a necessary mechanism-level closure---without
        an electron sector + accessible excitations + ledger closure, the model
        has no generative pathway for subsequent composites/modes.
  \item \textbf{Open work:} explicit construction of mode spectra + selection
        rules remains \tagOpen{}.
\end{itemize}
\end{tcolorbox}

\textbf{Connection to pipeline and ontology.}
This principle directly motivates the unified decay pipeline established in
\S\ref{sec:pipeline} and the ontology classification in \S\ref{sec:ontology}.
The electron sector and proton excitations provide the \emph{initial conditions};
the absorption--dissipation--release mechanism provides the \emph{dynamics};
ledger closure provides the \emph{selection rules} that constrain which
...
\midrule
OPEN-W1 & Weak sector & Toy derivation of $G_F$ scaling (structural pathway) \\
\bottomrule
\end{longtable}

\paragraph{OPEN-W1: Structural Pathway for $G_F$.}
OPEN-W1 now provides the structural pathway showing how an effective four-Fermi
coupling would emerge from thick-brane mediator exchange:
\begin{equation}
G_{\mathrm{EDC}} \sim \frac{g_{\mathrm{eff}}^2}{m_\phi^2}
  \times \mathcal{O}_{\mathrm{overlap}}
\end{equation}
where $g_{\mathrm{eff}} = g_5 \times \mathcal{O}_{\mathrm{overlap}} \times
\mathcal{O}_{\mathrm{BC}}$ and $\mathcal{O}_{\mathrm{BC}}$ includes the chirality
filter $\mathcal{P}_{\mathrm{chir}}$ from Companion V. \textbf{Numerical closure
remains OPEN} via: (i) mode profiles from brane geometry, (ii) KK spectrum for
$m_\phi$, (iii) BC operator evaluation. See
\texttt{paper\_3\_series/17\_open\_W1\_GF\_toy\_derivation/} for details.
