% Z_N Strong Pinning Regimes: Weak, Intermediate, and Strong
% File: edc_papers/_shared/derivations/zn_strong_pinning_regimes.tex
% Created: 2026-01-29
% Purpose: Extend delta-pinning mode analysis to strong-pinning regime
%
% EPISTEMIC STATUS:
%   [Der] - Regime classification, symmetry argument for mode index stability
%   [Dc]  - Exact localization bounds, intermediate regime details
%
% PREREQUISITE: zn_ring_delta_pinning_modes.tex
% COMPANION: docs/ZN_STRONG_PINNING_ROBUSTNESS_NOTE.md
...
\title{Z$_N$ Strong Pinning Regime Analysis\\[0.5em]
\large Eigenvalue Scaling, Mode Shape, and Localization}
\author{EDC Project}
\date{2026-01-29}
...
\begin{abstract}
We extend the Z$_N$ delta-pinning mode analysis from the weak-pinning regime
to the full range of pinning strengths. We show that the mode \textit{index}
$m = N$ remains stable across all regimes due to Z$_N$ symmetry, while the
mode \textit{shape} transitions from delocalized cosine to localized cusp-like
structure. We derive eigenvalue asymptotics in both limits and provide a
regime classification diagram.
\end{abstract}
