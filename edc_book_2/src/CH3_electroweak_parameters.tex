...
\footnote{This 0.08\% accuracy is \emph{conditional} on Standard Model RG running being valid between the lattice scale ($\sim 200$ MeV) and $M_Z$ ($91$ GeV). New physics at intermediate scales would modify this extrapolation.}
...
The electroweak \emph{mixing structure} follows from one geometric input: $\sin^2\theta_W = 1/4$ from $\mathbb{Z}_6 = \mathbb{Z}_2 \times \mathbb{Z}_3$.
The absolute scale requires baseline inputs \tagBL{}: $\alpha(M_Z)$ and Higgs VEV $v$.
...
\tagDc{}$^\dagger$
\textit{(Derived from $\mathbb{Z}_6$ subgroup structure.)}
...
\tagDc{}
\textit{(Applying neutron lifetime derivation from Chapter~\ref{ch:case_neutron}.)}
...
The Fermi constant arises from the overlap integral:
\begin{equation}
G_F = G_5 \int_0^\infty |f_L(\xi)|^4 \, d\xi = G_5 \times I_4
\end{equation}

where $f_L(\xi)$ is the left-handed fermion mode profile and $I_4$ has dimension
of \emph{energy} (not length). For the normalized exponential profile
$f_L = \sqrt{2m_0}\,e^{-m_0\xi}$, the overlap integral is exactly:
\begin{equation}
I_4 = m_0 \sim 200 \text{ MeV}
\label{eq:ch3_I4_remark}
\end{equation}

The mode overlap provides the \emph{geometric mechanism} for weak interaction
suppression. Quantitative closure requires the full electroweak relations.
\end{remark}
...
\end{equation}
This is correct: $[G_5] \times [I_4] = E^{-3} \times E = E^{-2} = [G_F]$ \tagM{}.

\textbf{3. Toy model: Gaussian profile (pedagogical only)}

For a Gaussian mode normalized on the \emph{full line} but integrated on the
\emph{half-line} $[0,\infty)$:
\begin{equation}
f_L(\xi) = \left(\frac{1}{\pi \sigma_L^2}\right)^{1/4} \exp\left(-\frac{\xi^2}{2\sigma_L^2}\right)
\quad \text{(full-line normalization)}
\end{equation}

The fourth power integrated on the half-line includes a factor of $\frac{1}{2}$:
\begin{equation}
\boxed{
...
\textbf{Note:} The Gaussian is a \emph{toy model}. The physical profile is asymmetric
(exponential decay into the bulk), which we treat next.

\textbf{4. Physical model: Exponential profile}

The asymmetric mass profile from Chapter~\ref{ch:va_structure} produces an
\emph{exponential} mode, not Gaussian. For the normalized half-line exponential:
\begin{equation}
f_L(\xi) = \sqrt{2m_0} \, e^{-m_0\xi}, \qquad \xi \geq 0
\label{eq:ch3_fL_exponential}
\end{equation}

This satisfies $\int_0^\infty |f_L|^2 d\xi = 1$. The overlap integral is:
\begin{equation}
\boxed{
I_4^{\text{exp}} = \int_0^\infty |f_L(\xi)|^4 \, d\xi
= \int_0^\infty (2m_0)^2 e^{-4m_0\xi} \, d\xi
= 4m_0^2 \times \frac{1}{4m_0} = m_0
}
\label{eq:ch3_I4_exponential_m0}
\end{equation}

\textbf{Key result:} For an exponential profile, $I_4 = m_0$ \emph{exactly} \tagDc{}.

\textbf{5. Numerical estimate:}

With $m_0 \sim 200$ MeV (typical localization scale):
\begin{equation}
I_4 = m_0 \sim 200 \text{ MeV} = 0.2 \text{ GeV}
\label{eq:ch3_I4_numerical}
\end{equation}

\textbf{Comparison:} The Gaussian toy model with $\sigma_L \sim 0.05$ fm gives
$I_4 \sim 1.5$ GeV, which is $\sim 8\times$ larger than the physical exponential
result. This illustrates why the correct profile matters for quantitative work.

\textbf{6. The open question: What is $G_5$?}

The overlap mechanism gives $G_F = G_5 \times I_4$ with $I_4 = m_0 \sim 0.2$ GeV
established. However, the 5D coupling $G_5$ is \textbf{not derived} from first
principles \tagP{}. Until $G_5$ is computed from the 5D action (OPR-19), the
overlap pathway provides only qualitative understanding.

\textbf{Naive estimate (illustrative only):}
\begin{equation}
G_F \sim \frac{g_5^2}{M_{5,\mathrm{Pl}}^3} \times I_4 \sim \frac{(4\pi)^2}{(200 \text{ GeV})^3} \times 0.2 \text{ GeV}
\sim 4 \times 10^{-6} \text{ GeV}^{-2}
\end{equation}

This is within an order of magnitude of $G_F^{\text{exp}} = 1.17 \times 10^{-5}$
GeV$^{-2}$, but the agreement is \textbf{not meaningful} without deriving $g_5$
and $M_{5,\mathrm{Pl}}$.

\textbf{7. Summary: Mechanism vs.\ Closure}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Component} & \textbf{Status} & \textbf{Tag} \\
\midrule
$I_4 = m_0$ (exponential profile) & Exact result & \tagDc{} \\
$I_4^{\text{Gauss}} = 1/(2\sqrt{2\pi}\sigma)$ & Half-line corrected & \tagM{} \\
$G_5$ (5D coupling) & Not derived & \tagP{} (OPR-19) \\
Qualitative ``why weak is weak'' & Mechanism established & \tagI{} \\
Quantitative $G_F = 1.17 \times 10^{-5}$ & Via EW relations & \tagDc{} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Bottom line:} The mode overlap mechanism explains \emph{why} $G_F$ is small
(geometric suppression from fermion localization). The \emph{numerical value} comes
from electroweak relations + $\sin^2\theta_W = 1/4$ (Chapter~\ref{ch:z6_program}), not
from the overlap integral alone.
\end{tcolorbox}
...
\item \textbf{Unification:} Once $\sin^2\theta_W$ is known, the electroweak
      ...
      everything else. \emph{Geometry fixes the mixing angle; baseline inputs
      \tagBL{} ($\alpha$, $v$) fix the absolute scale.}
...
$G_F$ & $g^2/(4\sqrt{2}M_W^2)$ & $1.166 \times 10^{-5}$ & $1.166 \times 10^{-5}$ & \textrm{---}$^\dagger$ & [BL] \\
$\tau_n$ & WKB tunneling & 830 s & 879 s & 6\% & \checkmark \\
\bottomrule
\end{tabular}
\end{center}

\noindent{\footnotesize $^\dagger$Using $M_W = gv/2$, the expression $G_F = g^2/(4\sqrt{2}M_W^2)$ reduces identically to $G_F = 1/(\sqrt{2}v^2)$. Hence $G_F$ is fixed by the baseline input $v$ \tagBL{} (scale input; not derived here) and is not an independent geometric prediction.}
...
\textbf{Scale vs.\ structure:} Geometry fixes the mixing angle $\sin^2\theta_W$.
The absolute normalization (e.g., $G_F = 1/\sqrt{2}v^2$) requires baseline scale
input $v$ \tagBL{}.

No \emph{new EDC-specific} free parameters were fitted; the only numerical inputs
are \tagBL{} $\alpha(M_Z)$ and \tagBL{} $v$, plus standard RG/electroweak relations.
