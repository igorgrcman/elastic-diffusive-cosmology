...
\begin{equation}
G_F \propto \int_0^\infty |f_L(\xi)|^4 \, d\xi = I_4
\end{equation}

where $f_L(\xi)$ is the left-handed fermion mode profile. For the asymmetric profile:
\begin{equation}
f_L(\xi) = N_L \exp\left(-m_0 \chi(\xi)\right), \quad \chi(\xi) = \xi - \lambda\left(1 - e^{-\xi/\lambda}\right)
\end{equation}

The mode is localized at $\xi = 0$ with width $\sigma_L = \sqrt{\lambda/(2m_0)}$.
...
For a Gaussian mode localized at $\xi = 0$ with width $\sigma_L$:
\begin{equation}
...
The fourth power is:
\begin{equation}
|f_L(\xi)|^4 = \left(\frac{1}{\pi \sigma_L^2}\right) \exp\left(-\frac{2\xi^2}{\sigma_L^2}\right)
\end{equation}

Integrating:
\begin{equation}
I_4 = \int_0^\infty |f_L(\xi)|^4 \, d\xi = \frac{1}{\sqrt{2\pi}\sigma_L}
\end{equation}
...
The asymmetric profile $f_L \propto \exp(-m_0 \chi(\xi))$ is \emph{not} Gaussian.
For large $\xi$: $\chi(\xi) \approx \xi - \lambda$, so $f_L \propto e^{-m_0(\xi-\lambda)}$.
