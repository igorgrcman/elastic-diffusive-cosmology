...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!50!black,
    title=\textbf{Box 14.2: Physical $\mu$-Sweep Results (OPEN-22-4b.1a)}]
...
\begin{itemize}[nosep]
    ...
    \item Boundary condition: Neumann ($\kappa = 0$) --- \emph{canonical}
    \item Wall-to-domain ratio: $\rho = \Delta/\ell = 0.2$
\end{itemize}
\emph{Different shapes, BCs, or $\rho$ values give different windows. No universal claims.}

\medskip
\textbf{Slice-family scan} (OPEN-22-4b.1): Only $\rho = 0.20$ achieves $N_{\text{bound}} = 3$;
thin walls ($\rho \leq 0.10$) fail for all tested $\kappa \in \{0, 0.5, 1, 2\}$.
...
\begin{tabular}{lcc}
\toprule
\textbf{Quantity} & \textbf{Range (\colorbox{green!20}{Canonical}: Neumann, $\rho=0.2$)} & \textbf{Status} \\
\midrule
$\mu = M_0\ell$ window & $[13.0, 15.5]$ & \tagDc{} shape-dependent \\
$x_1$ (first massive eigenvalue) & $[10.19, 11.38]$ & Computed \\
$|f_1(0)|^2$ (brane amplitude) & $[0.039, 0.158]$ & Computed \\
$G_{\text{eff}}/(g_5^2\ell)$ & $[1.5 \times 10^{-4}, 7.6 \times 10^{-4}]$ & Computed \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
    ...
    \item $N_{\text{bound}} = 3$ requires $\rho = 0.2$ \emph{and} $\mu \in [13, 15.5]$ (Neumann)
    \item Robin BC ($\kappa > 0$): \emph{exploratory} (OPEN-22-4b-R); \textbf{ERRATUM:} apparent $|f_1(0)|^2 \to 0$ was FD bug artifact (see below)
\end{itemize}

\textbf{Evidence}: \texttt{code/open22\_4b1\_slice\_family\_sweep.py},
\texttt{audit/evidence/OPEN22\_4b\_MU\_SWEEP\_AUDIT.md}
