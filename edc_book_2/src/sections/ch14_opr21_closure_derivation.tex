...
\textbf{Evidence files:} All derivation steps are reproducible via the BVP solver code.
\end{tcolorbox}
...
are real, satisfying the self-adjointness criterion (see Derivation Library for proof).
...
\textbf{Outputs:} JSON summary, phase diagram, and robustness table.
...
Full analysis available via the shape-dependence scan script.
...
\textbf{Evidence}: Reproducible via \texttt{open22\_4bR\_phys\_robin\_sweep.py}.
...
\textbf{Evidence}: Reproducible via the Robin sweep script.
\end{tcolorbox}
