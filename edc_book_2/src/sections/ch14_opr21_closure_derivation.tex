...
\begin{itemize}[nosep]
    ...
    \item Robin BC ($\kappa > 0$): \emph{exploratory} (OPEN-22-4b-R-PHYS); affects $|f_1(0)|^2$, not used in canonical tables
\end{itemize}
...
\begin{tcolorbox}[colback=orange!5!white, colframe=orange!50!black,
    title=\textbf{Correction Note: Robin BC Discretization (OPEN-22-4b-FD)}]
...
implementation and \texttt{solve\_bvp} match the analytic Robin eigenvalue equation
in the toy limit ($V = 0$), confirming that $\hat{\kappa}$ \textbf{does} affect both
eigenvalues and brane overlap.

Physical-domain-wall Robin scans remain an open work package (OPEN-22-4b-R-PHYS).
\textbf{Canonical path remains Neumann ($\kappa = 0$)}.

\textbf{Evidence}: \texttt{audit/evidence/OPEN22\_4bFD\_ROBIN\_FEM\_FIX\_REPORT.md},
\texttt{code/open22\_4bFD\_robin\_fd\_verification.py}
\end{tcolorbox}
