...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!50!black,
    title=\textbf{Box 14.2: Physical $\mu$-Sweep Results (OPEN-22-4b)}]
\label{box:ch14_physical_mu_sweep}

\textbf{Physical domain wall potential}: $V_L = M^2 - M'$ \tagDc{}

The following ranges are computed numerically for the \textbf{canonical physical path}
(Neumann BC, $\kappa = 0$, $\rho = \Delta/\ell = 0.2$):
...
\begin{tabular}{lcc}
\toprule
\textbf{Quantity} & \textbf{Range} & \textbf{Status} \\
\midrule
$\mu = M_0\ell$ & $[13, 17]$ & \tagDc{} shape-dependent \\
$x_1$ (first massive eigenvalue) & $[10.2, 11.2]$ & Computed \\
$|f_1(0)|^2$ (brane amplitude) & $[0.05, 0.16]$ & Computed \\
$G_{\text{eff}}/(g_5^2\ell)$ & $[2.1 \times 10^{-4}, 7.6 \times 10^{-4}]$ & Computed \\
$\sigma\Delta^3$ constraint & $[9, 15]$ & \tagDc{} (updated from toy $\sim$75) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[nosep]
    \item $N_{\text{bound}} = 3$ achieved for $\rho \approx 0.2$ across the $\mu$-window
    \item Convergence verified: $<1\%$ change at $N_{\text{grid}} = 4000$
    \item Robin BC with $\kappa > 0$ pushes modes deeper into brane (small $x_1$)
\end{itemize}

\textbf{Evidence}: \texttt{code/open22\_4b\_mu\_sweep\_physical.py},
