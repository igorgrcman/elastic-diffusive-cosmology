...
it is $\sim e^{-\Delta\xi/\kappa^{-1}}$ where $\Delta\xi$ is the separation and $\kappa^{-1}$
is the penetration depth. With $\Delta\xi/\kappa^{-1} \approx 14$, we get $m_\nu/m_e \sim 10^{-6}$
\tagDc{}.
...
\begin{itemize}[nosep]
    \item Large neutrino mass ($m_\nu > 1$ eV): would require $\Delta\xi/\kappa^{-1} < 14$,
...
\begin{itemize}[nosep]
    \item[$\times$] The actual form of $V(\xi)$ from EDC action (not derived: OPR-12)
    \item[$\times$] Angular structure of the edge mode (PMNS mixing requires this)
    \item[$\times$] Absolute mass scale (requires $\lambda$, $\xi_H$ values)
...
\end{equation}
where $\Delta\xi$ is the separation between the neutrino interface position
and the Higgs localization, and $\kappa^{-1}$ is the neutrino penetration depth.
\end{proposition}

\begin{proof}[Derivation \tagDc{}]
For the electron (interior mode) with profile $f_e(\xi)$ peaked at $\xi = \xi_H$:
\begin{equation}
...
\begin{equation}
    m_\nu \sim m_0 \int |f_\nu(\xi)|^2 h(\xi) \, d\xi
    \approx m_0 \cdot e^{-2\kappa \xi_H}
\end{equation}
since $|f_\nu(\xi_H)|^2 \approx e^{-2\kappa \xi_H}$.

The ratio follows:
\begin{equation}
    \frac{m_\nu}{m_e} \approx e^{-2\kappa \xi_H} = e^{-\Delta\xi/\kappa^{-1}}
    \quad \text{with } \Delta\xi \equiv 2\kappa \xi_H
\end{equation}
\end{proof}
...
\begin{equation}
    e^{-\Delta\xi / \kappa^{-1}} \sim 10^{-6}
    \quad\Longrightarrow\quad
    \frac{\Delta\xi}{\kappa^{-1}} \approx 14
\end{equation}
...
By analogy with the charged lepton mass hierarchy (see Chapter~\ref{ch:three_generations}),
the neutrino masses may scale with mode number:
\begin{equation}
    m_{\nu_i} \propto f(n_i) \cdot e^{-\Delta\xi_i/\kappa^{-1}}
...
and the dependence of $\Delta\xi$ on mode number are not derived.
...
Mixing from overlaps & RED & \tagP{} & Mechanism not computed (OPR-13) \\
$\theta_{12} \approx 33°$ & RED & (open) & Not derived (OPR-13) \\
$\theta_{23} \approx 45°$ & RED & (open) & Not derived (OPR-13) \\
$\theta_{13} \approx 8.5°$ & RED & (open) & Not derived (OPR-13) \\
CP phase $\delta_{\text{CP}}$ & RED & (open) & Not addressed (OPR-14) \\
\bottomrule
\end{tabular}
\end{table}
...
% ------------------------------------------------------------------------------
\input{sections/ch6_pmns_attempt1}

% ------------------------------------------------------------------------------
% Include the PMNS overlap model (Attempt 2)
% ------------------------------------------------------------------------------
\input{sections/ch6_pmns_attempt2}

% ------------------------------------------------------------------------------
% Include the Z6 discrete-phase refinement (Attempt 3)
% ------------------------------------------------------------------------------
\input{sections/ch6_pmns_attempt3_z6_refinement}

% ------------------------------------------------------------------------------
% Include the rank-2 and double-path mechanisms (Attempt 4)
% ------------------------------------------------------------------------------
\input{sections/ch6_pmns_attempt4_menu}

% ------------------------------------------------------------------------------
% Include the epsilon derivation (Attempt 4.1)
% ------------------------------------------------------------------------------
\input{sections/ch6_pmns_attempt4_1_derive_epsilon}

% ------------------------------------------------------------------------------
% Include the theta12 origin micro-attempt (Attempt 4.2)
% ------------------------------------------------------------------------------
\input{sections/ch6_pmns_attempt4_2_theta12_origin}
...
\begin{description}[style=nextline, leftmargin=1em, font=\normalfont\bfseries]
    ...
    \item[FM-2: Overlap suppression insufficient.]
        If $\Delta\xi / \kappa^{-1} \ll 14$, the mass ratio $m_\nu/m_e$ would be
...
\midrule
Absolute neutrino mass scale & Derive $\kappa^{-1}$ and $\Delta\xi$ from EDC action \\
Mass hierarchy (normal/inverted) & Connect to mode spectrum \\
PMNS angles from geometry & Explicit overlap calculation \\
CP violation phase $\delta_{\text{CP}}$ & Requires complex phase mechanism \\
...
PMNS: $\mathbb{Z}_6$ overlap (Attempt 2) & YELLOW & \tagDc{} (OPR-13) \\
PMNS: $\mathbb{Z}_6$ discrete phases (Attempt 3) & RED & \tagDc{} \\
PMNS: Rank-2 + $\varepsilon$ (Attempt 4) & \textcolor{green!60!black}{\textbf{GREEN*}} & \tagI{} (OPR-13) \\
$\theta_{23}$ from geometry & GREEN & \tagDc{} \\
$\theta_{12}, \theta_{13}$ structure & YELLOW & \tagI{} (values not derived) \\
Dirac/Majorana prediction & RED & (open) (OPR-15) \\
\bottomrule
\end{tabular}
