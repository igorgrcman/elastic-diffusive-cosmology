...
This chapter applies the $\mathbb{Z}_6 = \mathbb{Z}_2 \times \mathbb{Z}_3$ symmetry analysis
to quark mixing (CKM matrix):
\begin{itemize}[nosep]
    \item $\mathbb{Z}_3$ DFT baseline computed \tagDc{} $\to$ \textbf{strongly falsified}
    \item Overlap model: magnitude hierarchy $\lambda$, $\lambda^2$, $\lambda^3$ derived \tagDc{}
    \item Phase Cancellation Theorem: pure $\mathbb{Z}_3$ charges give $J = 0$ \tagDc{}
    \item Resolution: $\mathbb{Z}_2$ selection produces $\delta = 60°$, $J \simeq 2.9 \times 10^{-5}$ \tagDc{}
\end{itemize}
\textbf{What is NOT claimed:} Full $(\bar\rho, \bar\eta)$ derivation;
$\delta$ residual discrepancy (60° predicted vs.\ 65° observed, 5° gap).
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item \textbf{5D cause:} Generation separation along $z$; $\mathbb{Z}_6$ discrete structure.
    \item \textbf{Brane process:} Wavefunction overlaps across generations.
    \item \textbf{3D shadow:} CKM hierarchy $(\lambda, \lambda^2, \lambda^3)$; CP phase $\delta$.
\end{itemize}

\textbf{Standard Model} fits CKM parameters; EDC derives hierarchy from 5D overlap suppression.
\end{tcolorbox}
...
In the 5D picture, the three quark generations ($u/c/t$ and $d/s/b$) are localized
at different positions along the extra dimension $\xi$ (or equivalently, at different
angular positions in the $\mathbb{Z}_3$ lattice). Each generation has a wavefunction
profile $f_i(\xi)$ that is peaked at its localization center \tagP{}.

\textbf{Step 2: Flavor mixing = overlap.}
The CKM matrix element $V_{ij}$ is proportional to the \textbf{overlap integral}
between the up-type profile $f_i^{(u)}$ and the down-type profile $f_j^{(d)}$:
$V_{ij} \propto \int f_i^{(u)}(\xi) f_j^{(d)}(\xi) d\xi$ \tagDc{}.
...
off-diagonal overlaps are \textbf{exponentially suppressed}: $|V_{ij}| \sim e^{-|\xi_i - \xi_j|/2\kappa}$.
This produces the Wolfenstein hierarchy $\lambda$, $\lambda^2$, $\lambda^3$ \tagDc{}.

\textbf{Step 4: Quarks vs leptons = tight vs loose localization.}
...
This explains the CKM/PMNS asymmetry qualitatively \tagI{}.

\textbf{Step 5: CP phase from discrete structure.}
The $\mathbb{Z}_6 = \mathbb{Z}_2 \times \mathbb{Z}_3$ lattice structure introduces
discrete phases. The $\mathbb{Z}_2$ sign-selection mechanism produces $\delta = 60°$
(5° from PDG 65°) \tagDc{}.
\end{tcolorbox}
...
\begin{description}[style=nextline, leftmargin=1em, font=\normalfont\bfseries]
    ...
    \item[Attempt 2: Overlap model \tagDc{}/\tagP{}:]
        Single parameter $\Delta\xi/\kappa \approx 1.5$ produces Wolfenstein
        hierarchy: $|V_{us}| \sim \lambda$, $|V_{cb}| \sim \lambda^2$,
        $|V_{ub}| \sim \lambda^3$.

    \item[Attempt 2.1: Non-uniform spacing \tagDc{}/\tagCal{}:]
        Two parameters: $\Delta\xi_{12}/(2\kappa) = 1.49$, $\Delta\xi_{23}/(2\kappa) = 3.17$.
        Predicts $|V_{ub}| \approx 0.0094$ (vs.\ PDG $0.0037$): factor 2.5 overshoot.

    \item[Attempt 2.2: Prefactor analysis \tagDc{}/\tagI{}:]
        The 2.5$\times$ overshoot matches $1/|\bar\rho{-}i\bar\eta| = 2.6$ exactly.
        Overlap model gives $A\lambda^3$ structure; CP factor $|\bar\rho{-}i\bar\eta|$ is open.

    \item[Attempt 3: CP phase ansatz --- \textcolor{red}{FALSIFIED}:]
        Explored $\mathbb{Z}_3$ discrete phases; numerical match was based on
        inconsistent phase assignment. Phase Cancellation Theorem (Attempt~4)
        proves pure $\mathbb{Z}_3$ gives $J = 0$. Retained for historical record.

    \item[Attempt 4: $\mathbb{Z}_6 = \mathbb{Z}_2 \times \mathbb{Z}_3$ refinement \tagDc{}:]
        Phase Cancellation Theorem: any $\mathbb{Z}_3$ charge assignment gives $J = 0$.
        Resolution: $\mathbb{Z}_2$ sign selection produces $\delta = 60°$ (5° from PDG 65°).
        $J \simeq 2.9 \times 10^{-5}$ preserved via $\mathbb{Z}_2$ mechanism.
    ...
    \item[Open:]
        Derive $(\bar\rho, \bar\eta)$ from 5D geometry; derive specific $\mathbb{Z}_2$ parity assignment.
\end{description}
\end{tcolorbox}
...
% ==============================================================================
\subsection{\texorpdfstring{Attempt 1: $\mathbb{Z}_3$ DFT Baseline for CKM}{Attempt 1: Z3 DFT Baseline for CKM}}
...
extra dimension:
\begin{align}
    f_i^{(u)}(\xi) &= N_u \exp\!\bigl(-|\xi - \xi_i^{(u)}|/\kappa_u\bigr)
    \label{eq:ch7_profile_u} \\
    f_j^{(d)}(\xi) &= N_d \exp\!\bigl(-|\xi - \xi_j^{(d)}|/\kappa_d\bigr)
...
\begin{equation}
    O_{ij} \propto \exp\!\Bigl(-\frac{|\xi_i^{(u)} - \xi_j^{(d)}|}{2\kappa}\Bigr)
...
% --- FIGURE PLACEHOLDER: Generation Spacing ---
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.8\textwidth}{\centering
\textbf{[FIGURE PLACEHOLDER]}\\[1em]
\textit{Schematic: Generation localization and overlap suppression}\\[0.5em]
Three Gaussian-like profiles for $(u, c, t)$ at positions $\xi_1, \xi_2, \xi_3$,\\
with corresponding $(d, s, b)$ profiles slightly offset.\\
Shaded overlap regions show: large diagonal overlap, small off-diagonal.\\
Arrows indicate $\Delta\xi_{12}$ and $\Delta\xi_{23}$ separations.
}}
\caption{\textbf{Generation spacing determines CKM hierarchy.}
In the EDC picture, quark generations are localized at different positions along
the extra dimension. The CKM element $|V_{ij}|$ is determined by the overlap
between up-type profile $i$ and down-type profile $j$. Large separations
($\Delta\xi \gg \kappa$) produce exponential suppression of off-diagonal
elements, generating the Wolfenstein hierarchy $\lambda$, $\lambda^2$, $\lambda^3$
\tagDc{}.}
\label{fig:ch7_generation_spacing}
\end{figure}
...
Define the \textbf{inter-generation separation} $\Delta\xi$ (characteristic
distance between adjacent generation centers). Then:
\begin{align}
    |V_{ii}| &\sim 1 \quad\text{(same generation: maximal overlap)}
    \label{eq:ch7_diag} \\
    |V_{i,i\pm 1}| &\sim \exp(-\Delta\xi/2\kappa) \equiv \lambda
    \label{eq:ch7_offdiag1} \\
    |V_{i,i\pm 2}| &\sim \exp(-2\Delta\xi/2\kappa) = \lambda^2
...
\begin{itemize}[nosep]
    \item Non-uniform generation spacing: $\Delta\xi_{12} < \Delta\xi_{23}$
...
\begin{equation}
    \xi_i^{(u)} = i \cdot a, \qquad
    \xi_j^{(d)} = j \cdot a + \delta, \qquad
...
% --- FIGURE PLACEHOLDER: CKM vs PMNS Localization ---
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.85\textwidth}{\centering
\textbf{[FIGURE PLACEHOLDER]}\\[1em]
\textit{Schematic: CKM vs PMNS localization contrast}\\[0.5em]
\textbf{Left panel (Quarks/CKM):} Three narrow, well-separated Gaussian profiles\\
representing $(u,c,t)$ and $(d,s,b)$ with small $\kappa_q$.\\
Minimal overlap between adjacent generations $\Rightarrow$ near-diagonal CKM.\\[0.5em]
\textbf{Right panel (Leptons/PMNS):} Three broad, overlapping profiles\\
representing $(e,\mu,\tau)$ and $(\nu_1,\nu_2,\nu_3)$ with large $\kappa_\ell$.\\
Significant overlap $\Rightarrow$ large PMNS angles.\\[0.5em]
Annotation: $\kappa_q / \kappa_\ell \approx 0.4$ \tagI{}.
}}
\caption{\textbf{Why CKM is diagonal while PMNS is not.}
In the EDC localization picture, quark profiles are narrow (small $\kappa_q$, tight
confinement from color charge) while lepton profiles are broad (large $\kappa_\ell$,
color-neutral edge modes). The ratio $\kappa_q/\kappa_\ell \approx 0.4$ accounts
for the dramatic difference between near-diagonal CKM and large-angle PMNS \tagI{}.}
\label{fig:ch7_ckm_vs_pmns_localization}
\end{figure}
...
% ==============================================================================
\subsection{Attempt 2.1: Non-Uniform Generation Spacing}
\label{sec:ch7_attempt2_1}

The uniform-spacing model (Attempt~2) uses a single parameter $\Delta\xi/(2\kappa)$
for all inter-generation transitions. This implicitly assumes generations are
evenly spaced along the extra dimension---a simplification that obscures
predictive structure. We now upgrade to \textbf{non-uniform spacing}:
$\Delta\xi_{12} \neq \Delta\xi_{23}$.

\subsubsection{Two-Parameter Calibration}

\begin{postulate}[Non-Uniform Generation Spacing {\normalfont \tagCal{}}]
\label{post:ch7_nonuniform}
The inter-generation spacings are:
\begin{align}
    |V_{us}| &= \exp\bigl(-\Delta\xi_{12}/(2\kappa)\bigr)
    \quad\Rightarrow\quad \Delta\xi_{12}/(2\kappa) = -\ln|V_{us}| \approx 1.49
    \label{eq:ch7_d12_cal} \\
    |V_{cb}| &= \exp\bigl(-\Delta\xi_{23}/(2\kappa)\bigr)
    \quad\Rightarrow\quad \Delta\xi_{23}/(2\kappa) = -\ln|V_{cb}| \approx 3.17
    \label{eq:ch7_d23_cal}
\end{align}
The second gap is $\sim 2.1\times$ larger than the first.
\end{postulate}

This calibration is epistemically cleaner than Attempt~2: we fit \emph{two}
parameters to \emph{two} observables (rather than forcing uniform spacing).
The key test is whether the \emph{third} element is correctly predicted.

\subsubsection{The V\texorpdfstring{$_{ub}$}{ub} Prediction}

The $u \leftrightarrow b$ transition must traverse \textbf{both} gaps. The
overlap model predicts:
\begin{equation}
    |V_{ub}|_{\text{pred}} = \exp\Bigl(-\frac{\Delta\xi_{12} + \Delta\xi_{23}}{2\kappa}\Bigr)
    = \exp(-1.49 - 3.17) = \exp(-4.66) \approx 0.0094
    \label{eq:ch7_vub_pred}
\end{equation}

\paragraph{Comparison with PDG:}
\begin{equation}
    \frac{|V_{ub}|_{\text{pred}}}{|V_{ub}|_{\text{PDG}}}
    = \frac{0.0094}{0.00369} \approx 2.5
    \label{eq:ch7_vub_ratio}
\end{equation}

\begin{tcolorbox}[colback=orange!5, colframe=orange!50!black,
    title=\textbf{Attempt 2.1 Result: Factor 2.5 Discrepancy}]
The simple exponential overlap ansatz \textbf{overshoots} $|V_{ub}|$ by a factor of 2.5.
This is \emph{not} a failure---it is an \emph{informative discrepancy}:
\begin{itemize}[nosep]
    \item The ``product rule'' $|V_{ub}| \sim |V_{us}| \times |V_{cb}|$ gives
          $0.225 \times 0.042 \approx 0.0094$---exactly what the model predicts
    \item PDG shows $|V_{ub}| \approx 0.0037$, which is $\approx 0.4 \times$ the product
    \item \textbf{Implication:} There is additional suppression beyond simple
          distance traversal
\end{itemize}
\end{tcolorbox}

\subsubsection{Interpretation of the Discrepancy}

Three mechanisms could provide the additional factor $\sim 0.4$ suppression:

\begin{enumerate}
    \item \textbf{Interference effects:}
          In the standard CKM parametrization, $|V_{td}|$ and $|V_{ub}|$ receive
          contributions from multiple paths with relative phases. The overlap
          model ignores these interference terms.

    \item \textbf{Non-exponential profiles:}
          Gaussian profiles $f(\xi) \propto \exp(-z^2/2\sigma^2)$ have different
          overlap integrals. The \emph{product of Gaussians} gives a Gaussian
          with width $\sigma_{12+23} < \sigma_{12} + \sigma_{23}$, potentially
          providing additional suppression.

    \item \textbf{CP phase effects:}
          Complex phases in the overlap integrals can reduce the magnitude of
          certain matrix elements through destructive interference.
\end{enumerate}

\paragraph{Status:}
The factor-2.5 overshoot is \textbf{not catastrophic}---it identifies where
the simple ansatz breaks down and points toward required refinements. The
\emph{structure} (V$_{ub}$ determined by sum of gaps) is correct; the
\emph{prefactor} requires additional physics.

\subsubsection{Unitarity Consideration}

The overlap matrix before normalization:
\begin{equation}
    O_{ij} \propto \exp\Bigl(-\frac{|\xi_i - \xi_j|}{2\kappa}\Bigr)
    \label{eq:ch7_overlap_matrix}
\end{equation}
is \textbf{not unitary}. Unitarization via Gram--Schmidt or QR decomposition
introduces corrections of order $O(\lambda^2)$ to the CKM elements.

For the standard parametrization with mixing angles $\theta_{12}, \theta_{23}, \theta_{13}$:
\begin{itemize}[nosep]
    \item $\sin\theta_{12} \approx |V_{us}| \approx 0.225$ \tagCal{}
    \item $\sin\theta_{23} \approx |V_{cb}| \approx 0.042$ \tagCal{}
    \item $\sin\theta_{13} \approx |V_{ub}|$ --- this is the prediction target
\end{itemize}

The discrepancy suggests that the geometric identification
$\sin\theta_{13} = \exp(-(d_{12}+d_{23}))$ is \emph{approximate}, requiring
$\mathcal{O}(1)$ corrections from unitarization or interference.

\subsubsection{Connection to $\kappa_{\text{quark}} < \kappa_{\text{lepton}}$}

The observation that CKM is nearly diagonal while PMNS has large angles
suggests $\kappa_q \ll \kappa_\ell$ \tagP{}. From the observed hierarchies:
\begin{align}
    |V_{us}|_{\text{CKM}} &\approx 0.22 \quad\Rightarrow\quad
    \Delta\xi/(2\kappa_q) \approx 1.5 \\
    \sin\theta_{12}^{\text{PMNS}} &\approx 0.55 \quad\Rightarrow\quad
    \Delta\xi/(2\kappa_\ell) \approx 0.6
\end{align}

If the inter-generation spacing $\Delta\xi$ is \emph{universal} (same for quarks
and leptons), then:
\begin{equation}
    \frac{\kappa_q}{\kappa_\ell} \approx \frac{0.6}{1.5} \approx 0.4
    \label{eq:ch7_kappa_ratio}
\end{equation}

\paragraph{Physical interpretation:}
Quarks experience color confinement, which compresses their 5D profiles
(smaller $\kappa$). Leptons are color-neutral, allowing broader profiles
(larger $\kappa$) and hence larger inter-generation overlaps.

This is \textbf{qualitative} \tagI{}---the derivation of $\kappa_q/\kappa_\ell$
from QCD-EDC coupling remains (open).

\begin{tcolorbox}[colback=yellow!10, colframe=yellow!50!black,
    title=\textbf{Attempt 2.1 Summary: YELLOW}]
\begin{description}[nosep, font=\normalfont\bfseries]
    \item[Achieved:]
        Two-parameter fit (Δz$_{12}$, Δz$_{23}$) with $|V_{ub}|$ as genuine prediction \tagDc{}.

    \item[Partial success:]
        Structure correct ($|V_{ub}| \sim$ product of gaps), magnitude off by 2.5× \tagDc{}.

    \item[Insight gained:]
        Non-uniform spacing: 2nd gap is 2.1× larger than 1st \tagCal{}.
        Qualitative $\kappa_q/\kappa_\ell \approx 0.4$ from CKM/PMNS comparison \tagI{}.

    \item[Open:]
        Resolve 2.5× discrepancy (interference? non-exponential profiles?);
        derive $\kappa_{q,\ell}$ from 5D action.
\end{description}
\end{tcolorbox}

% ==============================================================================
\subsection{Attempt 2.2: Understanding the Prefactor Discrepancy}
\label{sec:ch7_attempt2_2}

Attempt~2.1 revealed a factor-2.5 overshoot: the overlap model predicts
$|V_{ub}| \approx 0.0094$, while PDG gives $0.0037$. This section analyzes
whether the discrepancy is a flaw or an expected feature of the real-overlap ansatz.

\subsubsection{The Wolfenstein Connection}

In the Wolfenstein parametrization \tagBL{}, the CKM elements scale as:
\begin{equation}
    |V_{us}| = \lambda, \quad
    |V_{cb}| = A\lambda^2, \quad
    |V_{ub}| = A\lambda^3 \sqrt{\bar\rho^2 + \bar\eta^2}
    \label{eq:ch7_wolfenstein_full}
\end{equation}
where $\lambda \approx 0.225$, $A \approx 0.83$, and $\sqrt{\bar\rho^2 + \bar\eta^2} \approx 0.38$.

The overlap model predicts $|V_{ub}| \sim \exp(-(d_{12}+d_{23})) = V_{us} \times V_{cb} / c_{13}$,
which equals $A\lambda^3 \approx 0.0094$---the Wolfenstein result \emph{without}
the $(\bar\rho, \bar\eta)$ factor.

\subsubsection{Quantitative Match}

\begin{table}[ht]
\centering
\caption{Overlap model vs.\ Wolfenstein structure}
\label{tab:ch7_attempt2_2}
\begin{tabular}{lccl}
\toprule
\textbf{Quantity} & \textbf{Overlap} & \textbf{Wolfenstein} & \textbf{Match} \\
\midrule
$|V_{us}|$ & $\exp(-a) = 0.225$ & $\lambda = 0.225$ & Exact \tagCal{} \\
$|V_{cb}|$ & $\exp(-b) = 0.042$ & $A\lambda^2 = 0.042$ & Exact \tagCal{} \\
$|V_{ub}|$ (simple) & $\exp(-(a{+}b)) = 0.0094$ & $A\lambda^3 = 0.0094$ & \textbf{Exact} \tagDc{} \\
$|V_{ub}|$ (full) & $0.0094$ & $A\lambda^3|\bar\rho{-}i\bar\eta| = 0.0036$ & $\times 2.6$ \\
$|V_{ub}|$ (PDG) & $0.0094$ & $0.0037$ & $\times 2.5$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Key insight.}
The expected overshoot from ignoring the CP-related $(\bar\rho, \bar\eta)$ structure is:
\begin{equation}
    \frac{1}{\sqrt{\bar\rho^2 + \bar\eta^2}} \approx \frac{1}{0.38} \approx 2.6
    \label{eq:ch7_overshoot_expected}
\end{equation}
The observed overshoot is $0.0094/0.0037 = 2.5$. \textbf{Agreement within 2\%.}

\subsubsection{Profile Shape Test}

One might hypothesize that a Gaussian profile $\mathcal{O}_{ij} \propto \exp(-d_{ij}^2)$
would provide additional corner suppression. Testing this (with recalibration to match
$|V_{us}|$ and $|V_{cb}|$) gives:
\begin{equation}
    |V_{ub}|_{\text{Gauss}} = \exp\bigl(-(d_{12}+d_{23})^2\bigr) \approx 10^{-4}
    \label{eq:ch7_gaussian_corner}
\end{equation}
This is \emph{far too small}---the Gaussian profile \textbf{over-suppresses} corner
elements. The exponential ansatz is the correct choice.

\subsubsection{Interpretation}

\begin{tcolorbox}[colback=blue!5, colframe=blue!50!black,
    title=\textbf{Attempt 2.2 Resolution}]
The $2.5\times$ overshoot is \textbf{not a flaw}---it is the \emph{expected result}
for a real-valued overlap model without CP structure.

\begin{description}[nosep, font=\normalfont\bfseries]
    \item[What the overlap model captures \tagDc{}:]
        The structure $|V_{ub}| \sim \lambda^3 \sim$ (product of gaps)
        follows geometrically from the extra-dimensional picture.

    \item[What the overlap model misses \tagI{}:]
        The complex factor $(\bar\rho - i\bar\eta)$ in Wolfenstein,
        which reduces $|V_{ub}|$ by $\sim 0.38$.

    \item[Physical implication:]
        To derive the full prefactor, EDC must provide a mechanism for complex
        phases in the overlap integrals---either complex z-positions or
        multiple interfering paths.
\end{description}
\end{tcolorbox}

\begin{tcolorbox}[colback=green!10, colframe=green!50!black,
    title=\textbf{Attempt 2.2 Summary: Structure GREEN, Prefactor YELLOW}]
\begin{itemize}[nosep]
    \item \textbf{Structure} $|V_{ub}| \sim A\lambda^3$: \textcolor{OliveGreen}{\textbf{GREEN}} \tagDc{}
          --- derived from overlap geometry
    \item \textbf{Prefactor} agreement with Wolfenstein: \textcolor{YellowOrange}{\textbf{YELLOW}} \tagI{}
          --- matches $A\lambda^3$, not $A\lambda^3|\bar\rho{-}i\bar\eta|$
    \item \textbf{CP factor} $|\bar\rho{-}i\bar\eta|$: \textcolor{BrickRed}{\textbf{RED}}
          --- requires derivation of complex phases
    \item \textbf{Profile shape}: \textcolor{OliveGreen}{\textbf{GREEN}} \tagDc{}
          --- exponential confirmed; Gaussian over-suppresses
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item Chapter~\ref{ch:three_generations}: $|\mathbb{Z}_3| = 3$ generation identification
    \item Baseline facts: PDG 2024 CKM values \tagBL{}
    \item Localized profile ansatz: exponential $f(\xi) \propto e^{-|\xi-\xi_0|/\kappa}$ \tagP{}
\end{itemize}

\textbf{What depends on this chapter:}
\begin{itemize}[nosep]
    \item Chapter 8 ($G_F$ derivation): uses overlap structure for Fermi constant
    \item Chapter 10 (Unitarity triangles): extends CP analysis
    \item OPR-09, OPR-10, OPR-11, OPR-12: flavor sector open problems
\end{itemize}

\textbf{Consistency checks (not closures):}
\begin{itemize}[nosep]
    \item[\ding{51}] Wolfenstein hierarchy $\lambda, \lambda^2, \lambda^3$ from single parameter \tagDc{}
    \item[\ding{51}] Structure $|V_{ub}| \sim A\lambda^3$ matches overlap geometry \tagDc{}
    \item[\ding{51}] Phase Cancellation Theorem: pure $\mathbb{Z}_3$ gives $J = 0$ \tagDc{}
    \item[\ding{51}] $\mathbb{Z}_2$ sign selection produces $\delta = 60°$ (5° from PDG) \tagDc{}
    \item[$\circ$] $(\bar\rho, \bar\eta)$ derivation: \textbf{OPEN} (OPR-11)
    \item[$\circ$] $\kappa_q / \kappa_\ell$ from first principles: \textbf{OPEN} (OPR-10)
\end{itemize}
\end{tcolorbox}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Attempt 2: Overlap model} \tagDc{}:
          Single parameter $\Delta\xi/\kappa$ produces Wolfenstein hierarchy
          $\lambda$, $\lambda^2$, $\lambda^3$.
    \item \textbf{Attempt 2.1: Non-uniform spacing} \tagDc{}/\tagCal{}:
          Two parameters ($\Delta\xi_{12}$, $\Delta\xi_{23}$) calibrated from $|V_{us}|$, $|V_{cb}|$.
          Predicts $|V_{ub}| \approx 0.0094$, overshoots PDG by factor 2.5.
    \item \textbf{Attempt 2.2: Prefactor analysis} \tagDc{}/\tagI{}:
          The 2.5$\times$ matches $1/|\bar\rho{-}i\bar\eta| = 2.6$ (Wolfenstein).
          Structure $|V_{ub}| \sim A\lambda^3$ is GREEN; CP factor is open.
    \item \textbf{Attempt 3: CP from $\mathbb{Z}_3$ phases} \tagDc{}:
          Generations carry $\mathbb{Z}_3$ charges; CKM acquires phases $\omega^{q_u - q_d}$.
          Jarlskog $J = 2.9 \times 10^{-5}$ predicted (PDG: $3.1 \times 10^{-5}$), 6\% agreement.
    \item \textbf{CKM vs.\ PMNS asymmetry} \tagI{}:
          Quarks localized (small $\kappa_q$) $\to$ near-diagonal;
          leptons delocalized $\to$ large angles. Estimate: $\kappa_q/\kappa_\ell \approx 0.4$.
    \item \textbf{Remaining open:}
          Derive $(\bar\rho, \bar\eta)$ from 5D; resolve $\delta$ discrepancy (120° vs 65°).
\end{enumerate}
\end{tcolorbox}
...
Attempt 2: Overlap model scaling & YELLOW & \tagDc{}/\tagP{} (OPR-09) \\
Attempt 2.1: Non-uniform spacing & YELLOW & \tagDc{}/\tagCal{} \\
Attempt 2.2: Structure $|V_{ub}| \sim A\lambda^3$ & \textcolor{OliveGreen}{GREEN} & \tagDc{} \\
Attempt 2.2: Prefactor $= 1/|\bar\rho{-}i\bar\eta|$ & YELLOW & \tagI{} \\
Wolfenstein hierarchy from $\Delta\xi/\kappa$ & \textcolor{OliveGreen}{GREEN} & \tagDc{} \\
Why quarks $\neq$ leptons ($\kappa_q/\kappa_\ell$) & YELLOW & \tagI{} (OPR-10) \\
\textbf{Attempt 3: Jarlskog $J$ magnitude} & \textcolor{red}{\textbf{FALSIFIED}} & --- \\
\textbf{Attempt 4: Phase Cancellation Theorem} & \textcolor{OliveGreen}{\textbf{GREEN}} & \tagDc{} \\
\textbf{Attempt 4: $\mathbb{Z}_2$ sign selection} & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagDc{}/\tagI{} \\
CP phase $\delta$ (60° vs 65°) & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagDc{}/\tagI{} (OPR-12) \\
CP factor $(\bar\rho, \bar\eta)$ derivation & RED & (open) (OPR-11) \\
\bottomrule
\end{tabular}
\end{table}
...
The CKM sector therefore splits cleanly into two layers. First, the magnitude
hierarchy---near-diagonality and the $\lambda$, $\lambda^2$, $\lambda^3$
suppression pattern---emerges from geometric localization and overlap structure
\tagDc{}, and is quantitatively consistent with the Wolfenstein scaling,
including the correct $A\lambda^3$ order for $|V_{ub}|$. Second, the remaining
discrepancy reduces to a complex prefactor $|\bar\rho - i\bar\eta|$, i.e.,
the CP sector, which is not produced by the present overlap-only model and
remains an explicit open problem (RED/open). In the roadmap of Part~II, this
isolates CP physics as the next high-value target: once a 5D mechanism generates
a controlled complex phase (and the associated Jarlskog invariant), the CKM
description can become fully predictive rather than structurally correct.

At this stage the overlap model fixes the magnitude hierarchy; the remaining
task is to produce a rephasing-invariant complex phase (nonzero Jarlskog $J$),
i.e., physical CP violation.

% ==============================================================================
% Include Attempt 3: CP Phase from Z3 Structure
% ==============================================================================
\input{sections/ch7_attempt3_cp_phase}

% ==============================================================================
% Include Attempt 4: Z6 Refinement for delta
% ==============================================================================
\input{sections/ch7_attempt4_cp_refinement}
