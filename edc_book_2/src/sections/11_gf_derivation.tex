...
$G_F \sim G_5 \int_0^\infty |f_L(\xi)|^4 \, d\xi = G_5 \times I_4$, where $[I_4] = E$.
For an exponential profile, $I_4 = m_0$ exactly (see \S\ref{sec:ch3_fermi}).
Because $f_L(\xi)$ is tightly localized, this integral is small \tagP{}.
...
% --- Figure: Mediator Integration ---
\begin{figure}[htbp]
\centering
\input{figures/fig_mediator_integration.tex}
...
% See Chapter 7 (Electroweak Parameters) for full derivation.
...
\end{equation}
where $f_L(\xi)$ is the left-handed fermion mode profile.

\paragraph{Dimensional analysis.}
If $f_L$ is normalized so that $\int_0^\infty |f_L|^2 \, d\xi = 1$, then
$[f_L] = L^{-1/2}$. Therefore:
\begin{itemize}[nosep]
    \item $|f_L|^4$ has dimension $L^{-2}$
    \item $d\xi$ has dimension $L$
    \item $I_4 = \int |f_L|^4 \, d\xi$ has dimension $L^{-1}$ (i.e., \textbf{energy})
\end{itemize}
...
\begin{figure}[htbp]
\centering
\input{figures/fig_mode_overlap.tex}
\caption{\textbf{Mode overlap determines $G_F$ magnitude.}
The 4D Fermi constant arises from integrating the fourth power of the
left-handed mode profile over the extra dimension. For an exponential
profile with decay constant $m_0$, the overlap integral is exactly
$I_4 = m_0$. Counterintuitively, tighter localization (larger $m_0$)
produces a \emph{larger} $I_4$---this is because tighter modes have
higher peak amplitude when normalized \tagM{}.}
\label{fig:ch11_mode_overlap}
\end{figure}

\paragraph{Exact result for exponential profile.}
The physical mode profile is asymmetric (not Gaussian), decaying exponentially
into the bulk. For a normalized half-line exponential \tagM{}:
\begin{equation}
...
\begin{equation}
    \boxed{
    I_4 = \int_0^\infty |f_L(\xi)|^4 \, d\xi
    = \int_0^\infty (2m_0)^2 e^{-4m_0\xi} \, d\xi
    = 4m_0^2 \times \frac{1}{4m_0} = m_0
    }
    \label{eq:ch11_I4_exact}
\end{equation}

\textbf{Key result:} For an exponential profile, $I_4$ equals the localization
mass scale $m_0$ exactly. This is a clean, model-independent result \tagDc{}.

\paragraph{Numerical estimate.}
For $m_0 \sim 200$ MeV (typical fermion localization scale):
\begin{equation}
    I_4 = m_0 \sim 200 \text{ MeV} \sim 0.2 \text{ GeV}
    \label{eq:ch11_I4_estimate}
\end{equation}

\begin{remark}[Gaussian vs.\ Exponential Profiles]
\label{rem:ch11_gaussian_vs_exp}
A Gaussian profile $f(\xi) \propto e^{-\xi^2/(2\sigma^2)}$ on the half-line gives
$I_4 = 1/(2\sqrt{2\pi}\sigma)$, which is \emph{inversely} proportional to the
width. The exponential profile is physically motivated by the asymmetric mass
$m(\xi) = m_0(1 - e^{-\xi/\lambda})$ from Chapter~\ref{ch:va_structure} and
gives the cleaner result $I_4 = m_0$ \tagDc{}.
\end{remark}

\subsubsection{Order-of-Magnitude Check}

\paragraph{The challenge: What is $G_5$?}
The overlap mechanism gives $G_F = G_5 \times I_4$ with $I_4 = m_0 \sim 0.2$ GeV
established above. However, the 5D coupling $G_5$ is \textbf{not defined} from
first principles \tagP{}. Dimensional analysis suggests $[G_5] = E^{-3}$, but
the numerical value depends on:
\begin{itemize}[nosep]
    \item 5D gauge coupling $g_5$ normalization (from 5D action)
    \item 5D Planck mass $M_{5,\mathrm{Pl}}$ (from bulk gravity)
    \item Angular integration factors
\end{itemize}

\paragraph{Naive estimate (illustrative only).}
If we guess $G_5 \sim g_5^2/M_{5,\mathrm{Pl}}^3$ with $g_5 \sim 4\pi$ and
$M_{5,\mathrm{Pl}} \sim 200$ GeV \tagP{}:
\begin{equation}
    G_F \sim \frac{(4\pi)^2}{(200 \text{ GeV})^3} \times 0.2 \text{ GeV}
    \sim \frac{160}{8 \times 10^6 \text{ GeV}^3} \times 0.2 \text{ GeV}
    \sim 4 \times 10^{-6} \text{ GeV}^{-2}
    \label{eq:ch11_GF_estimate}
\end{equation}
This is within an order of magnitude of $G_F^{\text{exp}} = 1.17 \times 10^{-5}$
GeV$^{-2}$, but the agreement is \textbf{not meaningful} because $g_5$ and
$M_{5,\mathrm{Pl}}$ are not derived.

\begin{tcolorbox}[colback=red!5, colframe=red!50!black,
    title=\textbf{Honest Assessment: $G_5$ Is Open (OPR-19)}]
Without a derivation of $G_5$ from the 5D action, the mode overlap mechanism
provides only \textbf{qualitative understanding}, not \textbf{quantitative
closure}. The numerical value of $G_F$ is correctly obtained via electroweak
relations (§\ref{sec:ch11_numerical}), not via the overlap pathway.
\end{tcolorbox}
...
\paragraph{Forward reference: BVP Work Package.}
The explicit evaluation of the overlap integral $I_4 = \int |f_L(\xi)|^4 \, d\xi$
requires solving the fermion boundary value problem with the potential $V(\xi)$
derived from membrane physics. This is the subject of Chapter~\ref{ch:bvp_framework}
(BVP Framework), which sets up the numerical infrastructure for OPR-21.
...
% ==============================================================================
% G_F CONSTRAINT FALSIFICATION BOX
% ==============================================================================
\input{\EDCPAPERS/_shared/boxes/gf_constraint_box}
