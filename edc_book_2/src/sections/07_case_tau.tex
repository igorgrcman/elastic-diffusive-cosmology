...
reproduce them. This section provides a consistent 5D$\to$brane$\to$3D mechanism
...
The fact that the same framework accommodates both muon (\S\ref{sec:case_muon})
and tau (this section) decay without contradiction is a non-trivial
consistency check:
...
\label{subsec:tau_stoplight}

\begin{tcolorbox}[colback=yellow!10!white, colframe=orange!60!black,
    title=\textbf{Case Tau: Stoplight Verdict}]
...
\midrule
Higher-mode brane excitation & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagP{} \\
Multi-channel opening & \textcolor{OliveGreen}{\textbf{GREEN}} & \tagDc{} \\
$\tau_\tau$ value & \textcolor{BrickRed}{\textbf{RED}} & \tagBL{} (not derived) \\
Mode index $n_\tau = 2$ & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagI{} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Overall: YELLOW} --- Threshold-gated channel opening is consistent;
mode spectrum not derived.

\textbf{Blockers:}
\begin{itemize}[nosep]
\item Mode spectrum quantification from BVP
\item Hadronic tau decay channels (requires pion ontology)
\item Lifetime from mode dynamics
\end{itemize}
