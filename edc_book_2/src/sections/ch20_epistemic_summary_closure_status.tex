...
\textbf{Audit evidence}: see companion repository.\footnote{Repository artifact: \texttt{audit/evidence/CH20\_EPISTEMIC\_SUMMARY\_AUDIT.md}}
\end{tcolorbox}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!50!black,
    title=\textbf{Quick Navigation: The Green Path}, width=\linewidth, left=2mm, right=2mm]
...
\begin{tikzpicture}[node distance=0.6cm, auto,
    block/.style={rectangle, draw, fill=green!10, text width=2.5cm, text centered, rounded corners, minimum height=0.9cm, font=\footnotesize},
    arrow/.style={->, thick, >=stealth}]

    \node [block] (ch14) {Ch14: BVP\\Box~14.2};
...
% ------------------------------------------------------------
% CH20 — 5D Origin Audit Table (Reader Checkpoint)
% ------------------------------------------------------------
\subsection{5D Origin Audit Table (Reader Checkpoint)}
\label{sec:ch20_5d_origin_audit}

\noindent\textbf{Legend:} \textbf{[Dc]} = structurally derived given stated primitives; \textbf{[P]} = primitive/ansatz input; \textbf{[M]} = model-dependent (e.g., specific kink family); \textbf{[Der]} = fully derived with no free primitive beyond declared baseline.

\begin{tcolorbox}[breakable, enhanced, title={5D Origin Audit Table --- from 5D cause to 3D shadow (no new claims)}, colback=white, colframe=black, left=1mm, right=1mm, boxsep=1mm, width=\linewidth]
\tiny
\setlength{\tabcolsep}{1.5pt}
\renewcommand{\arraystretch}{1.2}

\noindent\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{1.5cm} >{\raggedright\arraybackslash}p{1.5cm}@{}}
\toprule
\textbf{Claim} &
\textbf{5D Origin} &
\textbf{Bridge to 3D} &
...
\textbf{SL BVP:} $-\partial_\xi^2 f_n + Vf_n = m_n^2 f_n$ &
Mode expansion in thick-brane; separation of variables in $\xi$ &
Eigenvalue problem on $[0,\ell]$ with BC from junction; $\{m_n\}$ defines mediator tower &
$V(\xi)$, BC, $\ell$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Control:} $\mu := M_0 \ell$ &
Dimensionless control from domain size $\ell$ + mass scale $M_0$ &
$\mu$ governs $N_{\text{bound}}(\mu)$ (generation mechanism) &
$M_0$, $\ell$ &
\textbf{[Dc]} \\
\midrule

\textbf{Generations:} $N_{\text{bound}}=3$ &
Thick-brane localization + effective potential admits finite bound modes &
Counting SL eigenmodes gives ``3 gen'' as BVP spectrum shadow &
$V$ shape, BC family, $\rho$, $\mu$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Wall anchor (OPR-01):} $M_0 = \sqrt{3/2}\,y\sqrt{\sigma\Delta}$ &
Energy scale from wall tension $\sigma$, thickness $\Delta$, Yukawa $y$ &
Defines $M_0$ in $\mu=M_0\ell$; links primitives to BVP &
$\sigma$, $\Delta$, $y$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Thickness (OPR-04):} $\Delta$ from kink &
Scalar wall model provides soliton profile and width &
Fixes $\Delta$ meaning (vs $\delta$, $\ell$, $R_\xi$) &
Kink model choice &
\textbf{[M]/[Dc]} \\
\midrule

\textbf{Dim.\ red.\ (OPR-19):} $1/g_{4,n}^2 = (1/g_5^2)\int|f_n|^2 d\xi$ &
5D gauge action; metric factors cancel (warp cancellation) &
Profile normalization defines $g_{4,n}$ &
Action, measure, $\ell$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Eigenvalue (OPR-20):} $x_n := m_n \ell$ &
Dimensionless eigenvalue from SL spectrum &
Separates geometry ($x_n$) from scale ($\ell$) &
BVP, $\ell$ &
\textbf{[Dc]} \\
\midrule

\textbf{Mediator mass:} $m_1 = x_1/\ell$ &
Lowest SL mode = lightest mediator &
$1/\ell$ sets mass scale; $x_1$ carries BC+$V$ dependence &
$x_1(\kappa,V)$, $\ell$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Contact strength:} $C_{\text{eff}} = g_5^2\ell/x_1^2$ &
5D coupling + domain size + eigenvalue scaling &
Maps to EFT coefficient &
$g_5$, $\ell$, $x_1$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{EFT identity (OPR-22):} $G_{\text{eff}} := g_{4,1}^2/(2m_1^2)$ &
Tree-level exchange of lightest mediator &
Defines ``effective Fermi coupling'' &
Tree dominance &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Natural form:} $G_{\text{eff}} = (g_5^2\ell/2x_1^2)|f_1(0)|^2$ &
Brane overlap via mode amplitude at $\xi=0$ &
Connects OPR-19 + OPR-20 + brane extraction &
$g_5,\ell,x_1$ &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Brane amplitude:} $|f_1(0)|^2$ &
Mode profile from physical $V(\xi)$ + BC family &
Numerical extraction; not independent SM input &
$V$, BC, grid &
\textbf{[Dc]}+\textbf{[P]} \\
\midrule

\textbf{Robin BC (OPEN-22-4b-R):} &
Junction/matching yields Robin BC &
Shows ``green path'' slices where $N_{\text{bound}}=3$ holds &
$\hat\kappa,\rho,\mu,V$ &
\textbf{[Dc]}+\textbf{[P]} \\
\bottomrule
\end{tabularx}
...
\vspace{4pt}
\textit{Reader note: Navigation aid only---maps each statement to its 5D source and conditional status.}
\end{tcolorbox}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECTION 20.2b: PART I ↔ PART II CROSS-REFERENCE TABLE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Part I $\leftrightarrow$ Part II Cross-Reference}
\label{sec:ch20_part1_part2_xref}

The following table maps parameters and results established in Part~I (Framework v2.0)
to their usage in Part~II (Weak Sector). This aids readers navigating between volumes.

\begin{tcolorbox}[breakable, enhanced, colback=blue!3!white, colframe=blue!50!black,
    title=\textbf{Part I $\leftrightarrow$ Part II Parameter Bridge}, width=\linewidth]
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{3cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{1.5cm}@{}}
\toprule
\textbf{Part I Symbol} & \textbf{Part I Origin} & \textbf{Part II Usage} & \textbf{Status} \\
\midrule
$\sigma$ (tension) & Membrane energy density & Wall anchor $M_0 = f(\sigma, \Delta, y)$ in OPR-01 & \tagP{} \\
$r_e$ (electron radius) & Classical electron radius & Lattice scale $\mu_{\text{lattice}} = \hbar c / r_e$ & \tagBL{} \\
$\alpha = (4\pi + 5/6)/6\pi^5$ & Fine-structure constant & Electroweak relations in Ch3; note: $5/6$ is \tagP{} & \tagDc{} \\
$m_p/m_e = 6\pi^5$ & Proton/electron mass ratio & Implicit in $\alpha^{-1}$ structure & \tagDer{} \\
$\mathbb{Z}_6$ symmetry & Hexagonal brane geometry & $\sin^2\theta_W = 1/4$ derivation in Ch3 & \tagI{} \\
Plenum inflow & Energy flow toward brane & V--A chirality selection in Ch9 & \tagP{} \\
Thick brane $\Delta$ & Brane thickness scale & Domain-wall localization, BVP in Ch14 & \tagP{} \\
Frozen regime & $R_\xi \ll \Delta$ condition & Mode spectrum quantization & \tagP{} \\
\bottomrule
\end{tabularx}
...
\begin{itemize}[nosep]
    \item Part~I establishes geometric primitives ($\sigma$, $r_e$, $\mathbb{Z}_6$)
    \item Part~II uses these to derive weak-sector structure ($\sin^2\theta_W$, V--A, $G_{\text{eff}}$)
    \item The bridge is the Plenum inflow mechanism connecting bulk to brane
\end{itemize}

\textbf{What Part~II adds (not in Part~I):}
\begin{itemize}[nosep]
    \item Sturm--Liouville BVP formalism for mode spectra
    \item Robin BC from Israel junction conditions
    \item $N_{\text{bound}} = 3$ generation mechanism
    \item Quantitative $G_{\text{eff}}$ from mode overlap
\end{itemize}
\end{tcolorbox}
...
\begin{tikzpicture}[node distance=1cm, auto,
    prim/.style={rectangle, draw, fill=yellow!20, text width=2.2cm, text centered, rounded corners, minimum height=0.7cm, font=\footnotesize},
    deriv/.style={rectangle, draw, fill=green!20, text width=2.5cm, text centered, rounded corners, minimum height=0.7cm, font=\footnotesize},
    output/.style={rectangle, draw, fill=blue!20, text width=2.8cm, text centered, rounded corners, minimum height=0.7cm, font=\footnotesize},
    arrow/.style={->, thick, >=stealth}]

    % Primitives
    \node [prim] (g5) {$g_5$ [P]};
    \node [prim, below=0.4cm of g5] (sigma) {$\sigma, \Delta, y$ [P]};
    \node [prim, below=0.4cm of sigma] (ell) {$\ell, \rho, \hat\kappa$ [P]};

    % Derived intermediates
    \node [deriv, right=1.5cm of g5] (opr19) {OPR-19\\$g_{4,n} = g_5 f_n(0)$};
    \node [deriv, right=1.5cm of sigma] (opr01) {OPR-01\\$M_0 = f(\sigma,\Delta,y)$};
    \node [deriv, right=1.5cm of ell] (opr21) {OPR-21\\$V_L, N_{\text{bound}}, f_n$};

    % More derived
    \node [deriv, right=1.5cm of opr19] (opr20) {OPR-20\\$m_1 = x_1/\ell$};
    \node [deriv, below=0.25cm of opr20] (f10) {$|f_1(0)|^2$\\from BVP};

    % Output
    \node [output, right=1.5cm of opr20] (geff) {OPR-22\\$G_{\text{eff}}$};
    ...
    \draw [arrow] (f10) -- (geff);
    \draw [arrow] (opr19) to[bend left=15] (geff);
\end{tikzpicture}
\end{adjustbox}
\end{center}
...
Full registry: see companion repository.\footnote{Repository artifact: \texttt{canon/opr/OPR\_REGISTRY.md}}
\end{tcolorbox}
...
\toprule
\textbf{Claimed (Part II)} & \textbf{Not Claimed} \\
\midrule
...
All audit evidence is maintained in the companion repository.\footnote{Repository artifacts: OPR19/20/22 derivation reports, OPEN22 Robin audit, and CH20 summary audit in \texttt{audit/evidence/}.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OVERALL STOPLIGHT VERDICT (2026-01-29)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{tcolorbox}[breakable, enhanced, colback=blue!5!white, colframe=blue!50!black,
    title=\textbf{Book 2 Overall Stoplight Verdict}, width=\linewidth]

\textbf{Summary by Color:}

\begin{center}
\begin{tabular}{@{}lp{8cm}l@{}}
\toprule
\textbf{Status} & \textbf{Claims} & \textbf{Tag} \\
\midrule
\textcolor{OliveGreen}{\textbf{GREEN}} &
    $\sin^2\theta_W = 1/4$ (0.08\% after RG), $g^2$ (1.1\%), $M_W$ (0.2\%),
    V--A structure, $\theta_{23}$ PMNS (3\%), Phase Cancellation Theorem &
    [Der]/[Dc] \\
\addlinespace
\textcolor{YellowOrange}{\textbf{YELLOW}} &
    $G_F$ (self-consistent, mechanism identified), $\tau_n$ (6\%),
    $\theta_{12}$/$\theta_{13}$ PMNS (8--15\%), lepton mass ratios,
    CKM CP phase (5°), BVP mode structure &
    [Dc]/[P] \\
\addlinespace
\textcolor{BrickRed}{\textbf{RED/OPEN}} &
    First-principles $G_F$ (no SM inputs), PMNS from geometry alone,
    neutrino masses, $(g_5, m_\phi)$ from 5D action, Koide derivation &
    [OPEN] \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Verdict: YELLOW (Conditional Success)}

Part~II achieves \emph{structural closure} of the weak sector within EDC:
the derivation chain from 5D postulates to 3D observables is complete and
internally consistent. Quantitative agreement ranges from 0.08\% (excellent)
to 15\% (partial). True [Der] status requires closing the [P] primitives
($\sigma$, $\delta$, $g_5$) from deeper 5D principles---the subject of Part~III.

\textbf{Key achievement:} No Standard Model observable is used as a derivation
input. All SM parameters appear as \emph{outputs} of the geometric chain.

See \S\ref{sec:gate_registry} for the consolidated gate registry.
...
\begin{tcolorbox}[breakable, enhanced, colback=green!5!white, colframe=green!50!black,
    title=\textbf{Epistemic Checkpoint: Part II Status}, width=\linewidth]
