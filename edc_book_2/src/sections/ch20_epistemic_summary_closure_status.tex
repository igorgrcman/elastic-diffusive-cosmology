...
\begin{tcolorbox}[title={5D Origin Audit Table --- from 5D cause to 3D shadow (no new claims)}, colback=white, colframe=black]
\footnotesize
\setlength{\LTpre}{0pt}
\setlength{\LTpost}{0pt}

\begin{longtable}{p{0.17\linewidth} p{0.33\linewidth} p{0.28\linewidth} p{0.11\linewidth} p{0.11\linewidth}}
\textbf{Claim / Formula} &
...
Reduction to eigenvalue problem on $[0,\ell]$ with BC from matching/junction; spectrum $\{m_n\}$ defines mediator tower &
$V(\xi)$, BC family, $\ell$ &
...
\textbf{Generations mechanism:}\\
$N_{\text{bound}}=3$ in a $\mu$-window &
Thick-brane localization + effective potential admits finite bound modes (edge/localized states) &
Counting bound eigenmodes of the SL operator gives ``3 generations'' as shadow of BVP spectrum &
$V(\xi)$ shape family, BC family (Neumann/Robin), $\rho$, $\mu$ &
...
Defines $M_0$ used in $\mu=M_0\ell$; closes one link in the chain from primitives to BVP control &
$\sigma$, $\Delta$, $y$ &
\textbf{[Dc]} (given kink model), \textbf{[P]} (primitives)
\\ \hline

\textbf{Thickness from kink theory (OPR-04):}\\
$\Delta$ from $\lambda\phi^4$ kink (model) &
Scalar wall model provides explicit soliton profile and width parameter &
Fixes the meaning of $\Delta$ (separates it from $\delta$ boundary layer, $\ell$ domain, $R_\xi$ diffusion) &
Choice of kink model ($\lambda\phi^4$), parameterization &
\textbf{[M]} / \textbf{[Dc]} (model-dependent)
\\ \hline

\textbf{Dimensional reduction (OPR-19):}\\
$\dfrac{1}{g_{4,n}^2} = \dfrac{1}{g_5^2}\int_0^\ell d\xi\, |f_n(\xi)|^2$ &
5D gauge action $\int d^5x\sqrt{-G}\,F_{MN}F^{MN}$; metric factors cancel in the gauge kinetic term (warp cancellation) &
Normalization of $\xi$-profiles $f_n$ defines the effective 4D coupling $g_{4,n}$ &
Action form, measure, normalization convention, $\ell$ &
\textbf{[Dc]} (given action + cancellation), \textbf{[P]} (warp/model assumptions)
\\ \hline

\textbf{Eigenvalue definition (OPR-20):}\\
$x_n := m_n \ell$ &
Dimensionless eigenvalue emerges from SL spectrum on finite domain &
Separates ``geometry'' ($x_n$ from BVP) from ``scale'' ($\ell$) and avoids smuggling via $m_n=x_n/\ell$ as ``law'' &
BVP solution, $\ell$ &
\textbf{[Dc]}
\\ \hline

\textbf{Mediator mass (OPR-20):}\\
$m_{\text{med}} = m_1 = x_1/\ell$ &
Lowest SL mode gives lightest mediator in KK-like exchange picture &
$1/\ell$ sets overall mass scale; $x_1$ carries BC+$V(\xi)$ dependence &
$x_1(\kappa,V)$, $\ell$ &
\textbf{[Dc]} (structure), \textbf{[P]} (inputs)
\\ \hline

\textbf{Invariant contact strength (OPR-20):}\\
$C_{\text{eff}} = g_5^2\,\ell/x_1^2$ &
Combine 5D coupling + domain size with eigenvalue scaling; invariant under field redefinitions once mapped to $g_{4,1}^2/m_1^2$ &
Maps to low-energy EFT coefficient; separates coupling reduction (OPR-19) from spectrum (BVP) &
$g_5$, $\ell$, $x_1$ &
\textbf{[Dc]} (derived form), \textbf{[P]} (primitives)
\\ \hline

\textbf{Primary EFT identity (OPR-22):}\\
$G_{\text{eff}} := \dfrac{g_{4,1}^2}{2m_1^2}$ &
Tree-level exchange of the lightest mediator mode between brane-localized currents (EFT matching statement) &
Defines what ``effective Fermi coupling'' means in the 3D shadow language (invariant definition) &
...
Connects OPR-19 (coupling) + OPR-20 (spectrum) + OPR-21/OPEN-22 (brane amplitude extraction) &
$g_5,\ell,x_1,|f_1(0)|^2$ &
\textbf{[Dc]} (structure), \textbf{[Dc]} (if computed), \textbf{[P]} (primitives)
\\ \hline

\textbf{Brane amplitude export (OPEN-22-1 / OPEN-22-4*):}\\
$|f_1(0)|^2$ from physical BVP &
Mode profile determined by the physical $V(\xi)$ + BC family; amplitude is not an independent ``SM input'' &
Numerical extraction with convergence gates; supports $G_{\text{eff}}$ evaluation without PDG smuggling &
$V(\xi)$ family, BC family, grid/convergence settings &
\textbf{[Dc]} (computed in specified regime), \textbf{[P]} (model family)
\\ \hline

\textbf{Robin BC impact (OPEN-22-4b-R-*):}\\
Robin modifies $\mu$-window + overlaps &
Junction/matching yields Robin-type BC; BC enters the SL operator definition &
Shows family of canonical ``green path'' slices where $N_{\text{bound}}=3$ still holds, and where it fails &
$\hat\kappa$, $\rho$, $\mu$, physical $V(\xi)$ &
\textbf{[Dc]} (computed), \textbf{[P]} (family choices)
\\ \hline

\end{longtable}

\vspace{4pt}
\textit{Reader note: This table is a navigation aid. It does not introduce new equations; it maps where each key statement is sourced (5D action / matching / BVP / EFT identity) and what remains conditional.}
\end{tcolorbox}
