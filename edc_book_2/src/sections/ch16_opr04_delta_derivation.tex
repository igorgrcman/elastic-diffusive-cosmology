...
\midrule
$\Delta$ & Kink width & Scalar wall: $\phi = v\tanh(\xi/\Delta)$ & \tagM{} \\
$\delta$ & Boundary-layer & Transport regularization for Robin BC & \tagP{} \\
$\ell$ & Domain support & SL interval for OPR-21: $\mu = M_0\ell$ & \tagP{} \\
$R_\xi$ & Diffusion scale & Correlation length: $R_\xi = \hbar c/M_Z$ & \tagBL{} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    \item \textbf{OPR-01 anchor} (Chapter~\ref{ch:ch15_opr01}):
    \begin{equation}
...
The tension $\sigma$ is the integrated energy density (Chapter~\ref{ch:ch15_opr01}):
\begin{equation}
...
\textbf{Note (OPR-21R):} The window is shape-dependent; see Chapter~14 Box~\ref{box:opr21_mu_shape_dependent}.
For illustration, using toy benchmark $\mu \in [15, 18]$ (P\"oschl-Teller) and $\ell = n\Delta$:
\begin{equation}
...
$\ell = n\Delta$ with $n \sim 4$, then $\mu \approx 0.002$, far below any
physically relevant three-generation window (which is shape-dependent; see OPR-21R).
...
\begin{itemize}[nosep]
    ...
    \item OPR-21: $N_{\text{bound}} = 3$ requires $\mu = M_0\ell \in [\mu_3^-, \mu_3^+](V)$
          (shape-dependent; see OPR-21R) \tagDc{}
    \item $R_\xi = \hbar c/M_Z \approx 2.16 \times 10^{-3}$ fm \tagBL{}
\end{itemize}

\textbf{Conclusion:}
Under (A1)+(A2)+(A3), the resulting $\mu \approx 0.002 \ll \mu_3^-$, hence
...
\begin{itemize}[nosep]
    ...
    \item Audit: see OPR registry in companion repository\footnote{Repository artifact: \texttt{canon/opr/OPR\_REGISTRY.md}}
\end{itemize}
\end{tcolorbox}
...
% ==============================================================================
\subsection{OPR-04 Stoplight Verdict}
\label{subsec:opr04_stoplight}

\begin{tcolorbox}[colback=green!10!white, colframe=green!60!black,
    title=\textbf{OPR-04: Stoplight Verdict}]

\textbf{Status: \textcolor{OliveGreen}{GREEN} [Dc]}

\textbf{What is derived:}
\begin{itemize}[nosep]
\item $\Delta = 2/(v\sqrt{\lambda})$ from kink solution
\item Scale relation $\sigma\Delta = 4v^2/3$
\item Connection to OPR-01 via $M_0(\sigma, \Delta)$
\end{itemize}

\textbf{Conditional on:}
\begin{itemize}[nosep]
\item Kink (tanh) ansatz for scalar field
\item Real scalar field (not complex doublet)
\end{itemize}

\textbf{This OPR is GREEN} because the kink solution is mathematically rigorous
given the $\phi^4$ potential; the ansatz is standard domain-wall physics.
