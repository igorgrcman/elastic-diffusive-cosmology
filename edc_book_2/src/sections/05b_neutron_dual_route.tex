...
A minimal 2D extension using the $\mathbb{Z}_3$-doublet coordinate $\Delta$ does not help (the optimal path remains at $\Delta=0$), so the lifetime mechanism remains open.

% --- Neutron Status Summary Box (Routes A--D) ---
\begin{tcolorbox}[title={Neutron: Status Summary (Routes A--D)}, colback=black!2, colframe=black!35, boxrule=0.6pt]
\small
...
\paragraph{Updated neutron route status (post--Route D).}
...
\midrule
\multicolumn{4}{l}{\textbf{Open:} 3D modes, angular modes, non-WKB mechanisms} \\
\bottomrule
\end{tabular}
\end{center}
