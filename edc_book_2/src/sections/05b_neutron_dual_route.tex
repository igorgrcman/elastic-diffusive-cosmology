...
sector as the proton, without invoking Standard Model weak interaction machinery.
...
Topology-changing processes require energy $\gtrsim \sigma \cdot a^2 \sim \mathcal{O}(100\,\mathrm{MeV})$
to create/annihilate flux tube segments, where $\sigma$ is membrane tension and $a$ is
core size. The available Q-value $\Delta m_{np} c^2 \approx 1.3\,\mathrm{MeV}$ is far below
this threshold. Hence the topological sector is effectively frozen during neutron decay.
...
\begin{itemize}[nosep]
  ...
  \item The barrier mechanism is \textbf{topological/geometric} (core repulsion + confinement
        balance), consistent with the proton anchor derivation.
...
the \emph{effective} (1D WKB) quantitative lifetime. Neither route invokes Standard Model
weak interaction machinery.
...
\begin{itemize}[nosep]
  ...
  \item No Standard Model weak interaction machinery is required for this picture
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item We do \textbf{not} use SM beta decay ($d \to u + W^- \to \ldots$) language
