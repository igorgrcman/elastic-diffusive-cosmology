...
% --- Route C Status Summary Box ---
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!50!black,
                  title={\textbf{Route C Status Summary}}]
...
\textbf{NO-GO (WKB lifetime):}
\begin{itemize}[nosep]
  \item $B/\hbar = 0.009$ under anchored scales vs required $60.7$ \tagDc{} [NO-GO]
  \item 1D tunneling channel cannot explain $\tau_n$ with current barrier
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Find mechanism for $\tau_n$: large geometric factors, non-WKB, or multi-D effects
\end{itemize}

\textbf{Key summary (AC-D1):}
\begin{quote}
\emph{Route~C (5D$\to$1D corridor) closes the origin of $M(q)$, $V(q)$, and $\Gamma_0$;
but the current tunneling channel cannot explain $\tau_n$ because $B/\hbar$ is too small
under anchored scales.}
\end{quote}
...
\begin{itemize}[nosep]
  \item \texttt{derivations/S5D\_TO\_SEFF\_Q\_REDUCTION.md} --- Route C corridor overview
...
\textbf{Canonical cross-check (AC-D3):} The action was computed both ways:
\begin{itemize}[nosep]
...
\textbf{Supporting material:}
\texttt{derivations/ROUTED\_2D\_BOUNCE\_REPORT.md},
\texttt{derivations/code/routeD\_2D\_bounce.py}
\end{tcolorbox}
