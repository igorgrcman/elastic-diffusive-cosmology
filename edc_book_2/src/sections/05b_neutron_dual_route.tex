...
\begin{itemize}[nosep]
  ...
  \item Derive effective mass $M(q)$ from 5D geometry
...
\begin{itemize}[nosep]
  ...
  \item Derive $M(q)$ effective mass from bulk fluctuations
  \item Connect $\Gamma_0$ prefactor to 5D mode spectrum
\end{itemize}
...
--- & $M(q)$ from 5D geometry & OPEN & OPEN \\
--- & $\Gamma_0$ from mode spectrum & OPEN & OPEN \\
\bottomrule
\end{tabular}
\end{center}
...
$\delta$ & $0.1\,\mathrm{fm}$ & \tagI{} & This section & $C$, profile $f(q/\delta)$ \\
...
\begin{itemize}[nosep]
  ...
  \item $\delta = L_0/10$ is an aspect-ratio identification (pancake geometry) \tagI{}
  \item $\sigma$ is derived from $E_\sigma = m_e c^2/\alpha$ hypothesis \tagDc{}
  \item $R_\xi$ (electroweak scale) is distinct from $\delta$ (nucleon scale); ratio $\approx 50$
\end{itemize}

\textbf{Dependency chain:}
\[
\sigma \xrightarrow{\text{[Dc]}} E_0 = \sigma L_0^2 \xrightarrow{\text{[Dc]}} V(q)
