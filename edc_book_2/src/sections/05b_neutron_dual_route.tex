...
and Route B in Derivation Library). Both routes converge on the same physical picture:
...
Route~C (\S\ref{subsec:routeC_corridor}) --- the 5D$\to$1D reduction corridor --- has partially closed the derivation gap.

\textbf{What is NOW CLOSED (Route C):}
\begin{itemize}[nosep]
...
\begin{itemize}[nosep]
  ...
  \item \textbf{[NO-GO]} Route D: 2D $(q,\Delta)$ bounce gives $B_{2\mathrm{D}} = B_{1\mathrm{D}}$ --- no rescue from doublet mode
\end{itemize}

\textbf{What this section does:} Establishes the \emph{structural} picture (Route A),
the \emph{effective lifetime} (Route B), and the \emph{5D derivation status} (Route C: 5D$\to$1D corridor).
\end{tcolorbox}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Proton anchor:} Terminal state of decay (established via dual-route in
        \S\ref{sec:proton_routeA_anchor} and Derivation Library)
...
Full 5D derivation of $V_B$, $M$, $V$ & OPEN $\to$ PARTIAL & Route C \\
\bottomrule
\end{tabular}
\end{center}
...
\subsubsection{Route C: From 5D Action to Effective 1D Potential}
\label{subsec:routeC_corridor}
...
Remark~B. This derivation path is called \textbf{Route~C} (the 5D$\to$1D reduction corridor).

\textbf{Definition:} Route~C asks: \emph{Can the metastable effective potential $V(q)$,
mass $M(q)$, and prefactor $\Gamma_0$ be derived from the 5D action without hidden fitting?}

\begin{tcolorbox}[colback=green!5!white, colframe=green!50!black,
                  title={\textbf{Route C: Executive Summary}}]
...
``pancake'' junction core), not a derivation. See the $\delta$ Decision Tree below
(Box~\ref{box:delta_decision_tree}) for the complete scale taxonomy.
\end{tcolorbox}
...
$\delta_{\mathrm{nucl}}$ & $\sim 0.1\,\mathrm{fm}$ & Nucleon/junction & \tagI{} & Junction core, Route C \\
\bottomrule
\end{tabular}
\end{center}
...
% --- Bridge paragraph before Status Summary ---
\paragraph{Interpretation and scope.}
Routes A--C close the \emph{origin} of the effective 1D ingredients ($M(q)$, $V(q)$, $\Gamma_0$) from the 5D action corridor, fixing the MeV-scale barrier \emph{height}.
Yet the canonical bounce action implies essentially unsuppressed tunneling, so the present 1D WKB channel cannot explain $\tau_n$.
A minimal 2D extension using the $\mathbb{Z}_3$-doublet coordinate $\Delta$ does not help (Route D: optimal path remains at $\Delta=0$).
Testing internal-mode relaxation via a mechanical analogue also fails (Route E: finite discrete modes oscillate quasi-periodically rather than thermalizing).
The lifetime mechanism remains open.

% --- Neutron Status Summary Box (Routes A--E) ---
\begin{tcolorbox}[title={Neutron: Status Summary (Routes A--E)}, colback=black!2, colframe=black!35, boxrule=0.6pt]
\small

\textbf{CLOSED (structural elements established).}
\begin{itemize}\setlength\itemsep{0.2em}
  \item \textbf{Route A (structural picture):} The neutron is a metastable excitation above the proton anchor within the Y--junction framework. \tagDc{}
...
\begin{itemize}\setlength\itemsep{0.2em}
  ...
  \item \textbf{Route D (2D test, $q$ and doublet mode $\Delta$) fails:} For $a(q)>0$, $V(q,\Delta)$ increases monotonically with $\lvert\Delta\rvert$ and the optimal bounce remains at $\Delta=0$, yielding no enhancement ($B_{2\mathrm{D}}\approx B_{1\mathrm{D}}$). \tagNoGo{}
  \item \textbf{Route E (internal mode relaxation) fails:} A mechanical analogue with internal ring DOF shows NO sustained thermalization in conservative dynamics. Finite discrete modes oscillate quasi-periodically rather than thermalizing (FPU-like behavior). \tagNoGo{}
\end{itemize}

\medskip
\textbf{OPEN (what must change to explain $\tau_n$).}
\begin{itemize}\setlength\itemsep{0.2em}
  \item \textbf{Identify the true suppression mechanism:} either a qualitatively different transition channel (multi-dimensional configuration space beyond $(q,\Delta)$) or a non-WKB mechanism. \tagOpen{}
  \item \textbf{Geometric degrees of freedom:} independent leg-length modes (three-edge configuration space), angular/orientation modes of the junction in the brane, or additional collective coordinates implied by the full 5D reduction. \tagOpen{}
  \item \textbf{Robustness/anchoring:} confirm that anchored scales (e.g., $\delta_{\mathrm{nucl}}$ choice) do not hide a large action enhancement; otherwise document the required new scale explicitly. \tagOpen{}
\end{itemize}
...
\textbf{Canonical cross-check:} The action was computed both ways:
\begin{itemize}[nosep]
  \item Canonical $Q$-integral: $B = 1.765\,\mathrm{MeV{\cdot}fm}$
  \item Non-canonical $q$-integral with $M(q)$: $B = 1.765\,\mathrm{MeV{\cdot}fm}$
  \item Relative difference: $< 10^{-7}$ (numerically verified)
\end{itemize}
...
See derivation note ``Canonical Bounce Action Audit'' in the supplementary documentation.
\end{tcolorbox}

% --- Updated Route B Status Map ---
\paragraph{Updated Route B status (post--Route C).}
...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!60!black,
                  title={\textbf{Parameter Provenance Table (Route C)}}]
...
% ==============================================================================
% ROUTE D: 2D Coupled-Mode (q, Δ) Tunneling Test
% ==============================================================================

\subsubsection{Route D: Coupled-Mode (q, Δ) Tunneling Test}
\label{subsec:routeD_2D_bounce}

Route D tests whether extending the tunneling problem from 1D (collective coordinate $q$)
to 2D --- adding the $\mathbb{Z}_3$ doublet asymmetry coordinate $\Delta$ --- provides
additional suppression that could rescue the WKB mechanism.
\emph{This is a negative result:} extending to $(q,\Delta)$ does not increase the bounce action.

\paragraph{Model summary.}
The $\mathbb{Z}_3$-symmetric Y-junction has a doublet mode $\Delta$ measuring ``which leg
is special'' (departure from equal arm lengths). The 2D potential is constrained by symmetry:
\begin{equation}
V(q, \Delta) = V_0(q) + \tfrac{1}{2} a(q) \Delta^2 + \tfrac{1}{4} b(q) \Delta^4
\quad \text{\tagDc{}/\tagP{}}
\label{eq:routeD_potential}
\end{equation}
where $V_0(q)$ is the 1D potential from Route C, and $a(q), b(q)$ are phenomenological
couplings constrained by doublet stability: $a(q_n) > 0$ (no low-lying partners at the
neutron minimum).

\paragraph{Key result: NO-GO.}

\begin{center}
\begin{tabular}{lllp{5cm}}
\toprule
\textbf{Quantity} & \textbf{Value} & \textbf{Tag} & \textbf{Comment} \\
\midrule
$B_{1\mathrm{D}}/\hbar$ (Route C) & $0.0089$ & \tagDc{} & 1D tunneling (baseline) \\
$B_{2\mathrm{D}}/\hbar$ (Route D) & $0.0089$ & \tagDc{}/\tagP{} & 2D tunneling (this work) \\
$B_{2\mathrm{D}}/B_{1\mathrm{D}}$ & $1.0000$ & \tagDc{} & No enhancement \\
Required for $\tau_n$ & $60.7$ & \tagBL{} & To match $\tau_n = 879\,\mathrm{s}$ \\
\midrule
\textbf{Decision} & \textbf{NO-GO} & & Minimal 2D insufficient \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Physical reason.}
The 2D channel does NOT help because:
\begin{itemize}[nosep]
  \item $V(q, \Delta)$ increases monotonically with $|\Delta|$ (since $a(q) > 0$)
  \item No saddle point exists at $\Delta \neq 0$ with lower barrier
  \item The optimal tunneling path stays at $\Delta = 0$ throughout
  \item $\mathbb{Z}_3$ symmetry + stability constraint $\Rightarrow$ $\Delta = 0$ is minimum at every $q$
\end{itemize}

\textbf{Mathematical statement:} If $a(q) > 0$ for all $q \in [q_B, q_n]$, then the
minimum action path has $\Delta(s) = 0$ for all $s$. Any detour into $\Delta \neq 0$
increases both potential energy and path length.

\begin{tcolorbox}[colback=red!5!white, colframe=red!60!black,
                  title={\textbf{Route D Status: NO-GO}}]
\label{box:routeD_status}

\textbf{Tested:} Minimal 2D $(q, \Delta)$ coupled-mode tunneling with $\mathbb{Z}_3$ symmetry.

\textbf{Result:} $B_{2\mathrm{D}} = B_{1\mathrm{D}}$ (ratio = 1.0000). The doublet mode
provides \emph{no} additional suppression.

\textbf{Required multiplier:} $\sim 6800\times$ to reach $B/\hbar \approx 60$.

\textbf{Conclusion:} The $\mathbb{Z}_3$ doublet coordinate cannot rescue the WKB tunneling mechanism.

\textbf{Supporting material:}
See derivation note ``Route D: 2D Bounce Report'' in the supplementary documentation.
\end{tcolorbox}

% ==============================================================================
% ROUTE E: Internal Mode Relaxation Test
% ==============================================================================

\subsubsection{Route E: Internal Mode Relaxation Test}
\label{subsec:routeE_relaxation}

Route E tests whether coupling the collective coordinate $q$ to additional internal
degrees of freedom (a ``ring'' of springs connecting the Y-junction legs) could provide
an effective relaxation mechanism, thermalizing energy away from the tunneling coordinate.

\paragraph{Model.}
A toy mechanical analogue: 4-node spring-mass system with central node (mass $m_0$) connected
to three outer nodes (mass $m_{\mathrm{out}}$) via leg springs ($k_{\mathrm{leg}}$), and the
outer nodes connected in a triangular ring via ring springs ($k_{\mathrm{ring}}$). The collective
coordinate $q$ measures symmetric breathing; internal modes include ring deformations and doublet
asymmetries.

\paragraph{Relaxation modes tested.}
\begin{itemize}[nosep]
  \item \textbf{Mode 1 (damped):} Add Rayleigh viscous damping $\gamma$. Measures $t_{\mathrm{relax}}$.
  \item \textbf{Mode 2 (conservative):} No dissipation. Check if internal modes act as effective bath.
\end{itemize}
...
\toprule
\textbf{Test} & \textbf{Outcome} & \textbf{Tag} & \textbf{Comment} \\
\midrule
Mode 1 (damped) & $t_{\mathrm{relax}} \propto \gamma^{-1}$ & \tagDc{} & Expected scaling \\
Mode 2 (conservative) & No thermalization & \tagDc{} & Quasi-periodic (FPU-like) \\
Effective $q$-relaxation & Transient only & \tagDc{} & Not sustained \\
\midrule
\textbf{Decision} & \textbf{NO-GO} & & Internal modes insufficient \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Physical reason.}
In a finite-mode conservative system (8 DOF total, 6 independent after removing CM),
energy oscillates quasi-periodically between modes rather than thermalizing. True
relaxation requires either:
\begin{enumerate}[nosep]
  \item A continuum of modes (not present in the discrete toy model), or
  \item Genuine nonlinear mode-mode coupling with many DOF, or
  \item External dissipation channel (added in Mode 1).
\end{enumerate}

\begin{tcolorbox}[colback=red!5!white, colframe=red!60!black,
                  title={\textbf{Route E Status: NO-GO}}]
\label{box:routeE_status}

\textbf{Tested:} Y-junction + ring mechanical analogue with internal DOF.

\textbf{Result:} Mode 2 (conservative) shows NO sustained thermalization. Mode 1 (damped) relaxes
with $t_{\mathrm{relax}} \propto \gamma^{-1}$, requiring external dissipation.

\textbf{Implication:} Discrete internal modes cannot rescue the lifetime mechanism. A continuum
(bulk field modes, brane fluctuations) or quantum dissipation channel is required.

\textbf{Supporting material:}
180-run parameter scan documented in supplementary derivation note.
\end{tcolorbox}

% --- Updated Overall Status ---
\paragraph{Updated neutron route status (post--Route E).}

\begin{center}
\begin{tabular}{lllp{5.5cm}}
\toprule
\textbf{Route} & \textbf{What it tests} & \textbf{Status} & \textbf{Comment} \\
\midrule
A & Structural metastability & CLOSED & Neutron is metastable Y-junction \tagDc{}+\tagP{} \\
B & Effective 1D WKB & PARTIAL & Formula correct; $\tau_n$ not reproduced \\
C & 5D$\to$1D reduction & PARTIAL & $M(q)$, $V(q)$, $\Gamma_0$ derived \tagDc{} \\
  & 1D bounce action & \textbf{NO-GO} & $B/\hbar = 0.009$ vs required $60.7$ \\
D & 2D $(q,\Delta)$ bounce & \textbf{NO-GO} & $B_{2\mathrm{D}} = B_{1\mathrm{D}}$, no help \\
E & Internal mode relaxation & \textbf{NO-GO} & FPU-like, no thermalization \\
\midrule
\multicolumn{4}{l}{\textbf{Open:} Continuum modes, bulk dissipation, non-WKB mechanisms} \\
\bottomrule
\end{tabular}
\end{center}
