...
\label{subsec:muon_stoplight}

\begin{tcolorbox}[colback=yellow!10!white, colframe=orange!60!black,
    title=\textbf{Case Muon: Stoplight Verdict}]
...
\midrule
Brane-dominant ontology & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagP{} \\
Channel selection ($e^-$ only) & \textcolor{OliveGreen}{\textbf{GREEN}} & \tagDc{} \\
$\tau_\mu$ value & \textcolor{BrickRed}{\textbf{RED}} & \tagBL{} (not derived) \\
Chiral projection $\mathcal{P}_{\text{chir}}$ & \textcolor{YellowOrange}{\textbf{YELLOW}} & \tagP{} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Overall: YELLOW} --- Kinematic selection works; brane-dominant ontology
is postulated, not derived from 5D action.

\textbf{Blockers:}
\begin{itemize}[nosep]
\item Mode spectrum derivation from BVP (OPR-21)
\item Lifetime $\tau_\mu$ from first principles
\item Precise form of $\mathcal{P}_{\text{chir}}$ operator
\end{itemize}
