...
combined with the canonical $G_{\text{eff}}$ formula (OPR-22):
\begin{equation}
    G_{\text{eff}} \sim \frac{g_5^2 \ell |f_1(0)|^2}{2 x_1^2}
    \sim \frac{6650 \times 0.04 \times |f_1(0)|^2}{2\pi^2}
    \sim 13.5 \times |f_1(0)|^2 \text{ GeV}^{-2}
\end{equation}
...
\paragraph{Required overlap integral (legacy approach).}
\emph{Note: This uses the deprecated bulk-overlap formula; see OPR-22 (Ch.~\ref{ch:opr22_geff}) for canonical brane-localized approach.}
From the legacy closure spine Eq.~\eqref{eq:ch11_closure_spine}:
\begin{equation}
    G_F^{\text{(legacy)}} = \frac{g^2 \ell^2 I_4}{x_1^2}
    \quad\Rightarrow\quad
...
the exact relation is $I_4 = m_0$ (see \S\ref{sec:ch3_fermi}), so:
\begin{equation}
