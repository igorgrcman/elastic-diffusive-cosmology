...
\begin{tcolorbox}[colback=orange!5!white, colframe=orange!50!black,
    title=\textbf{Key Definition: Effective Contact Strength $G_{\text{eff}}$}]

We define the effective contact strength:
\begin{equation}
...
\begin{itemize}
    \item $[g_5^2] = L$
    \item $[f_1(0)^2] = 1$ (natural normalization)
    \item $[m_1^2] = L^{-2}$
    \item $[G_{\text{eff}}] = L \cdot 1 / L^{-2} = L^3$
\end{itemize}

\textbf{Wait} --- this gives $L^3$, but we need $L^2 = \text{GeV}^{-2}$.

\textbf{Resolution}: The careful dimensional analysis shows that in natural normalization,
$[f_n] = 1$ but the \emph{coupling} $g_{\text{eff},n} = g_5 f_n(0)$ has $[g_{\text{eff},n}] = L^{1/2}$,
so $[g_{\text{eff},n}^2] = L$, and:
\begin{equation}
    [G_{\text{eff}}] = \frac{L}{L^{-2}} = L^3 \neq L^2 \quad \text{(inconsistent)}
\end{equation}

This inconsistency arises from conflating two conventions. Let us be more careful.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECTION 9: COMPARISON BOX (OPTIONAL)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
