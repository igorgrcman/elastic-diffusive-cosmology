...
\begin{tcolorbox}[breakable, enhanced, colback=blue!5!white, colframe=blue!50!black,
    title=\textbf{Reader's Map: What This Chapter Does and Does Not Claim}, width=\linewidth]
...
\textbf{Dependencies}:
\begin{itemize}
    \item OPR-19 (Chapter~\ref{sec:ch17_opr19}): $g_5 \to g_4$ dimensional reduction
...
% ------------------------------------------------------------------------------
\begin{tcolorbox}[colback=orange!3!white, colframe=orange!40!black,
    title=\textbf{Robin BC Note}]
Earlier Robin-BC results showing apparent decoupling were an artifact of an incorrect
FD ghost-point discretization. A weak-form FEM implementation matches the analytic
Robin eigenvalue equation in the toy limit ($V=0$), confirming that $\hat{\kappa}$
does affect both eigenvalues and brane overlap. Physical Robin scans remain OPEN-22-4b-R-PHYS.
\textbf{Canonical path remains Neumann ($\kappa = 0$).}
\end{tcolorbox}
...
In the warped metric ansatz (cf.\ OPR-19, Chapter~\ref{sec:ch17_opr19}):
\begin{equation}
...
Following OPR-19 (Chapter~\ref{sec:ch17_opr19}), we adopt the \textbf{natural normalization}:
\begin{equation}
...
\subsection{Physical BVP Results (OPEN-22-4 / OPEN-22-4b)}
\label{sec:ch19_physical_results}

Running the pipeline OPR-01 $\to$ OPR-21 $\to$ OPR-22 with physical domain wall
potential $V_L = M^2 - M'$ \tagDc{} and Neumann BC ($\kappa = 0$), $\rho = \Delta/\ell = 0.2$:

\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|c|c|c|}
\hline
$\mu$ & $\rho$ & $N_{\text{bound}}$ & $x_1$ & $|f_1(0)|^2$ & $G_{\text{eff}}/(g_5^2\ell)$ \\
\hline
\textbf{13.0} & \textbf{0.20} & \textbf{3} & \textbf{10.19} & \textbf{0.158} & $\mathbf{7.62 \times 10^{-4}}$ \\
\textbf{14.0} & \textbf{0.20} & \textbf{3} & \textbf{10.69} & \textbf{0.093} & $\mathbf{4.05 \times 10^{-4}}$ \\
\textbf{15.0} & \textbf{0.20} & \textbf{3} & \textbf{11.16} & \textbf{0.052} & $\mathbf{2.11 \times 10^{-4}}$ \\
16.0 & 0.20 & 4 & 11.61 & 0.029 & $1.08 \times 10^{-4}$ \\
\hline
\end{tabular}
\caption{Physical BVP scan with domain wall potential. Target regime ($N_{\text{bound}}=3$)
    highlighted. Parameters: Neumann BC ($\kappa=0$), $\rho=0.2$, $y=1$. Source: OPEN-22-4b.}
\label{tab:ch19_physical_scan}
\end{table}

\textbf{Key observations} (OPEN-22-4b / OPR-21R):
\begin{itemize}[nosep]
    \item $N_{\text{bound}} = 3$ for $\mu \in [13, 17]$ with $\rho \approx 0.2$
    \item The deprecated ``universal'' $[25, 35)$ window was a \emph{toy artifact}
    \item $\mu_3$ is \textbf{shape-dependent}: domain wall gives $[13, 17]$, toy PT gives $[15, 18]$
\end{itemize}

\subsection{Physical $G_{\text{eff}}$ Formula}
\label{sec:ch19_geff_physical}

In the target regime (Neumann BC, $\rho = 0.2$, $\mu \in [13, 15.5]$), the physical ranges are:
\begin{align}
    x_1 &\in [10.19, 11.38] \notag \\
    |f_1(0)|^2 &\in [0.039, 0.158] \notag \\
    G_{\text{eff}}/(g_5^2 \ell) &\in [1.5 \times 10^{-4}, 7.6 \times 10^{-4}]
    \label{eq:ch19_geff_physical}
\end{align}

\textbf{NON-UNIVERSALITY}: These ranges are \textbf{CONDITIONAL} on
$(V_L = M^2 - M',\; \kappa = 0,\; \rho = 0.2)$. Different shapes give different bands.

\textbf{Comparison to toy limit} (for orientation only):
\begin{itemize}
    \item Toy ($V=0$, Neumann): $G_{\text{eff}}^{\text{(toy)}} = (g_5^2 \ell) / \pi^2 \approx 0.10 \times (g_5^2 \ell)$
    \item Physical (Neumann, $\rho=0.2$): $G_{\text{eff}}^{\text{(phys)}} \in [1.5 \times 10^{-4}, 7.6 \times 10^{-4}] \times (g_5^2 \ell)$
\end{itemize}

The physical potential yields a smaller $G_{\text{eff}}/(g_5^2\ell)$ ratio than the toy limit,
primarily because (1) $x_1 \sim 10$--$11$ (vs.\ $\pi$) and (2) $|f_1(0)|^2 \sim 0.04$--$0.16$ (vs.\ $\sim 2$).
The ratio $G_{\text{eff}}^{\text{(phys)}}/G_{\text{eff}}^{\text{(toy)}}$ is slice-dependent and
\textbf{no universal suppression factor} is claimed.
...
\begin{itemize}
    \item \textbf{OPR-19} (Chapter~\ref{sec:ch17_opr19}): $g_5 \to g_4$ dimensional reduction
...
% ==============================================================================
\subsection{OPR-22 Stoplight Verdict}
\label{subsec:opr22_stoplight}

\begin{tcolorbox}[colback=yellow!10!white, colframe=orange!60!black,
    title=\textbf{OPR-22: Stoplight Verdict}]

\textbf{Status: \textcolor{YellowOrange}{YELLOW} [Dc]}

\textbf{What is derived:}
\begin{itemize}[nosep]
\item $G_{\text{eff}} = g_5^2 \ell |f_1(0)|^2 / (2 x_1^2)$ from 5D exchange
\item No-smuggling certification (no SM inputs)
\item Physical domain-wall run with $N_{\text{bound}} = 3$
\end{itemize}

\textbf{Blockers to GREEN:}
\begin{itemize}[nosep]
\item $g_5$ not derived from UV completion (OPEN-22-2)
\item $\ell$ not derived from first principles (OPEN-22-3)
\item Brane kinetic term corrections (OPEN-22-5)
\end{itemize}

\textbf{Key achievement:} Self-consistent $G_{\text{eff}}$ within the framework;
comparison to $G_F$ is meaningful only after closing OPR-19/20.
