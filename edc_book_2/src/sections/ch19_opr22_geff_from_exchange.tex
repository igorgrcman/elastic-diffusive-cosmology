...
In the target regime ($\mu \in [13, 17]$, $N_{\text{bound}} = 3$, $\rho = 0.2$), the physical ranges are:
\begin{align}
    x_1 &\in [10.2, 11.2] \notag \\
    |f_1(0)|^2 &\in [0.05, 0.16] \notag \\
    G_{\text{eff}}/(g_5^2 \ell) &\in [2.1 \times 10^{-4}, 7.6 \times 10^{-4}]
...
\begin{itemize}
    ...
    \item Physical ($\mu \in [13,17]$, $\rho=0.2$): $G_{\text{eff}}^{\text{(phys)}} \in [2 \times 10^{-4}, 8 \times 10^{-4}] \times (g_5^2 \ell)$
\end{itemize}

The physical potential suppresses $G_{\text{eff}}$ by a factor of $\sim 100$--$500$ relative to
the toy limit, due to (1) larger $x_1 \sim 10$--$11$ and (2) smaller $|f_1(0)|^2 \sim 0.05$--$0.16$.
