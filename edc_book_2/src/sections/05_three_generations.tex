...
\begin{figure}[htbp]
\centering
\input{figures/fig_three_channel_localization.tex}
...
The EDC thick brane supports a hexagonal lattice of flux tubes (see Chapter~\ref{ch:ontology}).
...
$|\mathbb{Z}_3| = 3 \to N_{\text{gen}}$ & No & \textcolor{red!80!black}{\textbf{RED}} & Cardinality matching, not derivation (OPR-03) \\
...
Barrier form specified & No & \textcolor{red!80!black}{\textbf{RED}} & $V(\xi)$ not derived from EDC action (OPR-12) \\
Lifetime calculation & No & \textcolor{red!80!black}{\textbf{RED}} & No explicit $S_n$ computed (OPR-12) \\
Cutoff at $n = 3$ & No & \textcolor{red!80!black}{\textbf{RED}} & Would need specific barrier height (OPR-12) \\
\bottomrule
\end{tabular}
\end{center}
...
\medskip
\noindent\textbf{Status: \tagP{}/\tagOPEN{} (OPR-03).}
This mechanism requires specifying the global topology of $\mathcal{M}^5$, which is
not constrained by local EDC dynamics. No computation exists. See also the connection
to KK truncation (OPR-12), which would provide an alternative pathway if the effective
potential $V(\xi)$ could be derived from membrane physics.
\end{tcolorbox}
...
\midrule
A: $\mathbb{Z}_3 \subset \mathbb{Z}_6$ & Partial \tagI{} & No & \textcolor{orange!80!black}{\textbf{YELLOW}} (OPR-03) \\
B: KK truncation & No \tagP{} & No & \textcolor{red!80!black}{\textbf{RED}} (OPR-12) \\
C: $\pi_1(\mathcal{M}^5) = \mathbb{Z}_3$ & No \tagP{} & No & \textcolor{red!80!black}{\textbf{RED}} (OPR-03) \\
\bottomrule
\end{tabular}
\end{center}

% --- Figure: Overlap → mixing intuition ---
\begin{figure}[htbp]
\centering
\input{figures/fig_overlap_mixing.tex}
...
\begin{itemize}[nosep]
    \item[$\circ$] Why is $n \geq 3$ forbidden? (OPR-12: KK truncation)
    \item[$\circ$] Derive $V(\xi)$ potential from EDC action (OPR-12)
    \item[$\circ$] Show mode $n = 3$ is unstable or non-normalizable (OPR-12)
\end{itemize}
