...
\textbf{OPEN:}
\begin{itemize}[nosep]
    \item \sout{Derive $V(\xi)$ from $(\sigma, r_e)$ membrane parameters} $\to$ \textbf{DONE:}
          $V_L, V_R$ structure derived from 5D Dirac, see \S\ref{subsec:opr21_closure}
    \item \sout{Derive boundary conditions from junction physics (Robin from Israel matching?)} $\to$
          \textbf{DONE:} Robin BC from variational principle, see \S\ref{subsec:opr21_closure}
    \item Connect eigenvalue spectrum to generation counting (OPR-02) --- \emph{partially closed:}
          $N_{\text{bound}} = 3$ achieved for $\mu \in [\mu_3^-, \mu_3^+](V)$ (shape-dependent; see OPR-21R), $\mu$ value is \tagP{}
\end{itemize}
...
(see \S\ref{sec:ch3_fermi} for the physical half-line treatment).
...
\begin{tikzpicture}[node distance=1.8cm, >=stealth, font=\small]
    ...
    \node (GF) [draw, rounded corners, left of=I4, xshift=-1.2cm] {$G_{\text{eff}} = g_5^2 \ell |f_1(0)|^2/(2x_1^2)$};
    \draw[->] (brane) -- (pot);
    \draw[->] (pot) -- (bvp);
    \draw[->] (bvp) -- (profile);
    \draw[->] (profile) -- (I4);
...
\begin{itemize}[nosep]
    ...
    \item Ch.~11 ($G_F$ pathway): Closure spine $G_{\text{eff}} = g_5^2 \ell |f_1(0)|^2/(2x_1^2)$ (OPR-22) $\to$ defines what $|f_1(0)|^2$ is for
