...
\midrule
C/D & Scalar profile $f(\xi)$ & DD (implicit) & $\pi$ & $\sim$70 GeV & [P] \\
E   & Scalar profile $f(\xi)$ & NN (explicit) & $\pi/2$ & $\sim$35 GeV & [P] \\
F   & Scalar profile $f(\xi)$ & Robin (scanned) & varies & 35--70 GeV & [P] \\
G   & Scalar profile $f(\xi)$ & Robin ($\alpha = 2\pi$) & $\sim$2.4 & $\sim$53 GeV & [P] \\
\bottomrule
\end{tabular}
\end{table}
...
fixed points are at $\xi = 0$ and $\xi = \ell$ (or equivalently, at the boundaries
of the fundamental domain $[0, \ell]$).

\paragraph{Standard orbifold convention \tagBL{}.}
A field $\phi(\xi)$ must have definite parity under $Z_2$:
\begin{align}
    \textbf{Even parity:} \quad \phi(-\xi) &= +\phi(\xi)
        \quad \Rightarrow \quad \phi'(0) = 0 \text{ (Neumann)}, \\
    \textbf{Odd parity:} \quad \phi(-\xi) &= -\phi(\xi)
