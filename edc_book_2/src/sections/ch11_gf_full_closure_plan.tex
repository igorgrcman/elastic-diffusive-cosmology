...
with the mode overlap integral from \S\ref{sec:ch11_overlap}, an early target formula
for the Fermi constant was:
...
\end{equation}
where $I_4 = \int_0^\ell d\xi \, |f_L(\xi)|^4$ has dimension $[E]$.

\medskip
\textbf{This approach is superseded.} The canonical derivation is in
Chapter~\ref{ch:opr22_geff} (OPR-22), which uses the \textbf{invariant EFT formula}:
\begin{equation}
    \boxed{G_{\text{eff}} = \frac{g_{4,1}^2}{2 m_1^2}}
\end{equation}
with brane-localized current giving $g_{4,1} = g_5 \tilde{f}_1(0)$.

The canonical 5D expression (OPR-20/OPR-22) is:
\begin{equation}
    G_{\text{eff}} = \frac{g_5^2 \, \ell}{2 x_1^2} \cdot |f_1(0)|^2, \quad
    C_{\text{eff}} = \frac{g_5^2 \, \ell}{x_1^2}
\end{equation}
Note: one power of $\ell$, not $\ell^2$. The $|f_1(0)|^2$ factor is dimensionless
(natural normalization), unlike $I_4$ which has dimension $[E]$.
\end{tcolorbox}

\paragraph{Origin of factors (legacy approach).}
\begin{itemize}[nosep]
    \item $g_5^2/x_1^2$: From mediator propagator $\sim g_5^2/m_\phi^2$ with $m_\phi = x_1/\ell$
    \item $\ell^2$: Cancels from $m_\phi^{-2} = \ell^2/x_1^2$ (in bulk overlap approach)
    \item $I_4$: Bulk overlap integral---superseded by brane-localized $|f_1(0)|^2$
\end{itemize}
\textbf{For canonical treatment, see OPR-20 (Ch.~\ref{ch:opr20_mediator_mass}) and OPR-22 (Ch.~\ref{ch:opr22_geff}).}
...
\addlinespace
$f_L(\xi)$ & Left-handed mode profile & \textbf{[OPEN]} & Solve thick-brane BVP (OPR-21) \\
$I_4$ & Overlap integral $\int |f_L|^4 d\xi$ & \textbf{[OPEN]} & Compute from solved profiles \\
\midrule
...
\begin{itemize}[nosep]
    ...
    \item \textbf{$I_4$ value:} Requires solved BVP profiles with physical potential $V(\xi)$
\end{itemize}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Using measured $G_F$ to back-solve:} Setting
          $G_{\text{eff}} = 1.17 \times 10^{-5}$ GeV$^{-2}$ and
...
\begin{tikzpicture}[
    ...
    \node (I4) [fill=red!20, right=of ell] {OPR-21\\$f_L(\xi)$, $I_4$};
...
When OPR-19--21 are closed, the prediction will be (see OPR-22, Ch.~\ref{ch:opr22_geff}):
\begin{equation}
    G_F^{\text{pred}} = \frac{g_5^2 \, \ell}{2 x_1^2} \cdot |f_1(0)|^2
    \quad\stackrel{?}{=}\quad
...
\begin{itemize}[nosep]
    ...
    \item The actual BVP profiles with correct $V(\xi)$ are essential
...
\begin{itemize}[nosep]
    \item Target formula: $G_{\text{eff}} = g_5^2 \ell |f_1(0)|^2 / (2 x_1^2)$ \tagDc{} (Ch.~\ref{ch:opr22_geff})
...
$\square$ & $V(\xi)$ potential & OPR-21 & Derive from membrane parameters \\
$\square$ & $f_L(\xi)$ profiles & OPR-21 & Solve BVP numerically/analytically \\
$\square$ & $I_4$ from profiles & OPR-21 & Compute $\int |f_L|^4 d\xi$ \\
