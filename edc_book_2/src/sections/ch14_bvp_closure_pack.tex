...
\begin{enumerate}[nosep]
    \item[\CheckmarkBold] $V_L, V_R$ derived from 5D Dirac reduction (Eqs.~\ref{eq:opr21:VL}--\ref{eq:opr21:VR})
    \item[\CheckmarkBold] Robin BC derived from variational principle (Eq.~\ref{eq:opr21:robin_bc})
    \item[\CheckmarkBold] Self-adjointness verified (Theorem~\ref{thm:bvp:selfadjoint})
    \item[\CheckmarkBold] $N_{\text{bound}} = 3$ achieved for $\mu \in [25, 35)$ (Eq.~\ref{eq:opr21:three_gen_window})
\end{enumerate}

\textbf{Still required for FULL closure:}
\begin{itemize}[nosep]
    \item Derive $(M_0, \Delta, \ell)$ from membrane parameters $(\sigma, r_e, R_\xi)$
    \item Show $\mu \in [25, 35)$ follows from EDC inputs (linked to OPR-01)
\end{itemize}
