...
\chapter{OPR-21: The BVP as Master Key}
\label{ch:bvp_master_key}
...
\begin{itemize}[nosep]
    \item \textbf{5D cause:} Thick-brane geometry creates an effective potential $V(\xi)$.
...
\midrule
Potential $V(\xi)$ & Bound state count $N_{\text{bound}}$ & OPR-02: Why 3 generations? \\
Boundary conditions & Eigenvalue $x_1$ (first root) & OPR-22: $G_F$ spine scale \\
Domain $[0, \ell]$ or $[0, \infty)$ & Eigenfunctions $\psi_n(\xi)$ & CKM/PMNS: overlap integrals \\
Self-adjointness & Discrete spectrum guarantee & Quantization of masses \\
L/R coupling to $m(\xi)$ & Mode localization profiles & Ch9: V--A structure \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[nosep]
    \item The potential $V(\xi)$ is derived (not fitted) from membrane parameters
...
The thick-brane profile $f(\xi)$ satisfies a Schr\"odinger-type equation:
\begin{equation}
    \boxed{
    \hat{L} f \;=\; -\frac{d^2 f}{d\xi^2} + V(\xi) f \;=\; \lambda f
    }
    \label{eq:bvp:operator}
\end{equation}
where:
\begin{itemize}[nosep]
    \item $\xi \in \Omega$ is the extra-dimensional coordinate (domain specified below)
    \item $V(\xi)$ is the effective potential from membrane geometry \tagP{}
    \item $\lambda$ is the eigenvalue (related to 4D mass: $\lambda \sim m^2$)
    \item $f(\xi)$ is the profile function (wavefunction in extra dimension)
\end{itemize}

\paragraph{Dimensionless form.}
With $\zeta = \xi/\ell$ (where $\ell$ is the brane thickness scale), and
...
\begin{equation}
    \langle f, g \rangle = \int_\Omega f(\xi)^* g(\xi) \, d\xi
...
\int_\Omega w(\xi) f^* g \, d\xi$ with positive weight $w(\xi) > 0$.
...
\tagM{}
The Sturm--Liouville operator $\hat{L} = -d^2/d\xi^2 + V(\xi)$ on $[0, \ell]$ with
Robin BCs~\eqref{eq:bvp:robin_bc} is self-adjoint if and only if:
\begin{enumerate}[nosep]
    \item $V(\xi)$ is real-valued and locally integrable on $(0, \ell)$
...
potential $V(\xi)$ and boundary conditions from the 5D action. This is the key
...
\begin{itemize}[nosep]
    ...
    \item Formal pipeline: action $\to$ variation $\to$ operator $\to$ $V(\xi)$
    \item Specification of what closure would require
\end{itemize}

\textbf{What this section does NOT provide:}
\begin{itemize}[nosep]
    \item Explicit form of $V(\xi)$ (requires completing the pipeline)
...
The derivation of $V(\xi)$ requires combining several contributions from the 5D theory:
...
\end{equation}
where $M_{5,\mathrm{Pl}}$ is the 5D Planck mass and $R_5$ is the 5D Ricci scalar. \tagP{}
...
For a brane at position $\xi = \xi_0$, the Israel junction conditions
...
\begin{equation}
    S_{\text{GHY}} = M_{5,\mathrm{Pl}}^3 \int_{\partial\mathcal{M}} d^4x \sqrt{-h}\, K
...
\begin{enumerate}
    \item \textbf{Background solution:} Solve the Einstein equations with brane
          sources to obtain the warped metric $ds^2 = e^{2A(\xi)}\eta_{\mu\nu}dx^\mu dx^\nu + d\xi^2$.
          The warp factor $A(\xi)$ encodes the brane structure.
    ...
    \item \textbf{Dimensional reduction:} Integrate over the extra dimension to
          obtain an effective 4D theory. The profile functions $f_n(\xi)$ satisfy
    ...
    \item \textbf{Identify operator and potential:} The resulting equation takes
          ...
          \begin{equation}
              V(\xi) = V_{\text{warp}}(z) + V_{\text{mass}}(z) + V_{\text{coupling}}(z)
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!50!black,
    title=\textbf{OPR-21 Closure Condition: $V(\xi)$ Derivation}]
\label{box:opr21_vz_closure}

\textbf{Status:} CONDITIONAL \tagDc{} --- structure derived, parameters \tagP{}

\textbf{Completed (see \S\ref{subsec:opr21_closure}):}
\begin{enumerate}[nosep]
    \item[$\checkmark$] $V_L, V_R$ derived from 5D Dirac reduction (Eqs.~\ref{eq:opr21:VL}--\ref{eq:opr21:VR})
    \item[$\checkmark$] Robin BC derived from variational principle (Eq.~\ref{eq:opr21:robin_bc})
    \item[$\checkmark$] Self-adjointness verified (Theorem~\ref{thm:bvp:selfadjoint})
    \item[$\checkmark$] $N_{\text{bound}} = 3$ achieved for $\mu \in [\mu_3^-, \mu_3^+](V)$ --- shape-dependent (Eq.~\ref{eq:opr21:three_gen_window}, OPR-21R)
\end{enumerate}

\textbf{Still required for FULL closure:}
\begin{itemize}[nosep]
    \item Derive $(M_0, \Delta, \ell)$ from membrane parameters $(\sigma, r_e, R_\xi)$
    \item Show $\mu \in [\mu_3^-, \mu_3^+](V)$ follows from EDC inputs (linked to OPR-01; window is shape-dependent)
\end{itemize}

\textbf{Downstream unlocks (conditional):}
\begin{itemize}[nosep]
    \item OPR-02: $N_{\text{bound}} = 3$ realized; needs parameter derivation
    \item OPR-22: Can compute $x_1$ once parameters fixed
    \item CKM/PMNS: Can compute overlaps from eigenfunctions
\end{itemize}
\end{tcolorbox}
...
\begin{tabular}{lll}
\toprule
\textbf{Part I Parameter} & \textbf{Role in $V(\xi)$} & \textbf{Status} \\
\midrule
...
fix $V(\xi)$. All parameters must trace to membrane physics. This is the
``no-smuggling'' requirement of Framework~2.0.

% ==============================================================================
% OPR-21 CLOSURE DERIVATION — LEARNING-STYLE CHAIN
% ==============================================================================
\input{sections/ch14_opr21_closure_derivation}

% ==============================================================================
\subsection{\texorpdfstring{$V(\xi)$}{V(xi)} Candidates Catalogue (Toy Shapes, Not Derived)}
\label{subsec:bvp_vz_catalogue}

Before deriving $V(\xi)$ from the 5D action, it is useful to survey candidate
potential shapes that commonly arise in thick-brane, domain-wall, and localization
problems. These are \emph{toy ansätze} for intuition and future numerical testing;
none constitutes a derived EDC potential.

\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!60!black,
    title=\textbf{Epistemic Status: Toy Catalogue}]
\label{box:bvp_vz_catalogue_status}

\textbf{Classification:} \tagP{}/[Toy]

\textbf{Purpose:}
\begin{itemize}[nosep]
    \item Provide physically motivated candidate shapes for testing
    \item Build intuition for how potential features map to $N_{\text{bound}}$
    \item Prepare numerical infrastructure for eventual derived $V(\xi)$
\end{itemize}

\textbf{NOT claimed:}
\begin{itemize}[nosep]
    \item These are NOT derived from the 5D action
    \item Using any of these does NOT close OPR-21 or OPR-02
    \item No calibration to PDG values
\end{itemize}
\end{tcolorbox}

\subsubsection{Candidate 1: Volcano (Warp Localization)}
\label{subsubsec:vz_volcano}

\paragraph{Formula (schematic).}
\begin{equation}
    V_{\text{volcano}}(\zeta) = V_0 \left[ \frac{\zeta^2}{\zeta^2 + a^2} - 1 \right]
    \label{eq:bvp:vz_volcano}
\end{equation}
where $\zeta = \xi/\ell$ is dimensionless, $V_0 > 0$ is the well depth, and $a > 0$
sets the core width.

\paragraph{Spectral behavior.}
Deep central well with asymptotic approach to $V = 0$. The number of bound states
$N_{\text{bound}}$ increases stepwise with $V_0$ and $a$. Typical in RS-type
warped geometries where the warp factor derivatives create a volcano-shaped
effective potential.

\paragraph{5D origin.}
Could arise from $V_{\text{warp}} \sim (A')^2 - A''$ where $A(\xi)$ is the warp
factor in $ds^2 = e^{2A(\xi)}\eta_{\mu\nu}dx^\mu dx^\nu + d\xi^2$.
\emph{Status: \tagP{} (toy ansatz).}

\subsubsection{Candidate 2: Kink / Domain Wall (tanh)}
\label{subsubsec:vz_kink}

\paragraph{Formula (schematic).}
\begin{equation}
    V_{\text{kink}}(\zeta) = -V_0 \operatorname{sech}^2(\zeta/a)
    \label{eq:bvp:vz_kink}
\end{equation}
This is the reflectionless Pöschl--Teller potential, arising from a $\tanh$
kink background in scalar-field domain walls.

\paragraph{Spectral behavior.}
Exactly solvable~\cite{Teschl2012}. For integer $s = \sqrt{V_0 a^2 + 1/4} - 1/2$,
the potential supports exactly $\lfloor s \rfloor + 1$ bound states. The bound
state count increases discretely as $V_0$ crosses thresholds.

\paragraph{5D origin.}
The Pöschl--Teller potential arises when fermion modes couple to a scalar kink
$\phi(\xi) = \phi_0 \tanh(\xi/a)$ via Yukawa coupling $m(\xi) = y\phi(\xi)$.
The numerical demo (Sec.~\ref{subsec:bvp_numerical_demo}) uses this family.
\emph{Status: \tagM{} (mathematical; standard physics).}

\subsubsection{Candidate 3: Compact Well (Finite Interval)}
\label{subsubsec:vz_compact}
...
\begin{equation}
    ...
    \label{eq:bvp:vz_box}
\end{equation}
with Dirichlet or Robin BCs at $\zeta = 0$ and $\zeta = 1$.

\paragraph{Spectral behavior.}
Pure discrete spectrum. The count $N_{\text{bound}}$ scales roughly as
$\propto \sqrt{V_0}$ for a square well. Gaps between eigenvalues can be computed
analytically.

\paragraph{5D origin.}
Represents a brane with sharp boundaries---a finite-thickness ``slab'' embedded
in the bulk. Simpler than smooth profiles but may not capture junction physics.
\emph{Status: \tagP{} (toy geometry).}

\subsubsection{Candidate 4: Double-Well (Two-Lump Brane)}
\label{subsubsec:vz_double}

\paragraph{Formula (schematic).}
\begin{equation}
    V_{\text{double}}(\zeta) = -V_0 \left[ \operatorname{sech}^2(\zeta - d) +
        \operatorname{sech}^2(\zeta + d) \right]
    \label{eq:bvp:vz_double}
\end{equation}
where $2d$ is the separation between wells.

\paragraph{Spectral behavior.}
Two degenerate wells can produce near-degenerate eigenvalue pairs (bonding/antibonding).
If the wells are far apart ($d \gg a$), each contributes independently to
$N_{\text{bound}}$; if close, tunnel coupling lifts degeneracy.

\paragraph{5D origin.}
Could model a brane with two localized regions (e.g., matter and antimatter
sectors, or warped throats). Exotic in minimal EDC but relevant for extended models.
\emph{Status: \tagP{} (speculative).}

\subsubsection{Candidate 5: Exponential Tail (Asymptotically Decaying)}
\label{subsubsec:vz_exp}

\paragraph{Formula (schematic).}
\begin{equation}
    V_{\text{exp}}(\zeta) = -V_0 e^{-\zeta/a}
    \label{eq:bvp:vz_exp}
\end{equation}
This decays monotonically into the bulk.

\paragraph{Spectral behavior.}
Supports finitely many bound states if $V_0$ is large enough. The threshold
$\lambda_{\text{th}} = 0$ (essential spectrum onset). Weaker localization than
Pöschl--Teller for same $V_0$.

\paragraph{5D origin.}
Arises when the warp factor $A(\xi)$ decays exponentially (e.g., linear dilaton
backgrounds). The monotonic decay may be relevant for half-line domains with
natural bulk cutoff.
\emph{Status: \tagP{} (toy ansatz).}
...
\begin{tabular}{llll}
\toprule
\textbf{Candidate} & \textbf{Shape} & \textbf{$N_{\text{bound}}$ Scaling} & \textbf{Status} \\
\midrule
Volcano & Central well + asymptote & $\uparrow$ with depth/width & \tagP{} \\
Kink (PT) & $-\operatorname{sech}^2$ & $\propto \sqrt{V_0}$ (discrete steps) & \tagM{} \\
Compact well & Box potential & $\propto \sqrt{V_0}$ & \tagP{} \\
Double-well & Two lumps & Pair structure & \tagP{} \\
Exponential & Monotonic decay & Finitely many & \tagP{} \\
\bottomrule
\end{tabular}
\end{center}

\begin{tcolorbox}[colback=red!5!white, colframe=red!50!black,
    title=\textbf{No-Smuggling Guardrail}]
\label{box:bvp_vz_guardrail}

\textbf{These candidates are for intuition and future numerical testing only.}

\begin{itemize}[nosep]
    \item \textbf{Closure requires:} Deriving $V(\xi)$ and BCs from the 5D action
          (Sec.~\ref{subsubsec:vz_pipeline})
    \item \textbf{No calibration:} Do not tune $V_0$, $a$, or shape parameters to
          match $N_{\text{bound}} = 3$ or any PDG value
    \item \textbf{OPR-21/OPR-02 remain OPEN:} Using a toy potential does not close
          these items
\end{itemize}

\textbf{Cross-references:}
\begin{itemize}[nosep]
    \item $V(\xi)$ derivation pipeline: Sec.~\ref{subsubsec:vz_pipeline}
    \item Numerical demo (Pöschl--Teller): Sec.~\ref{subsec:bvp_numerical_demo}
    \item Closure conditions: Sec.~\ref{subsec:bvp_acceptance}
\end{itemize}
\end{tcolorbox}
...
$|\psi_n(\xi)|^2$ gives the probability density for finding the $n$-th mode at depth $\xi$.
Modes localized near $\xi = 0$ couple strongly to brane-localized interactions.
...
\begin{equation}
    I_4^{(ijkl)} = \int_\Omega \psi_i(\xi) \psi_j(\xi) \psi_k(\xi) \psi_l(\xi) \, d\xi
    \label{eq:bvp:I4_def}
\end{equation}
For the dominant (ground state) contribution:
\begin{equation}
    I_4 \equiv I_4^{(0000)} = \int_\Omega |\psi_0(\xi)|^4 \, d\xi
...
\tagDef{}
For a Schr\"odinger operator $\hat{L} = -d^2/d\xi^2 + V(\xi)$ on domain $\Omega$:
\begin{equation}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Half-line} $[0, \infty)$: $\sigma_{\text{ess}} = [V_\infty, \infty)$
          where $V_\infty = \lim_{\xi \to \infty} V(\xi)$.
\end{itemize}
\end{definition}
...
\tagM{}
Let $\hat{L}$ be a Sturm--Liouville operator with potential $V(\xi)$ on $[0, \ell]$.
...
\begin{enumerate}[nosep]
    \item The potential $V(\xi)$ is \textbf{derived} from membrane parameters $(\sigma, r_e)$
    ...
    \item Numerical computation shows $N_{\text{bound}} = 3$ for the derived $V(\xi)$
...
\textbf{Status:} \tagOPEN{} --- Conditions 1--2 require deriving $V(\xi)$ and BCs
from the 5D action (not yet done). Conditions 4--5 require numerical work.
\end{tcolorbox}
...
\begin{itemize}[nosep]
    ...
    \item ``What if $V(\xi)$ or BCs are slightly different? Does $N_{\text{bound}}$ jump to 2 or 4?''
    \item ``Is this tuned to give 3, or does 3 emerge generically?''
\end{itemize}

\textbf{What must be demonstrated to answer:}
\begin{enumerate}[nosep]
    \item \textbf{Spectral count:} Solve BVP with derived $V(\xi)$; count eigenvalues below
...
\textbf{Failure modes:}
\begin{itemize}[nosep]
    \item $N_{\text{bound}} \neq 3$ for physically motivated $V(\xi)$ $\to$ model falsified
...
Ch9 derived $R_{\text{LR}} \ll 1$ for the half-line domain with $m(\xi) > 0$.
...
\midrule
OPR-02 & $N_{\text{bound}} = 3$ & Robust under BC variations; derived $V(\xi)$ \\
OPR-21 & BVP fully specified & $V(\xi)$, BCs derived from $(\sigma, r_e)$; no fit \\
...
If $V(\xi) \to 0$ too fast at boundaries, modes may not be $L^2$ $\to$ no bound states.
...
If $V(\xi)$ or BCs are tuned to reproduce PDG masses $\to$ no predictive power.
...
\end{itemize}
The spine becomes predictive when these are computed from derived $V(\xi)$.
\end{tcolorbox}
...
\begin{itemize}[nosep]
    \item Derived $V(\xi)$ from membrane parameters (OPR-21 remains open)
    \item Derived BCs from junction physics (part of OPR-21)
    \item Claims that $N_{\text{bound}} = 3$ (requires physical $V(\xi)$)
\end{itemize}
\end{tcolorbox}
...
It is \emph{not} a claim that the toy potential below is the physical EDC potential $V(\xi)$.
Rather, it shows:
\begin{enumerate}[nosep]
    ...
    \item[(ii)] why the statement ``$N_{\text{gen}} = 3$'' must be treated as a
          ...
          $V(\xi)$ and admissible BCs---not as an automatic consequence of
...
\begin{itemize}[nosep]
    ...
    \item Uses a placeholder potential $V(\xi) = -V_0 \operatorname{sech}^2(\xi/a)$ (Pöschl--Teller)
    \item Demonstrates stability of $N_{\text{bound}}$ under truncation and BC variations
\end{itemize}

\textbf{What this is NOT:}
\begin{itemize}[nosep]
    \item NOT a derivation of $V(\xi)$ from 5D membrane action
    \item NOT a claim that $N_{\text{bound}} = 3$ (toy potential gives $N_{\text{bound}} = 2$)
    \item NOT OPR-21 closure
\end{itemize}

\textbf{Status:} OPR-21 remains \tagOPEN{} until $V(\xi)$ and BCs are derived from
membrane physics.
\end{tcolorbox}

\subsubsection{Setup: Half-Line Pöschl--Teller Potential}

We solve the dimensionless Schrödinger-type BVP on the half-line $\xi \in [0, \infty)$:
\begin{equation}
...
with toy potential:
\begin{equation}
    V(\xi) = -V_0 \operatorname{sech}^2(z/a), \qquad V_0 = 10, \quad a = 1
    \label{eq:bvp:toy_potential}
\end{equation}
These parameters are chosen \emph{a priori} (no fitting to any physical data).

\paragraph{Boundary conditions.}
At $\xi = 0$: Robin BC $\psi'(0) + \kappa\psi(0) = 0$ with $\kappa \in \{0, 0.1, 0.5\}$.
At $\xi = \xi_{\max}$: Dirichlet $\psi(\xi_{\max}) = 0$ (truncation of half-line).

\paragraph{Essential spectrum threshold.}
Since $V(\xi) \to 0$ as $\xi \to \infty$, the threshold is $\lambda_{\text{th}} = 0$
...
$\xi_{\max}$ and Robin parameter $\kappa$.

% Include generated table
\begin{table}[htbp]
\centering
\caption{Half-line BVP toy outputs (\tagM{}/[Toy], dimensionless units).
$N_{\text{bound}} = 2$ is stable across all tested $(\xi_{\max}, \kappa)$ pairs,
...
\begin{enumerate}[nosep]
    \item \textbf{$N_{\text{bound}}$ is stable:} All $(\xi_{\max}, \kappa)$ combinations
          yield $N_{\text{bound}} = 2$, confirming Lemma~\ref{lem:bvp:spectral_stability}.
    \item \textbf{Eigenvalues converge:} $E_0$ varies by $< 0.3\%$ across $\xi_{\max}$ values.
    ...
    \item \textbf{NOT three generations:} The toy potential gives 2 bound states,
          not 3. This is expected---the physical $V(\xi)$ (derived from membrane) may
...
\begin{itemize}[nosep]
    \item Derive $V(\xi)$ from membrane parameters $(\sigma, r_e)$
...
fixed ($a=1$, $\xi_{\max}=14$, $\kappa=0$). Table~\ref{tab:bvp_phase_diagram} shows
the result.
...
OPR-02 closure requires deriving $V(\xi)$ from 5D membrane physics.}
...
\begin{enumerate}[nosep]
    \item Derive $V(\xi)$ from membrane parameters $(\sigma, r_e, \ldots)$
...
Figure~\ref{fig:bvp_toy_profile} shows the toy potential $V(\xi)$ and the ground-state
wavefunction $\psi_0(\xi)$ for $V_0 = 10$, $a = 1$. The wavefunction is localized near
$\xi = 0$ where the potential well is deepest, and decays exponentially for $\xi \gg a$.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{code/output/bvp_halfline_toy_figure.pdf}
\caption{Toy BVP profiles: potential $V(\xi) = -V_0 \operatorname{sech}^2(\xi/a)$ (blue)
and normalized ground-state wavefunction $\psi_0(\xi)$ (red) for $V_0 = 10$, $a = 1$.
...
\begin{itemize}[nosep]
    ...
    \item \texttt{code/output/bvp\_halfline\_toy\_figure.pdf} --- $V(\xi)$ and $\psi_0$ plot
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Closure is conditional:} The statement ``$N_{\text{gen}} = 3$'' is a
          \emph{closure condition} that constrains the physically derived $V(\xi)$ and
          admissible BC family, not an automatic consequence of group-quotient arguments.
\end{itemize}

\medskip
\textbf{Consequently:} OPR-02 remains \tagOPEN{} until the physical $V(\xi)$ is derived from
...
\begin{equation}
    \text{derive } V(\xi) \text{ \& BCs from 5D action}
