...
\begin{itemize}[nosep]
    ...
    \item The dimensionless eigenvalue $x_n := m_n \ell$ is determined by the BVP
          (depends on $\kappa$, $V(\xi)$)
    \item The effective contact strength has the invariant form
          $C_{\text{eff}} = g_{4,1}^2 / m_1^2$
\end{itemize}
...
\textbf{Prerequisites:}
\begin{itemize}[nosep]
    \item Chapter~\ref{sec:ch17_opr19} (OPR-19): $g_5 \to g_4$ dimensional reduction
    \item Chapter~\ref{ch:bvp_master_key} (OPR-21): BVP framework and Robin BC structure
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!50!black,
    title=\textbf{Key Definition: Dimensionless Eigenvalue}]
\textbf{Definition.} The \emph{dimensionless eigenvalue} $x_n$ is defined by:
\begin{equation}
...
\textbf{Critical point:} The value of $x_n$ is \emph{not universal}---it is the output of solving
the Sturm--Liouville BVP with given $V(\xi)$ and BC parameters $\kappa_0, \kappa_\ell$:
\begin{equation}
    x_n = x_n(\kappa_0, \kappa_\ell, \tilde{V})
    \label{eq:ch18:xn_dependence}
\end{equation}

\textbf{Toy limit (V = 0, Neumann BC):} For $\tilde{V} = 0$ and $\kappa = 0$, we recover
the familiar result $x_n = n\pi$ (see eigenvalue table in \S\ref{subsec:ch18_eigenvalue_table}).

\textbf{Epistemic status:} Definition \tagM{}; value $x_n(\kappa, V)$ is \tagDc{} given BVP inputs.
\end{tcolorbox}
...
From Chapter~\ref{sec:ch17_opr19}, Eq.~(\ref{eq:ch17:g4_formula}):
\begin{equation}
...
\textbf{Dimensions} (natural units $\hbar = c = 1$):
\begin{itemize}[nosep]
    \item $[g_5^2] = L$ \quad (5D coupling squared has dimension of length)
    \item $[g_4^2] = 1$ \quad (4D coupling squared is dimensionless)
    \item $[\ell] = L$ \quad (domain size)
    \item $[f_n(\xi)] = L^{-1/2}$ \quad (for $\int |f_n|^2 d\xi$ to be dimensionless)
\end{itemize}
...
\begin{equation}
    ...
    \label{eq:ch18:g4_from_g5}
\end{equation}

\textbf{Dimensional check:} $[g_5^2/\ell] = L / L = 1$ \checkmark
\end{tcolorbox}
...
\textbf{Step 1: Invariant structure.} From exchange of the massive mediator:
\begin{equation}
    \boxed{C_{\text{eff}} = \frac{g_{4,1}^2}{m_1^2} \times (\text{overlap factors})}
    \label{eq:ch18:contact_exchange}
\end{equation}
This structure is \emph{universal}---it does not depend on normalization conventions.

\textbf{Step 2: Express in 5D parameters.} Using $g_{4,1}^2 = g_5^2/\ell$ and $m_1 = x_1/\ell$:
\begin{align}
    C_{\text{eff}} &= \frac{g_5^2/\ell}{(x_1/\ell)^2} = \frac{g_5^2/\ell}{x_1^2/\ell^2}
                   = \frac{g_5^2 \ell}{x_1^2}
    \label{eq:ch18:contact_derivation}
\end{align}
...
\textbf{Epistemic status:} \tagDc{} (structure); $g_5$, $\ell$, $x_1(\kappa, V)$ depend on
underlying postulated parameters.
\end{tcolorbox}
...
\midrule
$x_n$ & BVP eigenvalue $x_n(\kappa, V)$ & \tagDc{} (given inputs) \\
$m_n$ & $x_n / \ell$ (by definition) & \tagDc{} \\
$m_{\text{med}}$ & $m_1 = x_1 / \ell$ & \tagDc{} \\
$g_{4,n}$ & $\sqrt{g_5^2 / \ell}$ & \tagDc{} \\
$C_{\text{eff}}$ & $g_{4,1}^2 / m_1^2 = g_5^2 \ell / x_1^2$ & \tagDc{} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{Dimensionless eigenvalue} (Eq.~\ref{eq:ch18:mass_formula}):
          $x_n := m_n \ell$ is defined as output of the BVP; $x_n = x_n(\kappa, V)$.
    ...
    \item \textbf{Effective contact strength} (Eq.~\ref{eq:ch18:contact_final}):
          $C_{\text{eff}} = g_{4,1}^2 / m_1^2 = g_5^2 \ell / x_1^2$
          (invariant structure + OPR-19 normalization).
\end{enumerate}
...
\begin{itemize}[nosep]
    \item \textbf{OPR-19} (Chapter~\ref{sec:ch17_opr19}): $g_5 \to g_4$ reduction,
          warp factor cancellation
    \item \textbf{OPR-21} (Chapter~\ref{ch:bvp_master_key}): BVP framework, Robin BC structure,
...
% ==============================================================================
\subsection{OPR-20 Stoplight Verdict}
\label{subsec:opr20_stoplight}

\begin{tcolorbox}[colback=yellow!10!white, colframe=orange!60!black,
    title=\textbf{OPR-20: Stoplight Verdict}]

\textbf{Status: \textcolor{YellowOrange}{YELLOW} [Dc]}

\textbf{What is derived:}
\begin{itemize}[nosep]
\item $m_{\text{med}} = x_1/\ell$ from Sturm-Liouville eigenvalue
\item Eigenvalue equation structure
\item BC dependence mapping
\end{itemize}

\textbf{Blockers to GREEN:}
\begin{itemize}[nosep]
\item $V(\xi)$ not derived from 5D gauge action (OPEN-20-1)
\item BC parameter $\kappa$ not derived from Israel junction (OPEN-20-2)
\item $\ell$ not derived from first principles (OPEN-20-3)
\end{itemize}

\textbf{Next upgrade:} Numerical eigenvalue computation for realistic $V(\xi)$.
