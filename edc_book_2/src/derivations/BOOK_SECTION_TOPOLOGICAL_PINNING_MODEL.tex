...
% ============================================================================
\subsubsection{Complementary Analysis: Coordination Distance $d(n)$ [I]}
\label{subsubsec:dn-analysis}

An alternative approach to quantifying topological frustration uses the \textbf{coordination distance} $d(n)$, defined as the distance from the effective coordination number to the nearest allowed value:
\begin{equation}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=V7.8 Audit Results {[Der]}]
Analysis of 106 $\alpha$-emitters ($Z = 83$--100) with hindrance controls yields:
\begin{align}
    \log_{10}(t_{1/2}) &= a \frac{Z}{\sqrt{Q_\alpha}} + b + c_1 I(\text{H1}) + c_2 I(\text{H2}) + g \cdot d(n) \\
    g &= -1.64 \pm 0.14, \quad p < 0.001, \quad R^2 = 0.9805 \nonumber
\end{align}
\textbf{Key finding}: The negative coefficient confirms that higher $d(n)$ (more frustration) correlates with \emph{faster} decay, consistent with the $\varepsilon_f$ analysis above.

\textbf{Robustness tests passed}:
\begin{itemize}
    \item Permutation test: $p_{\text{perm}} < 0.001$
    \item Cross-validation: predictive improvement confirmed
    \item Deformation proxy control: absorbed by $d(n)$ ($p = 0.67$ when both included)
    \item Full model (M7): $g = -1.71$, only 4.2\% change from baseline
\end{itemize}

\textbf{Interpretation}: The effect is most consistent with \textbf{prefactor (S$_\alpha$) enhancement}, not barrier modification. Frustration enhances $\alpha$-cluster preformation probability.

\textit{Source: audit/radioactivity\_v7\_8\_Salpha\_deformation/07\_FIT\_RESULTS\_V7\_8.md}
\end{tcolorbox}
...
Fitted coefficient & $c = -2.40$ & $g = -1.64$ \\
$R^2$ & 0.9941 & 0.9805 \\
...
Both approaches confirm the central prediction: \textbf{topological frustration accelerates $\alpha$-decay}.
...
\begin{tcolorbox}[colback=orange!5!white, colframe=orange!75!black, title=Open Questions from Topological Pinning Model]
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=Topological Pinning Model: Final Verdict]
