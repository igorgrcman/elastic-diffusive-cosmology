...
\title{Neutron Lifetime from 5D Topology\\[0.5em]\large Book Section Draft}
\author{EDC Project}
\date{January 2026}
...
\begin{tabular}{lcccc}
\toprule
& $L_0/\delta$ & $m_p$ prediction & $\tau_n$ (with $A=1$) & $A$ needed \\
\midrule
Route S ($\pi^2$) & 9.87 & 923 MeV ($-1.6\%$) & $\sim$30,000 s & \textbf{0.03} \\
Route D ($r_p+\delta$) & 9.33 & 985 MeV ($+4.9\%$) & $\sim$700 s & \textbf{0.8} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{equation}
\frac{\tau_n^{\text{EDC}}}{\tau_n^{\text{exp}}} = \frac{879}{879.4} \approx 1.00
\end{equation}

Agreement within $<1\%$ with prefactor $A \approx 0.84$ (O(1), no fine-tuning).
\end{tcolorbox}
%------------------------------------------------------------------------------
...
\begin{enumerate}
\item \textbf{Reproduces $\tau_n$} within $\sim$20\% using O(1) prefactor
...
\noindent\textit{But the neutron attempts to tunnel extremely rapidly---$10^{22}$ times per second. The combination $10^{22} \times 10^{-26} = 10^{-4}$ per second gives a lifetime of about $10^4$ seconds, which is the correct order of magnitude.}
