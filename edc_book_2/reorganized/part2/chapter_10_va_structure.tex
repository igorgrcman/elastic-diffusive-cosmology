...
(\emph{Terminology:} ``edge'' refers to localized modes near the boundary/transition
region of the brane microstructure along $\xi$; see Appendix~\ref{app:notation}.)
...
deferred to Chapter~\ref{ch:bvp} (BVP Work Package).
...
\textbf{The equation.} The 5D Dirac equation with position-dependent mass is:
\begin{equation}
    \left( i\gamma^\mu \partial_\mu + i\gamma^5 \partial_\xi - m(\xi) \right) \Psi = 0
    \label{eq:ch9_5d_dirac}
\end{equation}
This is standard relativistic quantum mechanics extended to 5D (no [BL] tag---it is
mathematical structure, not empirical data).
...
Define the 4D chiral projectors \tagM{}:
\begin{equation}
...
BVP work package, Chapter~\ref{ch:bvp}). The zero-mode analysis captures the leading
...
separating chiralities gives the coupled first-order equations \tagDer{}:
\begin{align}
...
\begin{equation}
    m(\xi) \sim \kappa_T \left( T^{\xi\xi}(\xi) - T^{\xi\xi}(0) \right)
    \label{eq:ch9_mass_from_stress}
\end{equation}
where $\kappa_T > 0$ is the fermion--stress coupling constant.
(\emph{Notation:} $\kappa_T$ here differs from the localization parameter $\kappa$
used in Chapters~\ref{ch:neutrinos} and~\ref{ch:ckm}; subscript $T$ denotes coupling
to stress tensor.)

\textbf{Dimensional check:} $[m] = \text{mass} = 1/\text{length}$ (in $\hbar = c = 1$),
$[T^{\xi\xi}] = \text{energy}/\text{volume} = \text{mass}^4$, so $[\kappa_T] = 1/\text{mass}^3$.
...
\textbf{The consequence.} Since Plenum flows inward, the stress $T^{\xi\xi}$ is
larger in the bulk than at the boundary:
\begin{equation}
    T^{\xi\xi}(\xi) > T^{\xi\xi}(0) \quad \text{for } \xi > 0
...
\begin{equation}
    m(\xi) = m_0 \left(1 - e^{-\xi/\lambda}\right) \approx m_0 \frac{\xi}{\lambda}
    \quad \text{for small } \xi
...
\end{itemize}
The finite-interval case is addressed in the BVP work package (Chapter~\ref{ch:bvp}).
...
\begin{equation}
    f_L^{\text{(toy)}}(\xi) = \frac{1}{(\pi w_L^2)^{1/4}} \exp\left(-\frac{\xi^2}{2w_L^2}\right)
    \label{eq:ch9_toy_fL}
\end{equation}
...
\begin{equation}
    f_R^{\text{(toy)}}(\xi) = \frac{1}{(\pi w_R^2)^{1/4}} \exp\left(-\frac{(\xi - \xi_0)^2}{2w_R^2}\right)
    \label{eq:ch9_toy_fR}
\end{equation}
This is peaked at $\xi = \xi_0 > 0$ with width $w_R$.
...
\begin{itemize}[nosep]
    ...
    \item $\xi_0$: displacement of right-handed mode into bulk
\end{itemize}
...
Chapter~\ref{ch:bvp}; $C$ remains model-dependent via the profile shape.
...
The chirality asymmetry ratio $R_{\text{LR}}$ is defined in the BVP framework
(Chapter~\ref{ch:bvp}). The overlap integrals $I_4$ and related quantities
can be computed once the BVP is solved with a derived $V(\xi)$.

\paragraph{Cross-references.}
See Chapter~\ref{ch:bvp} for the complete BVP framework including eigenvalue
calculations and overlap integrals.
...
% BACKFILL-GAP1: Dictionary box BEGIN
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!50!black,
    title=\textbf{Dictionary: EDC Chirality $\leftrightarrow$ SM V–A}]
\textbf{EDC geometric result} \tagDc{}:
\begin{itemize}[nosep]
\item Boundary-localized modes satisfy $P_L \Psi|_{\xi=0} = \Psi|_{\xi=0}$
\item Effective coupling: $\mathcal{L} \propto \bar\psi \gamma^\mu P_L \psi \, W_\mu$
\end{itemize}

\textbf{SM phenomenological encoding} \tagBL{}:
\begin{itemize}[nosep]
\item Left-handed doublets: $(\nu_L, e_L)^T$ transform under $SU(2)_L$
\item Charged current: $J^\mu_{\text{CC}} = \bar\psi \gamma^\mu \tfrac{1}{2}(1-\gamma^5) \psi$
\end{itemize}

\textbf{Dictionary identification} \tagDc{}:
\[
\underbrace{\text{EDC boundary mode}}_{\text{5D cause}} \;\longleftrightarrow\; \underbrace{\text{SM } SU(2)_L \text{ doublet}}_{\text{3D shadow}}
\]
The chirality selection from 5D localization \emph{is} what SM encodes via gauge
quantum number assignments. This is an identification of descriptions, not a
derivation of SM group structure (which remains \tagOpen{}).
\end{tcolorbox}
% BACKFILL-GAP1: Dictionary box END
...
Fermion-stress coupling $m(\xi) \sim \kappa_T T^{\xi\xi}$ & Physical hypothesis & \tagP{} \\
